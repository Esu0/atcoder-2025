...
fn mpow(mut base: u32, mut exp: u32, m: u32) -> u32 {
    let mut res = 1;
    while exp > 0 {
        if exp & 1 != 0 {
            res = res * base % m;
...
fn repdigrem(l: u32, m: u32) -> u32 {
    if l == 0 {
        return 0;
    }
    if l.is_multiple_of(2) {
        (1 + mpow(10, l / 2, m)) * repdigrem(l / 2, m) % m
    } else {
        (1 + (10 + mpow(10, l / 2 + 1, m)) * repdigrem(l / 2, m)) % m
    }
}

type MInt = modulo::ModInt<10007>;
...
fn main() {
    ...
    let mut rem = 0;
    let mut p10 = 1;
    for &(ci, li) in cl.iter().rev() {
        rem = (rem + p10 * repdigrem(li, m) % m * ci as u32) % m;
        p10 = p10 * mpow(10, li, m) % m;
    }

    // eprintln!("{rem}");
    let mut rem2 = MInt::new(-(rem as i64));
    let mut p10 = MInt::new(1);
    let inv9 = MInt::new(9).inv();
    for &(ci, li) in cl.iter().rev() {
        rem2 += MInt::new(ci as _) * (MInt::new(10).pow(li) - MInt::new(1)) * inv9 * p10;
        p10 *= MInt::new(10).pow(li);
    }

    rem2 *= MInt::new(m as _).inv();
    println!("{}", rem2);
}
