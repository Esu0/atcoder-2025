use proconio::input;
use modulo::combination::Factorials;
type Fact = Factorials<1000000007>;
...
fn main() {
    ...
    // for i in 0..=m {
    //     sum += f.multi_choose(m - i + 1, l - 1);
    //     acc[i] = sum;
    // }
    let mut ans = f.multi_choose(m + 1, l - 1) * f.multi_choose(m + 1, n - l);
    for j in 1..=m {
        ans += f.multi_choose(m + 1 - j, l - 1) * f.multi_choose(m + 1, n - l - 1);
    }
    println!("{ans}");
}
pub mod modulo {
