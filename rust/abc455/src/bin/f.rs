...
impl MapMonoid for OpAdd {
    ...
    fn mapping(f: &Self::F, x: &<Self::M as Monoid>::S) -> <Self::M as Monoid>::S {
        (x.0 + *f * x.2, x.1 + *f * *f * x.2 + x.0 * *f * MInt::new(2), x.2)
    }
}
#[proconio::fastout]
fn main() {
    ...
    let mut seg = LazySegtree::<OpAdd>::from(vec![(MInt::new(0), MInt::new(0), MInt::new(1)); n]);
    ...
    for _ in 0..q {
        ...
        let l = l - 1;
        seg.apply_range(l..r, MInt::new(a as _));
        let (s, s2, _) = seg.prod(l..r);
        println!("{}", (s * s - s2) * inv2);
    }
}
pub mod modulo {
