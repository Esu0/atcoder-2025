...
fn dfs(memo: &mut [Vec<Option<MInt>>], n: usize, k: usize) -> MInt {
    if k > n {
        return MInt::new(0);
    }
    if n == 0 {
        return if k == 0 { MInt::new(1) } else { MInt::new(0) };
    }
    if k == 0 {
        return MInt::new(0);
    }
    ...
    let ans = dfs(memo, n - 1, k - 1) + dfs(memo, n, k * 2);
    memo[n][k] = Some(ans);
    ans
}
