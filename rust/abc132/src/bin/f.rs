type MInt = modulo::ModInt<1000000007>;
...
fn main() {
    ...
    // let mut dp0 = vec![MInt::new(1); n / (B + 1) + 1];
    // dp0[0] = MInt::new(0);
    // let mut dp1 = vec![MInt::new(0); B + 1];
    // for i in 1..=B {
    //     dp1[i] = MInt::new((n / i - n / (i + 1)) as _);
    // }
    // let mut ndp0 = vec![MInt::new(0); dp0.len()];
    // let mut ndp1 = vec![MInt::new(0); dp1.len()];
    let mut dp = vec![MInt::new(1); n + 1];
    dp[0] = MInt::new(0);
    ...
    for _ in 1..k {
        ...
        dp.clone_from_slice(&ndp);
    }
    let ans = dp.iter().copied().reduce(|a, b| a + b).unwrap();
    println!("{ans}");
}
pub mod modulo {
    use std::{
