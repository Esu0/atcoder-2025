...
fn main() {
    ...
    b.sort_unstable();
    let mut cumsum = vec![MInt::new(0); m + 1];
    for i in 0..m {
        cumsum[i + 1] = cumsum[i] + MInt::new(b[i] as _);
    }

    let mut ans = MInt::new(0);
    for &ai in &a {
        let j = b.partition_point(|&x| x < ai);
        ans += MInt::new(ai as _) * MInt::new(j as _) - cumsum[j];
        ans += cumsum[m] - cumsum[j] - MInt::new(ai as _) * MInt::new((m - j) as _);
    }
    println!("{ans}");
}
