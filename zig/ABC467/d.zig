...
pub fn solve() !void {
    const t = readInt(u32);
    for (0..t) |_| {
        var p: [4]struct {i32, i32} = undefined;
        inline for (0..4) |i| {
            p[i] = .{ readInt(i32), readInt(i32) };
        }
        const d1 = .{p[0][0] - p[1][0], p[0][1] - p[1][1]};
        const d2 = .{p[2][0] - p[3][0], p[2][1] - p[3][1]};
        if (@as(i64, d1[0]) * d2[1] == @as(i64, d1[1]) * d2[0]) {
            const m1: struct {i64, i64} = .{p[0][0] + p[1][0], p[0][1] + p[1][1]};
            const m2: struct {i64, i64} = .{p[2][0] + p[3][0], p[2][1] + p[3][1]};
            const d3 = .{m1[0] - m2[0], m1[1] - m2[1]};
            if (d3[0] * d1[0] + d3[1] * d1[1] == 0) {
                print("Yes\n", .{});
            } else {
                print("No\n", .{});
            }
