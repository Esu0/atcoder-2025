...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const l = readInt(u32);
    var a: [500]u32 = undefined;
    for (0..n) |i| a[i] = readInt(u32);
    var memo: [500][500]u32 = @splat(@splat(0));
    {
        var i: u32 = 0;
        while (i < m) : (i += 1) {
            for (0..n) |j| {
                memo[i][j % l] += (i + m - a[j]) % m;
            }
        }
    }
    // for (0..m) |i| std.debug.print("{any}\n", .{memo[i][0..l]});
    var dp: [500]u32 = @splat(math.maxInt(u32));
    dp[0] = 0;
    {
        var i: u32 = 0;
        while (i < l) : (i += 1) {
            var ndp: [500]u32 = @splat(math.maxInt(u32));
            var j: u32 = 0;
            while (j < m) : (j += 1) {
                var k: u32 = 0;
                while (k < m) : (k += 1) {
                    ndp[(j + k) % m] = @min(ndp[(j + k) % m], dp[j] +| memo[k][i]);
                }
            }
            dp = ndp;
            // std.debug.print("{any}\n", .{dp[0..m]});
