...
pub fn solve() !void {
    ...
    const dist = try allocator.alloc(u32, n + 1);
    var i: u32 = 1;
    const ans = try allocator.alloc(u32, n);
    @memset(ans, 0);
    while (i <= n) : (i += 1) {
        var queue: FixedQueue(u32, 2<<10) = .{};
        queue.push(i);
        @memset(dist[1..n+1], math.maxInt(u32));
        dist[i] = 0;
        while (queue.pop()) |u| {
            const d = dist[u] + 1;
            if (u != 1 and dist[u - 1] == math.maxInt(u32)) {
                queue.push(u - 1);
                dist[u - 1] = d;
            }
            if (u != n and dist[u + 1] == math.maxInt(u32)) {
                queue.push(u + 1);
                dist[u + 1] = d;
            }
            if (u == x and dist[y] == math.maxInt(u32)) {
                queue.push(y);
                dist[y] = d;
            }
            if (u == y and dist[x] == math.maxInt(u32)) {
                queue.push(x);
                dist[x] = d;
            }
        }
