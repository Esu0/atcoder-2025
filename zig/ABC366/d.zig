...
pub fn solve() !void {
    const n = readInt(u32);
    var a: [101][101][101]u32 = @splat(@splat(@splat(0)));
    for (0..n) |i| {
        for (0..n) |j| {
            for (0..n) |k| a[i+1][j+1][k+1] = readInt(u32);
        }
    }
    for (0..n+1) |i| {
        for (0..n+1) |j| {
            for (0..n) |k| a[i][j][k + 1] += a[i][j][k];
        }
    }
    for (0..n+1) |i| {
        for (0..n) |j| {
            for (0..n+1) |k| a[i][j+1][k] += a[i][j][k];
        }
    }
    for (0..n) |i| {
        for (0..n+1) |j| {
            for (0..n+1) |k| a[i+1][j][k] += a[i][j][k];
        }
    }
    const q = readInt(u32);
    for (0..q) |_| {
        const lx = readInt(u32) - 1;
        const rx = readInt(u32);
        const ly = readInt(u32) - 1;
        const ry = readInt(u32);
        const lz = readInt(u32) - 1;
        const rz = readInt(u32);
        const ans = a[rx][ry][rz] -% a[rx][ry][lz] -% a[rx][ly][rz] -% a[lx][ry][rz] +% a[rx][ly][lz] +% a[lx][ry][lz] +% a[lx][ly][rz] -% a[lx][ly][lz];
        print("{d}\n", .{ans});
    }
}
