...
pub fn solve() !void {
    ...
    var col: [2<<17]?u1 = @splat(null);
    var u: u32 = 0;
    var stack: std.ArrayList(u32) = try .initCapacity(allocator, n);
    while (u < n) : (u += 1) {
        if (col[u] != null) continue;
        ...
        while (stack.pop()) |v| {
            ...
            for (g[v].items) |w| {
                if (col[w]) |cw| {
                    ...
                    // std.debug.print("{d} -> {d}\n", .{v, w});
                    col[w] = c;
                    stack.appendAssumeCapacity(w);
                    (if (c == 0) &c0 else &c1).* += 1;
                }
            }
        }
        // std.debug.print("cc: {d} {d}\n", .{c0, c1});
        const vs = c0 + c1;
        if (vs == 1) iso += 1 else if (vs % 2 == 1 and vs >= 2) eo += 1 else if (c0 % 2 == 0) ee += 1 else oo += 1;
        x += @as(u64, c0) * c1;
    }
    // std.debug.print("ee: {d}, eo: {d}, oo: {d}\n", .{ee, eo, oo});
    x -= m;
    var ans: bool = undefined;
    if (n % 2 == 1) {
        ans = (oo + x) % 2 == 1;
    } else if (eo == 0) {
        ans = (iso / 2 + x) % 2 == 1;
    } else if (eo == 1 or eo == 2) {
        ans = true;
    } else {
        ans = (oo + x) % 2 == 1;
    }
    try stdout.writeAll(if (ans) "Aoki\n" else "Takahashi\n");
}
