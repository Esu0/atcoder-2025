...
pub fn solve() !void {
    ...
    const x = readInt(u32);
    var wsum: u64 = 0;
    var w: [4<<17]u32 = undefined;
    var wcumsum: [4<<17]u64 = undefined;
    wcumsum[0] = 0;
    for (0..n) |i| {
        w[i] = readInt(u32);
        w[i+n] = w[i];
    ...
    for (0..n+n) |i| wcumsum[i+1] = wcumsum[i] + w[i];

    var cnt: [2<<17]u32 = undefined;
    var double: [40][2<<17]u32 = undefined;

    for (0..n) |i| {
        const a = x % wsum;
        const offset = wcumsum[i];
        const k = std.sort.lowerBound(u64, wcumsum[0..n+n+1], offset + a, struct {fn compare(lhs: u64, rhs: u64) math.Order {
            return math.order(lhs, rhs);
        }}.compare);
        cnt[i] = @intCast(k - i + x / wsum * n);
        double[0][i] = @intCast(k % n);
    }
    std.debug.print("{any}\n", .{cnt[0..n]});
    for (1..40) |e| {
        for (0..n) |i| {
            double[e][i] = double[e - 1][double[e - 1][i]];
        }
    }
    for (0..q) |_| {
        const k = readInt(u64) - 1;
        var j: u32 = 0;
        comptime var i = 0;
        inline while (i < 40) : (i += 1) {
            if (k & (1 << i) != 0) {
                j = double[i][j];
            }
        }
