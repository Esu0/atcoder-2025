...
pub fn solve() !void {
    ...
    inline for (0..8) |i| s[i] = readString();
    var p: [8][8]bool = @splat(@splat(true));
    @setEvalBranchQuota(10000);
    inline for (0..8) |i| {
        inline for (0..8) |j| {
            if (s[i][j] == '#') {
                inline for (0..8) |k| p[i][k] = false;
                inline for (0..8) |k| p[k][j] = false;
            }
        }
    }
    var ans: u32 = 0;
    inline for (0..8) |i| {
        inline for (0..8) |j| ans += @intFromBool(p[i][j]);
    }
    print("{d}\n", .{ans});
}
