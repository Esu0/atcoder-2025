...
pub fn solve() !void {
    ...
    const a = readInt(u32);
    const b = readInt(u32);
    var g: [1<<20]std.ArrayList(u32) = undefined;
    for (0..m*m) |i| g[i] = .initBuffer(&.{});
    var stack = try std.ArrayList(u32).initCapacity(allocator, m * m);
    var vis: [1<<20]bool = @splat(false);
    {var i: u32 = 0; while (i < m) : (i += 1) {
        var j: u32 = 0;
        while (j < m) : (j += 1) {
            const ni = (i * a + j * b) % m;
            const nj = i;
            try g[ni*m+nj].append(allocator, i * m + j);
            if (i == 0 or j == 0) {
                stack.appendAssumeCapacity(i * m + j);
                vis[i*m+j] = true;
            }
        }
    }}
    ...
    var ans: u32 = 0;
    for (vis[0..m*m]) |f| ans += @intFromBool(!f);
    print("{d}\n", .{ans});
}
