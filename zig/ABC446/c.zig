...
pub fn solve() !void {
    ...
    for (0..t) |_| {
        ...
        var i: u32 = 0;
        while (i < n) : (i += 1) {
            for (0..a[i]) |_| queue.push(i);
            for (0..b[i]) |_| assert(queue.pop() != null);
            if (i < d) continue;
            while (queue.wp != queue.rp and queue.buf[queue.rp] <= i - d) assert(queue.pop() != null);
        }
        print("{d}\n", .{queue.wp - queue.rp});
    }
}
