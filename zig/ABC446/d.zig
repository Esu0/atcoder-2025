...
pub fn solve() !void {
    ...
    for (0..n) |_| {
        const a = readInt(u32);
        const v = (map.get(a - 1) orelse 0) + 1;
        const result = try map.getOrPut(a);
        if (result.found_existing) result.value_ptr.* = @max(result.value_ptr.*, v) else result.value_ptr.* = v;
    }
    var iter = map.valueIterator();
    var ans: u32 = 0;
    while (iter.next()) |v| ans = @max(v.*, ans);
    print("{d}\n", .{ans});
}
