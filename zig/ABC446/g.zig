...
pub fn solve() !void {
    const n = readInt(u32);
    var dp: [5<<17]std.ArrayList(MInt) = undefined;
    for (1..n+1) |i| dp[i] = .initBuffer(&.{});
    var ans = MInt.zero;
    for (0..n) |_| {
        const a = readInt(u32);
        if (dp[a].items.len >= a) {
            ans = ans.sub(dp[a].items[dp[a].items.len - a]);
        }
        try dp[a].append(allocator, ans.add(.fromRaw(1)));
        if (dp[a].items.len >= a) {
            ans = ans.add(dp[a].items[dp[a].items.len - a]);
        }
    }
    print("{d}\n", .{ans.value});
}
