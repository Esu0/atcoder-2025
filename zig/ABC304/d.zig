...
const safety = false;
fn compare(lhs: u32, rhs: u32) math.Order {
...
pub fn solve() !void {
    ...
    _ = readInt(u32);
    const n = readInt(u32);
    var pq: [2<<17]struct {u32, u32} = undefined;
    for (0..n) |i| pq[i] = .{ readInt(u32), readInt(u32) };
    const acnt = readInt(u32);
    const a = try allocator.alloc(u32, acnt);
    for (0..a.len) |i| a[i] = readInt(u32);
    const bcnt = readInt(u32);
    ...
    for (pq[0..n]) |pqi| {
        ...
        const r = std.sort.upperBound(u32, b, qi, compare);
        (try set.getOrPutValue(.{ @intCast(c), @intCast(r) }, 0)).value_ptr.* += 1;
    }
    var min: u32 = if (set.count() != @as(u64, a.len + 1) * (b.len + 1)) 0 else math.maxInt(u32);
    var max: u32 = 0;
