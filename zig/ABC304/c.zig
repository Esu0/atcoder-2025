...
pub fn solve() !void {
    ...
    var xy: [2000]struct {i32, i32} = undefined;
    for (0..n) |i| {
        const xi = readInt(i32);
        const yi = readInt(i32);
        for (0..i) |j| {
            const xj, const yj = xy[j];
            const dx = xi - xj;
            const dy = yi - yj;
            if (dx * dx + dy * dy <= d2) _ = uf.unite(@intCast(i), @intCast(j));
        }
        xy[i] = .{xi, yi};
    }
    var i: u32 = 0;
    while (i < n) : (i += 1) {
        try stdout.writeAll(if (uf.find(0) == uf.find(i)) "Yes\n" else "No\n");
    }
}
