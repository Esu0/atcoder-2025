...
pub fn solve() !void {
    const n = readInt(u32);
    var a: [500]u64 = undefined;
    for (0..n) |i| a[i] = readInt(u64);
    var cumxor: [501]u64 = undefined;
    cumxor[0] = 0;
    for (0..n) |i| cumxor[i + 1] = cumxor[i] ^ a[i];

    var dp: [501][501]u64 = undefined;
    @memset(dp[0][0..n+1], 0);
    @memset(dp[1][0..n], 0);
    var d: u32 = 2;
    while (d <= n) : (d += 1) {
        if (d % 2 == 0) {
            var l: u32 = 0;
            while (l + d <= n) : (l += 1) {
                var v: u64 = math.maxInt(u64);
                const all_xor = cumxor[l + d] ^ cumxor[l];
                var m: u32 = 1;
                while (m < d) : (m += 1) {
                    v = @min(v, dp[m][l] + dp[d-m][l+m] + m%2 * 2 * all_xor);
                }
                dp[d][l] = v;
            }
        } else {
            ...
            while (l + d <= n) : (l += 1) {
                ...
                var m: u32 = 0;
                while (m < d) : (m += 2) {
                    v = @min(v, dp[m][l] + dp[d-m-1][l+m+1]);
                }
                dp[d][l] = v;
            }
        }
    }
    print("{d}\n", .{if (n % 2 == 1) dp[n][0] + cumxor[n] else dp[n][0]});
}
