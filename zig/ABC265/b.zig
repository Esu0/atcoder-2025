...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var t: i64 = readInt(u32);
    var a: [1<<17]u32 = undefined;
    for (1..n) |i| a[i] = readInt(u32);
    // var xy: [1<<17]struct {u32, u32} = undefined;
    // for (0..m) |i| xy[i] = .{ readInt(u32), readInt(u32) };
    var cur: u32 = 1;
    for (0..m) |_| {
        const x = readInt(u32);
        const y = readInt(u32);
    ...
    while (cur < n) : (cur += 1) t -= a[cur];
    try stdout.writeAll(if (t <= 0) "No\n" else "Yes\n");
}
