...
pub fn solve() !void {
    ...
    var queue: FixedDeque(struct{u32, u32}, 1<<20, 1<<20) = .{};
    var dist: [502][502]u32 = @splat(@splat(math.maxInt(u32)));
    queue.pushBack(.{502+1, 0});
    while (queue.popFront()) |elem| {
        const u, const d = elem;
        const i = u / 502;
        const j = u % 502;
        if (dist[i][j] != math.maxInt(u32)) {
            continue;
        }
        dist[i][j] = d;
        const nij: [4]struct {u32, u32} = .{
            .{ i - 1, j },
            .{ i + 1, j },
            .{ i, j - 1 },
            .{ i, j + 1 },
        ...
        for (nij) |nijk| {
            const ni, const nj = nijk;
            if (s[ni][nj] == 0) continue;
            if (s[ni][nj] == '#') {
                queue.pushBack(.{ ni * 502 + nj, d + 1 });
            } else {
                queue.pushFront(.{ ni * 502 + nj, d });
            }
        }
    }
    print("{d}\n", .{dist[n][m]});
