...
pub fn solve() !void {
    ...
    var b: [2<<17]u32 = undefined;
    var brev: [2<<17]u32 = undefined;
    for (0..n) |i| {
        b[i] = readInt(u32) - 1;
        brev[b[i]] = @intCast(i);
    }
    ...
    for (0..n) |i| {
        mem.sortUnstable(u32, g[i].items, b[0..n], struct {fn lessThan(ctx: []u32, lhs: u32, rhs: u32) bool {
            return ctx[lhs] > ctx[rhs];
        }}.lessThan);
    }

    var vis: [2<<17]bool = @splat(false);
    var ans: std.ArrayList(u32) = try .initCapacity(allocator, n);
    var ctx: [2<<17]u32 = @splat(0);
    {
        const u: u32 = 0;
        vis[u] = true;
        ans.appendAssumeCapacity(u);
        var cur: u32 = u;
        next: while (true) {
            // std.debug.print("cur: {d}\n", .{cur});
            while (ctx[cur] < g[cur].items.len) {
                const to = g[cur].items[ctx[cur]];
                ctx[cur] += 1;
                if (!vis[to]) {
                    cur = to;
                    vis[to] = true;
                    ans.appendAssumeCapacity(to);
                    continue :next;
                }
            }
            break;
