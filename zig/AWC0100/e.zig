...
pub fn solve() !void {
    ...
    var s: [2<<17]u32 = undefined;
    for (0..n) |i| {
        a[i] = readInt(u32);
        s[i] = a[i];
    }
    mem.sortUnstable(u32, s[0..n], {}, std.sort.desc(u32));
    for (0..n) |i| {
        const ai = a[i];
        const k = std.sort.lowerBound(u32, s[0..n], ai, struct {fn compare(lhs: u32, rhs: u32) math.Order {
