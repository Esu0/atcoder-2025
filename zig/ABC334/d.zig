...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var r: [2<<17]u32 = undefined;
    for (0..n) |i| r[i] = readInt(u32);
    var c: [2<<17]u64 = undefined;
    mem.sortUnstable(u32, r[0..n], {}, std.sort.asc(u32));
    c[0] = 0;
    for (0..n) |i| c[i + 1] = c[i] + r[i];
    for (0..q) |_| {
        const x = readInt(u64);
        const k = std.sort.upperBound(u64, c[0..n+1], x, struct {fn compare(lhs: u64, rhs: u64) math.Order {
