...
pub fn solve() !void {
    const s = readString();
    const t = readString();
    var dp: [3001][3001]u32 = undefined;
    @memset(&dp[0], 0);
    for (0.., s) |i, si| {
        dp[i + 1][0] = 0;
        for (0.., t) |j, tj| {
    ...
    var ans: std.ArrayList(u8) = try .initCapacity(allocator, dp[s.len][t.len]);
    var i = s.len;
    var j = t.len;
    while (i != 0 and j != 0) {
        const k = dp[i][j];
        if (s[i - 1] == t[j - 1] and dp[i - 1][j - 1] == k - 1) {
            ans.appendAssumeCapacity(s[i - 1]);
            i -= 1;
            j -= 1;
        } else if (dp[i - 1][j] == k) {
            i -= 1;
