...
pub fn solve() !void {
    const n = readInt(u32);
    var k = readInt(u64);
    var mat: [50][50]MInt = @splat(@splat(.zero));
    for (0..n) |i| mat[i][i] = .one;
    var a: [50][50]MInt = undefined;
    var buf: [50][50]MInt = undefined;
    for (0..n) |i| {
        for (0..n) |j| {
            const aij = readInt(u1);
            a[i][j] = .fromRaw(aij);
        }
    }
        ...
        if (k % 2 != 0) {
            for (0..n) |i| {
                for (0..n) |j| {
                    var sum: MInt = .zero;
                    for (0..n) |m| {
                        sum = sum.add(mat[i][m].mul(a[m][j]));
                    }
                    buf[i][j] = sum;
                }
            }
            for (0..n) |i| @memcpy(mat[i][0..n], buf[i][0..n]);
        }
        for (0..n) |i| {
            for (0..n) |j| {
                var sum: MInt = .zero;
                for (0..n) |m| {
                    sum = sum.add(a[i][m].mul(a[m][j]));
                }
                buf[i][j] = sum;
            }
        }
        for (0..n) |i| @memcpy(a[i][0..n], buf[i][0..n]);
    }
    var ans: MInt = .zero;
    for (0..n) |i| {
        for (0..n) |j| ans = ans.add(mat[i][j]);
    }
    print("{d}\n", .{ans.value});
}
