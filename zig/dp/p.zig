...
var dp: [][2]MInt = undefined;
var vis: []bool = undefined;
fn dfs(u: u32) void {
    vis[u] = true;
    dp[u] = .{ .one, .one };
    for (G[u].items) |v| {
        if (vis[v]) continue;
        dfs(v);
        dp[u][0] = dp[u][0].mul(dp[v][0].add(dp[v][1]));
        dp[u][1] = dp[u][1].mul(dp[v][0]);
    }
}

pub fn solve() !void {
    const n = readInt(u32);
    G = try allocator.alloc(std.ArrayList(u32), n);
    for (0..n) |i| G[i] = .initBuffer(&.{});
    for (0..n - 1) |_| {
        const x = readInt(u32) - 1;
        const y = readInt(u32) - 1;
        try G[x].append(allocator, y);
        try G[y].append(allocator, x);
    }
    dp = try allocator.alloc([2]MInt, n);
    vis = try allocator.alloc(bool, n);
    @memset(vis, false);
    dfs(0);
    print("{d}\n", .{dp[0][0].add(dp[0][1]).value});
}
