...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var dp = try allocator.alloc(u32, n);
    var indeg = try allocator.alloc(u32, n);
    @memset(indeg, 0);
    @memset(dp, 0);
    var g = try allocator.alloc(std.ArrayList(u32), n);
    ...
    for (0..n) |i| {
        if (indeg[i] == 0) {
            stack.appendAssumeCapacity(@intCast(i));
        }
    }
    while (stack.pop()) |u| {
        for (g[u].items) |v| {
            ...
            indeg[v] -= 1;
            if (indeg[v] == 0) stack.appendAssumeCapacity(v);
        }
    }
    print("{d}\n", .{mem.max(u32, dp)});
}
