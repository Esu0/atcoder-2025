...
pub fn solve() !void {
    const n = readInt(u32);
    const dp = try allocator.alloc([][]f64, n + 1);
    for (0..n + 1) |i| {
        dp[i] = try allocator.alloc([]f64, n + 1);
        for (0..n + 1) |j| {
            dp[i][j] = try allocator.alloc(f64, n + 1);
        }
    }
    dp[0][0][0] = 0;
    var a3: u32 = 0;
    while (a3 <= n) : (a3 += 1) {
        ...
        while (a3 + a2 <= n) : (a2 += 1) {
            var a1: u32 = 0;
            while (a3 + a2 + a1 <= n) : (a1 += 1) {
                const s: f64 = @floatFromInt(a3 + a2 + a1);
                if (s == 0) continue;
                const p3 = @as(f64, @floatFromInt(a3)) / s;
                const p2 = @as(f64, @floatFromInt(a2)) / s;
                const p1 = @as(f64, @floatFromInt(a1)) / s;
                var val: f64 = @as(f64, @floatFromInt(n)) / s;
                if (a3 > 0) {
                    val += dp[a3-1][a2+1][a1] * p3;
                }
                if (a2 > 0) {
                    val += dp[a3][a2-1][a1+1] * p2;
                }
                if (a1 > 0) {
                    val += dp[a3][a2][a1-1] * p1;
                }
                dp[a3][a2][a1] = val;
            }
        }
