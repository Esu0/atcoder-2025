...
pub fn solve() !void {
    const n = readInt(u5);
    const dp = try allocator.alloc(MInt, @as(u32, 1)<<n);
    const ndp = try allocator.alloc(MInt, @as(u32, 1) << n);
    @memset(dp, .zero);
    dp[0] = .one;
    // var a: [21][21]bool = undefined;

    { var i: u5 = 0; while (i < n) : (i += 1) {
        var s: u32 = 0;
        var a: [21]bool = undefined;
        for (0..n) |j| a[j] = readInt(u1) == 1;
        @memset(ndp, .zero);
        while (s < @as(u32, 1) << n) : (s += 1) {
            if (dp[s].value == 0) continue;
            var j: u5 = 0;
            while (j < n) : (j += 1) {
                if ((s >> j) & 1 == 0 and a[j]) {
                    const idx = s | (@as(u32, 1) << j);
                    ndp[idx] = ndp[idx].add(dp[s]);
                }
            }
        }
        @memcpy(dp, ndp);
        // for (0..n) |j| a[i][j] = readInt(u1) == 1;
    }}
    print("{d}\n", .{dp[(@as(u32, 1) << n) - 1].value});
}
