...
pub fn solve() !void {
    const t = readInt(u32);
    for (0..t) |_| {
        const x1: i32 = readInt(u31);
        const y1: i32 = readInt(u31);
        const r1: i32 = readInt(u31);
        const x2: i32 = readInt(u31);
        const y2: i32 = readInt(u31);
        const r2: i32 = readInt(u31);
        const dx: i64 = x1 - x2;
        const dy: i64 = y1 - y2;
        const d2: u64 = @intCast(dx * dx + dy * dy);
        if (d2 < @as(i64, r1 - r2) * (r1 - r2) or d2 > @as(i64, r1 + r2) * (r1 + r2)) {
            print("No\n", .{});
        } else {
            print("Yes\n", .{});
        }
