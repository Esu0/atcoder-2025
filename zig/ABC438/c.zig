...
pub fn solve() !void {
    ...
    for (0..n) |_| {
        stack.appendAssumeCapacity(readInt(u32));
        const len = stack.items.len;
        if (len >= 4) {
            if (
                stack.items[len - 1] == stack.items[len - 2] and
                stack.items[len - 2] == stack.items[len - 3] and
                stack.items[len - 3] == stack.items[len - 4]
            ) {
                stack.shrinkRetainingCapacity(len - 4);
            }
        }
    }
    print("{d}\n", .{stack.items.len});
