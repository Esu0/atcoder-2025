...
pub fn solve() !void {
    const n1 = readInt(u32);
    const n2 = readInt(u32);
    const m = readInt(u32);
    var g: [3<<17]std.ArrayList(u32) = undefined;
    for (0..n1+n2) |i| g[i] = .initBuffer(&.{});
    for (0..m) |_| {
    ...
    var queue: FixedQueue(u32, 2<<17) = .{};
    var dist: [3<<17]u32 = @splat(math.maxInt(u32));
    dist[0] = 0;
    queue.push(0);
    while(queue.pop()) |u| {
        const d = dist[u] + 1;
        for (g[u].items) |to| {
            if (dist[to] != math.maxInt(u32)) continue;
            dist[to] = d;
            queue.push(to);
        }
    }
    ...
    const ans = mem.max(u32, dist[0..n1]) + mem.max(u32, dist[n1..n1+n2]) + 1;
    print("{d}\n", .{ans});
}
