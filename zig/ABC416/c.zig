...
var all: std.ArrayList([]u8) = undefined;
var s: [10][]u8 = undefined;
var arr: [5][]const u8 = undefined;
var N: u8 = undefined;
var K: u8 = undefined;

fn dfs(i: u8) void {
    if (i == K) {
        const t = mem.concat(allocator, u8, arr[0..K]) catch unreachable;
        all.appendAssumeCapacity(t);
        return;
    }
...
pub fn solve() !void {
    ...
    K = k;
    const x = readInt(u32);
    for (0..n) |i| s[i] = readString();
    all = .initBuffer(&buffer);
    dfs(0);
    mem.sort([]u8, all.items, {}, struct {fn lessThan(_: void, lhs: []u8, rhs: []u8) bool {
