...
pub fn solve() !void {
    ...
    for (0..t) |_| {
        ...
        var b: [3<<17]u32 = undefined;
        for (0..n) |i| a[i] = readInt(u32);
        for (0..n) |i| b[i] = readInt(u32);
        var sum: u64 = 0;
        for (0..n) |i| sum += a[i] + b[i];
        mem.sort(u32, a[0..n], {}, std.sort.asc(u32));
        mem.sort(u32, b[0..n], {}, std.sort.asc(u32));
        var ok: u32 = 0;
        var ng: u32 = n + 1;
        while (ng - ok > 1) {
            const mid = ok + (ng - ok) / 2;
            var f = true;
            for (0..mid) |i| {
                if (a[n - mid + i] + b[n - i - 1] < m) f = false;
            }
            if (f) ok = mid else ng = mid;
        }
        print("{d}\n", .{sum - @as(u64, ok) * m});
    }
}
