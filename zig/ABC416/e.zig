...
fn addEdge(mat: [][501]u64, u: u32, v: u32, w: u32) void {
    const n = mat.len;
    for (0..n) |i| {
        for (0..n) |j| {
...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var mat: [501][501]u64 = @splat(@splat(math.maxInt(u64)));
    for (0..n+1) |i| mat[i][i] = 0;
    for (0..m) |_| {
        const a = readInt(u32);
        const b = readInt(u32);
        const c = readInt(u32);
    ...
    for (0..q) |_| {
        ...
        if (ty == 1) {
            ...
            const y = readInt(u32);
            const t = readInt(u32);
            addEdge(mat[0..n+1], x, y, t);
            addEdge(mat[0..n+1], y, x, t);
        } else if (ty == 2) {
            const x = readInt(u32);
            addEdge(mat[0..n+1], 0, x, T);
            addEdge(mat[0..n+1], x, 0, 0);
        } else {
            assert(ty == 3);
            var ans: u64 = 0;
            // std.debug.print("{any}\n", .{mat[0][0..n+1]});
            for (1..n+1) |i| {
                for (1..n+1) |j| {
                    if (mat[i][j] != math.maxInt(u64)) ans += mat[i][j];
                }
                // std.debug.print("{any}\n", .{mat[i][0..n+1]});
            }
            print("{d}\n", .{ans});
