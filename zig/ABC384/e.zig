...
pub fn solve() !void {
    ...
    const w = readInt(u32);
    const x = readInt(u32);
    const p = readInt(u16) - 1;
    const q = readInt(u16) - 1;
    var s: [500][500]u64 = undefined;
    for (0..h) |i| {
        for (0..w) |j| {
            s[i][j] = readInt(u64);
        }
    }
    var vis: [500][500]bool = @splat(@splat(false));
    vis[p][q] = true;
    var sum = s[p][q];
    var queue: std.PriorityQueue(struct {u16, u16}, *[500][500]u64, struct { fn compare(ctx: *[500][500]u64, lhs: struct {u16, u16}, rhs: struct {u16, u16}) math.Order {
        return math.order(ctx[lhs[0]][lhs[1]], ctx[rhs[0]][rhs[1]]);
    }}.compare) = .init(allocator, &s);
    const dij = [5]u16{ 0, 1, 0, math.maxInt(u16), 0};
    if (p > 0) try queue.add(.{p - 1, q});
    if (p + 1 < h) try queue.add(.{p + 1, q});
    if (q > 0) try queue.add(.{p, q - 1});
    if (q + 1 < w) try queue.add(.{p, q + 1});
    while (queue.removeOrNull()) |ij| {
        const i, const j = ij;
        if (vis[i][j]) continue;
        if (math.divCeil(u64, sum, x) catch unreachable > s[i][j]) {
            sum += s[i][j];
            vis[i][j] = true;
            inline for (0..4) |k| {
                const ni = i +% dij[k];
                const nj = j +% dij[k + 1];
                if (ni < h and nj < w) try queue.add(.{ni, nj});
            }
        } else {
            break;
        }
