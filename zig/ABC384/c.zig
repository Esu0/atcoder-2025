...
pub fn solve() !void {
    ...
    var abcde: [5]u32 = undefined;
    for (0..5) |i| abcde[i] = readInt(u32);

    mem.sort([]u8, &names, &abcde, struct {fn lessThan(p: *[5]u32, lhs: []u8, rhs: []u8) bool {
        var s0: u32 = 0;
        for (lhs) |c| {
            s0 += p[c - 'A'];
        }
        var s1: u32 = 0;
        for (rhs) |c| s1 += p[c - 'A'];
        return (math.order(s1, s0).differ() orelse mem.order(u8, lhs, rhs)) == .lt;
    }}.lessThan);
    for (names) |name| {
