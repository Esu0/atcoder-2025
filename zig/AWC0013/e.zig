...
var g: []std.ArrayList(u32) = undefined;
var cap: [202][202]u1 = undefined;
var dist: []u8 = undefined;
var t: u32 = undefined;
var ctx: [202]u8 = undefined;

fn dfs(u: u32, flow: u1) u1 {
    if (u == t) return flow;
    var k = ctx[u];
    defer ctx[u] = k;
...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    g = try allocator.alloc(std.ArrayList(u32), n + m + 2);
    for (0..n+m+2) |i| g[i] = try .initCapacity(allocator, n + m + 1);
    const start = n + m;
    const end = n + m + 1;
    var i: u32 = 0;
    while (i < n) : (i += 1) {
        ...
        for (0..k) |_| {
            ...
            g[i].appendAssumeCapacity(c + n);
            g[c + n].appendAssumeCapacity(i);
            cap[i][c+n] = 1;
            cap[c+n][i] = 0;
        }
        g[start].appendAssumeCapacity(i);
        g[i].appendAssumeCapacity(start);
        cap[start][i] = 1;
        cap[i][start] = 0;
    }
    ...
    i = 0;
    while (i < m) : (i += 1) {
        g[n+i].appendAssumeCapacity(end);
        g[end].appendAssumeCapacity(n+i);
        cap[n+i][end] = 1;
        cap[end][n+1] = 0;
    }

    var queue: FixedQueue(u32, 41000) = .{};
    dist = try allocator.alloc(u8, n+m+2);
    t = end;
    var flow: u32 = 0;
    while (true) {
        queue.clear();
        queue.push(start);
        @memset(dist, math.maxInt(u8));
        dist[start] = 0;
        while (queue.pop()) |u| {
            const d = dist[u] + 1;
            for (g[u].items) |v| {
                if (cap[u][v] == 0) continue;
                if (dist[v] != math.maxInt(u8)) continue;
                dist[v] = d;
                queue.push(v);
            }
        }
        if (dist[t] == math.maxInt(u8)) break;
        @memset(ctx[0..n+m+2], 0);
        while (true) {
            const f = dfs(start, 1);
            if (f == 0) break;
            flow += f;
