...
pub fn solve() !void {
    const n = readInt(u32);
    const x0: i32 = readInt(u31);
    const y0: i32 = readInt(u31);
    var minx: i32 = x0 + y0;
    var maxx: i32 = x0 + y0;
    var miny: i32 = x0 - y0;
    var maxy: i32 = x0 - y0;
    for (1..n) |_| {
        ...
        const y: i32 = readInt(u31);
        const nx = x + y;
        const ny = x - y;
        if (nx < minx) minx = nx else if (maxx < nx) maxx = nx;
        if (ny < miny) miny = ny else if (maxy < ny) maxy = ny;
    }
