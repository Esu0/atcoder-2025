...
pub fn solve() !void {
    ...
    {var i: u32 = 0; while (i < n - 1) : (i += 1) {
        const a = readInt(u32);
        const b = readInt(u32);
        const x = readInt(u32) - 1;
        g[i] = try .initCapacity(allocator, 2);
        g[i].appendSliceAssumeCapacity(&.{ .{i + 1, a}, .{x, b}});
    }}
    g[n - 1] = .initBuffer(&.{});
