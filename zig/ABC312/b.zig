...
pub fn solve() !void {
    ...
    for (0..n-9+1) |i| {
        next: for (0..m-9+1) |j| {
            ...
            for (0..3) |a| {
                if (s[i+3][j+a] == '#') continue :next;
                if (s[i+a][j+3] == '#') continue :next;
            }
            if (s[i+3][j+3] == '#') continue;
            for (6..9) |a| {
                if (s[i+5][j+a] == '#') continue :next;
                if (s[i+a][j+5] == '#') continue :next;
            }
            if (s[i+5][j+5] == '#') continue;
            print("{d} {d}\n", .{i + 1, j + 1});
        }
    }
}
