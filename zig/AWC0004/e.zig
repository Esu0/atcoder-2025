...
const Context = struct {
    pub fn hash(_: Context, key: i64) u64 {
        const bytes: []const u8 = mem.asBytes(&key);
        var cur = mem.readInt(u32, @ptrCast(bytes[0..4]), .little) *% 0x27220a95;
        const k = mem.readInt(u32, @ptrCast(bytes[4..]), .little);
        cur = (math.rotl(u32, cur, 5) ^ k) *% 0x27220a95;
        return cur;
    }
    pub fn eql(_: Context, a: i64, b: i64) bool {
        return a == b;
    }
};
pub fn solve() !void {
    ...
    var prev: i64 = 0;
    var ans: u64 = 0;
    var set: std.HashMap(i64, u32, Context, 80) = .initContext(allocator, .{});
    // var set: std.AutoHashMap(i64, u32) = .init(allocator);
    try set.put(0, 1);
    for (0..n) |_| {
        prev += readInt(i32);
        ans += set.get(prev - k) orelse 0;
        (try set.getOrPutValue(prev, 0)).value_ptr.* += 1;
    }
    print("{d}\n", .{ans});
}
