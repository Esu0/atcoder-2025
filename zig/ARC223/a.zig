...
pub fn solve() !void {
    ...
    for (0..t) |_| {
        const n = readInt(u32);
        var cumsum: [61]u64 = undefined;
        cumsum[0] = 0;
        var W = readInt(u64);
        var wv: [60]struct {u64, u64} = undefined;
        for (0..n) |i| {
            wv[i] = .{ readInt(u64), readInt(u64) };
            cumsum[i + 1] = cumsum[i] + wv[i][1];
        }
        ...
        while (i > 0) {
            ...
            ans = @max(ans, V + cumsum[i]);
            W -= wv[i][0];
            V += wv[i][1];
        }
        // std.debug.print("w, v: {d} {d}\n", .{W, V});
        print("{d}\n", .{@max(ans, V)});
    }
}
