...
pub fn solve() !void {
    const n = readInt(u32);
    const k = readInt(u32);
    var queue0 = FixedQueue(struct {u32, i32}, 2<<17){};
    var queue1 = FixedQueue(struct {u32, i32}, 2<<17){};

    var i: u32 = 0;
    while (i < k - 1) : (i += 1) {
        const h = readInt(i32);
        while (true) {
            const x = queue0.get_back();
            if (x == null or x.?[1] > h) break;
            _ = queue0.pop_back();
        }
        queue0.push(.{i, h});
        while (true) {
            const x = queue1.get_back();
            if (x == null or x.?[1] < h) break;
            _ = queue1.pop_back();
        }
        queue1.push(.{i, h});
    ...
    while (i < n) : (i += 1) {
        ...
        const f0 = queue0.get().?;
        const f1 = queue1.get().?;
        ans = @max(ans, f0[1] - f1[1]);
        if (f0[0] == i + 1 - k) _ = queue0.pop();
        if (f1[0] == i + 1 - k) _ = queue1.pop();
    }
    print("{d}\n", .{ans});
}
...
fn FixedQueue(comptime T: type, comptime max_size: u32) type {
    return struct {
        ...
        pub fn get(self: *Self) ?T {
            if (self.wp == self.rp) return null;
            return self.buf[self.rp];
        }

        pub fn get_back(self: *Self) ?T {
            if (self.wp == self.rp) return null;
            return self.buf[self.wp - 1];
        }
    
        pub fn pop_back(self: *Self) ?T {
            if (self.wp == self.rp) return null;
            self.wp -= 1;
            return self.buf[self.wp];
        }
    };
}
