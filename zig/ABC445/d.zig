...
var hw: [2<<17]struct {u32, u32} = undefined;

fn lessThanH(_: void, lhs: u32, rhs: u32) bool {
    return hw[lhs][0] < hw[rhs][0];
}

fn lessThanW(_: void, lhs: u32, rhs: u32) bool {
    return hw[lhs][1] < hw[rhs][1];
}
...
pub fn solve() !void {
    ...
    var w = readInt(u32);
    const n = readInt(u32);
    for (0..n) |i| hw[i] = .{ readInt(u32), readInt(u32) };

    const used = try allocator.alloc(bool, n);
    @memset(used, false);
    const ph = try allocator.alloc(u32, n);
    const pw = try allocator.alloc(u32, n);
    {var i: u32 = 0; while (i < n) : (i += 1) {
        ph[i] = i;
        pw[i] = i;
    }}
    mem.sortUnstable(u32, ph, {}, lessThanH);
    mem.sortUnstable(u32, pw, {}, lessThanW);
    // std.debug.print("{any}\n", .{ph});
    const ans = try allocator.alloc(struct {u32, u32}, n);
    {
        var i: u32 = n;
        var j: u32 = n;
        while (i > 0 or j > 0) {
            if (i > 0) {
                const k = ph[i - 1];
                assert(used[k] == false);
                const hk, const wk = hw[k];
                if (hk == h) {
                    w -= wk;
                    ans[k] = .{ 1, w + 1 };
                    used[k] = true;
                    while (i > 0 and used[ph[i - 1]]) i -= 1;
                    while (j > 0 and used[pw[j - 1]]) j -= 1;
                }
            }
            if (j > 0) {
                const k = pw[j - 1];
