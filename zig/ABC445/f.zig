...
pub fn solve() !void {
    const n = readInt(u32);
    var k = readInt(u32);
    var c: [100][100]u64 = undefined;
    for (0..n) |i| {
        for (0..n) |j| c[i][j] = readInt(u32);
    }
    var buf: [100][100]u64 = undefined;
    var ans: [100][100]u64 = @splat(@splat(math.maxInt(u64)));
    for (0..n) |i| ans[i][i] = 0;
    while (k > 0) : (k >>= 1) {
        if (k & 1 != 0) {
            for (0..n) |i| @memset(buf[i][0..n], math.maxInt(u64));
            for (0..n) |i| {
                for (0..n) |j| {
                    for (0..n) |m| {
                        buf[i][m] = @min(buf[i][m], c[i][j] +| ans[j][m]);
                    }
                }
            }
            for (0..n) |i| @memcpy(ans[i][0..n], buf[i][0..n]);
        }
        for (0..n) |i| @memset(buf[i][0..n], math.maxInt(u64));
        for (0..n) |i| {
            for (0..n) |j| {
                for (0..n) |m| {
                    buf[i][m] = @min(buf[i][m], c[i][j] +| c[j][m]);
                }
            }
        }
        for (0..n) |i| @memcpy(c[i][0..n], buf[i][0..n]);
    }
    for (0..n) |i| print("{d}\n", .{ans[i][i]});
}
