...
// sに一文字挿入
fn check(s: []const u8, t: []const u8) bool {
    assert(s.len + 1 == t.len);
    var max: u32 = 0;
    while (max < s.len and s[max] == t[max]) max += 1;
    var min: u32 = @intCast(s.len);
    while (min > 0 and s[min - 1] == t[min]) min -= 1;
    return min <= max;
}
...
pub fn solve() !void {
    ...
    if (s.len == t.len) {
        var diff: u32 = 0;
        for (s, t) |si, ti| diff += @intFromBool(si != ti);
        if (diff <= 1) ans = true;
    ...
    try stdout.writeAll(if (ans) "Yes\n" else "No\n");
}
