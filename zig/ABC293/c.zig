...
var H: u32 = undefined;
var W: u32 = undefined;
var A: [10][10]u32 = undefined;
var BUF: [20]u32 = undefined;
var stack: std.ArrayList(u32) = undefined;
var ans: u32 = 0;

fn dfs(i: u8, j: u8) void {
    stack.appendAssumeCapacity(A[i][j]);
    defer assert(stack.pop() != null);
    if (i == H - 1 and j == W - 1) {
        var arr: [20]u32 = undefined;
        assert(stack.items.len == H + W - 1);
        @memcpy(arr[0..stack.items.len], stack.items);
        mem.sortUnstable(u32, arr[0..stack.items.len], {}, std.sort.asc(u32));
        for (0..stack.items.len - 1) |k| {
            if (arr[k] == arr[k + 1]) return;
        }
        ans += 1;
        return;
    }
