...
pub fn solve() !void {
    const n = readInt(u32);
    var cnt: [1<<17]u8 = @splat(6);
    var a: [6<<17]struct {u32, u32} = undefined;
    for (0..n) |i| {
        inline for (0..6) |j| a[i*6+j] = .{readInt(u32), @intCast(i)};
    }
    mem.sortUnstable(struct {u32, u32}, a[0..n*6], {}, struct {fn lessThan(_: void, lhs: struct {u32, u32}, rhs: struct {u32, u32}) bool {
        return lhs[0] > rhs[0];
    }}.lessThan);

    var invs: [7]MInt = undefined;
    for (1..7) |i| invs[i] = MInt.init(i).inv();

    var p = MInt.one;
    var ans = MInt.zero;
    for (a[0..n*6]) |ai| {
        const v, const i = ai;
        const inv = invs[cnt[i]];
        cnt[i] -= 1;
        ans = ans.add(MInt.init(v).mul(p).mul(inv));
        p = p.mul(MInt.one.sub(inv));
        // ans += v * p * 1 / cnt[i]
        if (p.value == 0) break;
    }
    print("{d}\n", .{ans.value});
