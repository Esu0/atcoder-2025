...
pub fn solve() !void {
    const n = readInt(u32);
    const cntlr = readInt(u32);
    var dp0: [301][301][300]u16 = undefined;

    var dp: [301][301][300]MInt = undefined;
    var lr: [300][301]bool = @splat(@splat(false));
    for (0..cntlr) |_| {
        const l = readInt(u32) - 1;
        const r = readInt(u32);
        lr[l][r] = true;
    }
    for (0..n) |l| {
        dp0[1][l][l] = if (lr[l][l+1]) @intCast(l) else math.maxInt(u16);
    ...
    while (d <= n) : (d += 1) {
        ...
        while (l + d <= n) : (l += 1) {
            if (lr[l][l+d]) {
                @memset(dp0[d][l][l..l+d], l);
            } else {
                @memcpy(dp0[d][l][l..l+d-1], dp0[d-1][l][l..l+d-1]);
                dp0[d][l][l+d-1] = math.maxInt(u16);
                for (l+1..l+d+1) |j| {
                    dp0[d][l][j] = @min(dp0[d][l][j], dp0[d-1][l+1][j]);
                }
            }
            // std.debug.print("[{d}, {d}): {any}\n", .{l, l+d, dp0[d][l][l..l+d]});
            // for (l..l+d) |i| {
            //     assert(dp0[d][l][i] == math.maxInt(u16) or l <= dp0[d][l][i] and dp0[d][l][i] <= i);
            // }
        }
    }}
    for (0..n+1) |l| dp[0][l][l] = .one;
    for (0..n) |l| dp[1][l][l] = .one;
    {var d: u16 = 2;
    while (d <= n) : (d += 1) {
        var l: u16 = 0;
        while (l + d <= n) : (l += 1) {
            ...
            while (m > l + 1) {
                m -= 1;
                defer dp[d][l][m] = sum;
                // if (dp0[d][l][m] == math.maxInt(u16)) continue;
                if (dp0[d][l][m] >= m) continue;
                // if (d == 3 and l == 1) {
                //     std.debug.print("{d} {d} {d} {d} {d}\n", .{m - l, dp0[d][l][m], l + d - m - 1, m + 1, dp[m - l][l][dp0[d][l][m]].value});
                // }
                sum = sum.add(dp[m - l][l][dp0[d][l][m]].mul(dp[l+d-m-1][m+1][m+1]));
            }
            sum = sum.add(dp[d - 1][l + 1][l + 1]);
            dp[d][l][l] = sum;
            // std.debug.print("[{d}, {d}): {any}\n", .{l, l + d, dp[d][l][l..l+d]});
        }
    }}
