...
pub fn solve() !void {
    const n = readInt(u32);
    const x = readInt(u32);
    var apbq: [100]struct {u32, u32, u32, u32} = undefined;
    var dp: [100][10000]u64 = undefined;
    for (0..n) |i| {
        const a = readInt(u32);
        const p = readInt(u32);
        const b = readInt(u32);
        const q = readInt(u32);
        apbq[i] = .{ a, p, b, q };
        @memset(&dp[i], math.maxInt(u64));
        dp[i][0] = 0;
        for (1..a * b) |j| {
            var c: u64 = math.maxInt(u64);
            if (j >= a) c = dp[i][j - a] + p;
            if (j >= b) c = @min(c, dp[i][j - b] + q);
            dp[i][j] = c;
        }
    }
