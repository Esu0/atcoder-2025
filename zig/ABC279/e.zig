...
pub fn solve() !void {
    ...
    for (0..m) |i| a[i] = readInt(u32);
    var p1: u32 = 1;
    var q: [2<<17]u32 = undefined;
    {var i: u32 = 1; while (i <= n) : (i += 1) q[i] = i;}
    {var i: u32 = m; while (i > 0) {
        i -= 1;
        mem.swap(u32, &q[a[i]], &q[a[i]+1]);
    }}
    var i: u32 = 0;
    while (i < m) : (i += 1) {
        mem.swap(u32, &q[a[i]], &q[a[i]+1]);
        print("{d}\n", .{q[p1]});
        if (p1 == a[i]) p1 += 1 else if (p1 == a[i] + 1) p1 -= 1;
    }
}
