...
fn calc(a: f64, b: f64, t: u64) f64 {
    return a / math.sqrt(@as(f64, @floatFromInt(t + 1))) + b * @as(f64, @floatFromInt(t));
}
...
pub fn solve() !void {
    ...
    const ft = blk: {
        const tmp = fa / (fb * 2);
        break :blk math.cbrt(tmp * tmp) - 1;
    };
    if (ft <= 0) {
        print("{d}\n", .{a});
        return;
    }
    const t: u64 = @intFromFloat(ft);
    var ans = calc(fa, fb, t);
    if (t > 0) ans = @min(ans, calc(fa, fb, t - 1));
    ans = @min(ans, calc(fa, fb, t + 1));
    print("{d:.15}\n", .{ans});
    // print("{d:.15}\n", .{t});
}
