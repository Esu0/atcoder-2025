...
pub fn solve() !void {
    const h = readInt(u32);
    const w = readInt(u32);
    const s = try allocator.alloc([]u8, w);
    const t = try allocator.alloc([]u8, w);
    for (0..w) |i| {
        s[i] = try allocator.alloc(u8, h);
        t[i] = try allocator.alloc(u8, h);
    }
    for (0..h) |i| {
        const row = readString();
        for (0.., row) |j, c| {
            s[j][i] = c;
        }
    }
    for (0..h) |i| {
        const row = readString();
        for (0.., row) |j, c| t[j][i] = c;
    }
    mem.sortUnstable([]u8, s, {}, stringLessThan);
    mem.sortUnstable([]u8, t, {}, stringLessThan);
    for (s, t) |si, ti| {
        if (!mem.eql(u8, si, ti)) {
