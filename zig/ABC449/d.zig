...
pub fn solve() !void {
    ...
    var ans: u64 = 0;
    while (r >= l and u >= d) {
        var arr: [4]struct {u8, u32} = .{
            .{0, @abs(l)},
            .{1, @abs(r)},
            .{2, @abs(d)},
            .{3, @abs(u)},
        };
        mem.sortUnstable(struct {u8, u32}, &arr, {}, struct {fn lessThan(_: void, lhs: struct{u8, u32}, rhs: struct{u8, u32}) bool {
            return lhs[1] > rhs[1];
        }}.lessThan);
        const x, const y = arr[0];
        if (y % 2 == 0) {
            if (x == 0 or x == 1) {
                ans += @intCast(u - d + 1);
            } else {
        ...
        if (x == 0) l += 1 else if (x == 1) r -= 1 else if (x == 2) d += 1 else u -= 1;
    }
    print("{d}\n", .{ans});
}
