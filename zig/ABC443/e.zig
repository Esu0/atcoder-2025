...
pub fn solve() !void {
    ...
    for (0..t) |_| {
        const n = readInt(u32);
        const c = readInt(u32);
        var s: [3000][]u8 = undefined;
        for (0..n) |i| {
            s[i] = readString();
            assert(s[i].len == n);
        }
        var wall = try allocator.alloc(?u32, n);
        @memset(wall, null);
        for (0..n) |i| {
            for (0..n) |j| {
                if (s[i][j] == '#') wall[j] = @as(u32, @intCast(i));
            }
        }
        var dp: [3000][3000]bool = undefined;
        for (0..n) |i| @memset(dp[i][0..n], false);
        dp[n-1][c-1] = true;
        var i: u32 = n - 1;
        while (i > 0) {
            i -= 1;
            for (0..n) |j| {
                if (dp[i][j]) continue;
                var f = dp[i + 1][j];
                if (j > 0) f = f or dp[i+1][j-1];
                if (j < n - 1) f = f or dp[i+1][j+1];
                if (!f) continue;
                if (s[i][j] == '.') {
                    dp[i][j] = true;
                } else if (i == wall[j]) {
                    dp[i][j] = true;
                    for (0..i) |k| dp[k][j] = true;
                }
            }
        }
        for (0..n) |j| try stdout.writeByte("01"[@intFromBool(dp[0][j])]);
