...
//     var prev: u64 = 9;
//     while (cur > 0) : (cur /= 10) {
//         if (cur % 10 > prev) return false;
//         prev = cur % 10;
//     }
//     return true;
// }
// fn solve_naive(n: u32) void {
//     var cnt: u32 = 0;
//     for (1..100000) |i| {
//         if (cnt >= 10) return;
//         const d = n * i;
...
pub fn solve() !void {
    const n = readInt(u32);
    // solve_naive(n);
    var dp: [10][1<<22]?u32 = @splat(@splat(null));
    var queue = FixedQueue(u32, 10<<22){};
    queue.push(0);
    var v: ?u32 = null;
    while (queue.pop()) |u| {
        const r = u & ((1<<22)-1);
        ...
        for (d..10) |nd| {
            const nr = (r * 10 + nd) % n;
            const nu = (nd << 22) | nr;
            if (dp[nd][nr] != null) continue;
            dp[nd][nr] = u;
            queue.push(@intCast(nu));
        }
    }
    if (v == null) {
    ...
    var cur = v.?;
    var buf: [3<<20]u8 = undefined;
    var ans: std.ArrayList(u8) = .initBuffer(&buf);
    while (cur != 0) {
        const d = cur >> 22;
        ans.appendAssumeCapacity(@intCast(d));
        const r = cur & ((1<<22)-1);
        cur = dp[d][r].?;
    }
    while(ans.pop()) |d| {
        try stdout.writeByte(d+'0');
    }
