...
pub fn solve() !void {
    var a: [3][6]u8 = undefined;
    for (0..3) |i| {
        for (0..6) |j| a[i][j] = readInt(u8);
    }
    var ans: u32 = 0;
    for (0..6) |i| {
        for (0..6) |j| {
            for (0..6) |k| {
                var buf: [3]u8 = .{ a[0][i], a[1][j], a[2][k] };
                mem.sortUnstable(u8, &buf, {}, std.sort.asc(u8));
                if (buf[0] == 4 and buf[1] == 5 and buf[2] == 6) ans += 1;
            }
        }
    }
    print("{d:.15}\n", .{@as(f64, @floatFromInt(ans)) / 216});
}
