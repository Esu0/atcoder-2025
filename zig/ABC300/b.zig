...
pub fn solve() !void {
    const h = readInt(u32);
    const w = readInt(u32);
    var a: [30][]u8 = undefined;
    var b: [30][]u8 = undefined;
    for (0..h) |i| a[i] = readString();
    for (0..h) |i| b[i] = readString();
    for (0..h) |_| {
        for (0..w) |_| {
            for (0..h) |i| mem.rotate(u8, a[i], 1);
            var ok = true;
            for (0..h) |i| ok &= mem.eql(u8, a[i], b[i]);
            if (ok) {
                try stdout.writeAll("Yes\n");
                return;
            }
        }
        mem.rotate([]u8, a[0..h], 1);
    }
