...
fn getSum(r: u32, c: u32, pat: *const [3][5]u8) u64 {
    const dr = r / 2;
    const dc = c / 4;
    const rr = r % 2;
    const cr = c % 4;
    return @as(u64, dr) * dc * pat[2][4] + @as(u64, dr) * pat[2][cr] + @as(u64, dc) * pat[rr][4] + pat[rr][cr];
}
pub fn solve() !void {
    const a = readInt(i32);
    const b = readInt(i32);
    ...
    var patsum: [3][5]u8 = @splat(@splat(0));
    for (1..3) |i| {
        for (1..5) |j| patsum[i][j] = patsum[i][j - 1] + pat[i - 1][j - 1];
    }
    for (1..3) |i| {
        for (1..5) |j| patsum[i][j] += patsum[i - 1][j];
    }
    const OFF = 1_000_000_000;
    const r0: u32 = @intCast(b + OFF);
    const c0: u32 = @intCast(a + OFF);
    const r1: u32 = @intCast(d + OFF);
    const c1: u32 = @intCast(c + OFF);
    // std.debug.print("{d}, {d}: {d}\n", .{r0, c0, getSum(r0, c0, &patsum)});
    // std.debug.print("{d}, {d}: {d}\n", .{r1, c1, getSum(r1, c1, &patsum)});
    print("{d}\n", .{getSum(r1, c1, &patsum) + getSum(r0, c0, &patsum) - getSum(r1, c0, &patsum) - getSum(r0, c1, &patsum)});
}
