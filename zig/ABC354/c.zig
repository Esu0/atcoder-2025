...
pub fn solve() !void {
    const n = readInt(u32);
    var acis: [2<<17]struct {u32, u32, u32} = undefined;
    for (0..n) |i| acis[i] = .{ readInt(u32), readInt(u32), @intCast(i) };

    mem.sortUnstable(struct {u32, u32, u32}, acis[0..n], {}, struct {fn lessThan(_: void, lhs: struct {u32, u32, u32}, rhs: struct {u32, u32, u32}) bool {
        return lhs[0] > rhs[0];
    }}.lessThan);
    var minc: u32 = math.maxInt(u32);

    var ans: std.ArrayList(u32) = try .initCapacity(allocator, n);
    for (acis[0..n]) |aci| {
        _, const c, const i = aci;
        if (c < minc) {
            ans.appendAssumeCapacity(i);
            minc = c;
        }
    }
    mem.sortUnstable(u32, ans.items, {}, std.sort.asc(u32));
    print("{d}\n", .{ans.items.len});
    for (ans.items) |ai| print("{d} ", .{ai + 1});
