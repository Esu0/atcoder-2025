...
var N: u5 = undefined;
var ab: [18]struct {u32, u32} = undefined;
var dp: [1<<18]?bool = @splat(null);
fn dfs(state: u32) bool {
    if (dp[state]) |ans| return ans;
    ...
    { var i: u5 = 0; while (i < N) : (i += 1) {
        ...
        while (j < N) : (j += 1) {
            ...
            if (ab[i][0] == ab[j][0] or ab[i][1] == ab[j][1]) {
                const mask = (@as(u32, 1) << i) | (@as(u32, 1) << j);
                if (dfs(state & ~mask) == false) {
                    // std.debug.print("{b} -> {b}\n", .{state, state & ~mask});
                    dp[state] = true;
                    return true;
                }
...
pub fn solve() !void {
    ...
    const ans = dfs((@as(u32, 1) << N) - 1);
    try stdout.writeAll(if (ans) "Takahashi\n" else "Aoki\n");
}
