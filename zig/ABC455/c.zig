...
pub fn solve() !void {
    ...
    for (0..n) |_| {
        const a = readInt(u32);
        (try set.getOrPutValue(a, 0)).value_ptr.* += 1;
        sum += a;
    }
    var a: std.ArrayList(u64) = try .initCapacity(allocator, n);
    var iter = set.iterator();
    while (iter.next()) |kv| {
        a.appendAssumeCapacity(@as(u64, kv.key_ptr.*) * kv.value_ptr.*);
    }
    mem.sortUnstable(u64, a.items, {}, std.sort.desc(u64));
    for (0..@min(k, a.items.len)) |i| {
        sum -= a.items[i];
    }
    print("{d}\n", .{sum});
}
