...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var cards: [3<<17]Node = undefined;
    for (0..n) |i| cards[i] = .{ .up = null, .down = null };
    var m: [3<<17]*Node = undefined;
    for (0..n) |i| {
        const node = try allocator.create(Node);
        node.* = .{ .up = &cards[i], .down = null };
        cards[i].down = node;
        m[i] = node;
    }
    ...
    for (0..q) |_| {
        ...
        cards[c].down.?.up = null;
        assert(cards[p].up == null);
        cards[p].up = &cards[c];
        cards[c].down = &cards[p];
    }
