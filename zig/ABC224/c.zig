...
fn f(p0: struct {i32, i32}, p1: struct {i32, i32}, p2: struct {i32, i32}) bool {
    const x0 = p1[0] - p0[0];
    const y0 = p1[1] - p0[1];
    const x1 = p2[0] - p0[0];
    const y1 = p2[1] - p0[1];
    return @as(i64, x0) * y1 - @as(i64, x1) * y0 != 0;
}
pub fn solve() !void {
    const n = readInt(u32);
    var xy: [300]struct {i32, i32} = undefined;
    for (0..n) |i| xy[i] = .{ readInt(i32), readInt(i32) };
    var ans: u32 = 0;
    for (0..n) |i| {
        for (i + 1..n) |j| {
            for (j + 1..n) |k| {
                ans += @intFromBool(f(xy[i], xy[j], xy[k]));
            }
        }
    }
    print("{d}\n", .{ans});
