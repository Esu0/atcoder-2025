...
pub fn solve() !void {
    const n = readInt(u32);
    var aab: [2<<17]struct {i32, i32} = @splat(.{0, 0});
    // var a: [2<<17]i32 = @splat(0);
    // var b: [2<<17]i32 = @splat(0);
    // var ab: [2<<17]i32 = @splat(0);
    for (0..n) |_| {
        const l = readInt(u32);
        const r = readInt(u32);
        const nl = n - r;
    ...
    var c: u32 = 1;
    while (c < n) : (c += 1) {
        const a, const ab = aab[c];
        const b = aab[n - c][0];
        if (a + b + ab == n and a <= c and b <= n - c) {
            const x: u32 = @intCast(ab);
            const y = c - @as(u32, @intCast(a));
            ans = ans.add(C.combi(x, y));
        }
    }
    print("{d}\n", .{ans.value});
}
