...
var H: u32 = undefined;
var W: u32 = undefined;
var ans: std.ArrayList(u8) = undefined;
var S: [1000][]u8 = undefined;
var vis: [1000][1000][4]bool = @splat(@splat(@splat(false)));
var start: struct {u32, u32} = undefined;
var goal: struct {u32, u32} = undefined;
const dij: [4]struct {u32, u32} = .{
    .{0, 1},
    .{0, math.maxInt(u32)},
    .{1, 0},
    .{math.maxInt(u32), 0},
...
fn dfs(i: u32, j: u32, d: u2) bool {
    if (goal[0] == i and goal[1] == j) {
        return true;
    }
    vis[i][j][d] = true;
    comptime var nd = 0;
    inline while (nd < 4) : (nd += 1) {
        const di, const dj = dij[nd];
        const ni = i +% di;
        const nj = j +% dj;
        if (ni < H and nj < W) {
            const c = S[i][j];
            if (!vis[ni][nj][nd] and !(c == '#' or c == 'o' and nd != d or c == 'x' and nd == d)) {
                if (dfs(ni, nj, nd)) {
                    ans.appendAssumeCapacity(dir[nd]);
                    return true;
                }
            }
        }
...
pub fn solve() !void {
    ...
    for (0..H) |i| {
        ...
        for (0..W) |j| {
            if (S[i][j] == 'S') {
                start = .{ @intCast(i), @intCast(j) };
            }
            if (S[i][j] == 'G') {
                goal = .{ @intCast(i), @intCast(j) };
            }
        }
    }
    ans = try .initCapacity(allocator, 5_000_000);
    if (dfs(start[0], start[1], 0)) {
        mem.reverse(u8, ans.items);
        print("Yes\n{s}\n", .{ans.items});
    } else {
        print("No\n", .{});
    }
