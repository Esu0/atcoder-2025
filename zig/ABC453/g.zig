...
fn update(root: ?*Node, dep: u5, idx: u32, value: u64) *Node {
    ...
    const dir = idx >> (dep - 1);
    if (root) |r| {
        new_node.children = r.children;
    } else {
        new_node.children = @splat(null);
    }

    new_node.children[dir] = update(new_node.children[dir], dep - 1, idx & ~(@as(u32, 1) << (dep - 1)), value);
    var nv: u64 = 0;
    inline for (0..2) |d| {
        if (new_node.children[d]) |ch| nv += ch.value;
    }
    new_node.value = nv;
    return new_node;
}

fn rangeSum(root: ?*Node, a: u32, b: u32, l: u32, r: u32) u64 {
    if (root == null) return 0;
    if (b <= l or r <= a) return 0;
    if (l <= a and b <= r) return root.?.value;
    const children = root.?.children;
    const mid = (a + b) / 2;
    return rangeSum(children[0], a, mid, l, r) + rangeSum(children[1], mid, b, l, r);
}
...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const q = readInt(u32);
    var trees: [2<<17]?*Node = undefined;
    const LOG = 18;
    @memset(trees[0..m], null);
    for (0..q) |_| {
        const t = readChar() - '0';
        if (t == 1) {
            ...
            const y = readInt(u32) - 1;
            const z = readInt(u32);
            trees[x] = update(trees[x], LOG, y, z);
        } else {
            assert(t == 3);
            const x = readInt(u32) - 1;
            const l = readInt(u32) - 1;
            const r = readInt(u32);
            print("{d}\n", .{rangeSum(trees[x], 0, 1 << LOG, l, r)});
        }
    }
}
