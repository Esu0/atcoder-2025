...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const k = readInt(u32);
    const g = try allocator.alloc(std.ArrayList(u32), n);
    for (0..n) |i| g[i] = .initBuffer(&.{});
    for (0..m) |_| {
        const a = readInt(u32) - 1;
        const b = readInt(u32) - 1;
        try g[a].append(allocator, b);
        try g[b].append(allocator, a);
    }
    const vs = try allocator.alloc(std.ArrayList(u32), n);
    for (0..n) |i| vs[i] = .initBuffer(&.{});
    for (0..k) |_| {
        const p = readInt(u32) - 1;
        const h = readInt(u32);
        try vs[n - h].append(allocator, p);
    }

    const dist = try allocator.alloc(u32, n);
    @memset(dist, math.maxInt(u32));
    var stack0: std.ArrayList(u32) = try .initCapacity(allocator, n);
    var stack1: std.ArrayList(u32) = try .initCapacity(allocator, n);
    var d: u32 = 0;
    while (d < n) : (d += 1) {
        for (vs[d].items) |u| {
            if (dist[u] == math.maxInt(u32)) {
                stack0.appendAssumeCapacity(u);
                dist[u] = d;
            }
        }
        while (stack0.pop()) |u| {
            for (g[u].items) |v| {
                if (dist[v] == math.maxInt(u32)) {
                    dist[v] = d + 1;
                    stack1.appendAssumeCapacity(v);
                }
            }
        }
        mem.swap(@TypeOf(stack0), &stack0, &stack1);
    }

    var i: u32 = 0;
    var ans: std.ArrayList(u32) = try .initCapacity(allocator, n);
    while (i < n) : (i += 1) {
        if (dist[i] != math.maxInt(u32)) ans.appendAssumeCapacity(i + 1);
    }
    print("{d}\n", .{ans.items.len});
    for (ans.items) |v| print("{d} ", .{v});
