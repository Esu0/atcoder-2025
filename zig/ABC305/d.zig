...
fn compare(x: u32, y: u32) math.Order {
...
pub fn solve() !void {
    ...
    for (0..q) |_| {
        ...
        {
            const k = std.sort.upperBound(u32, a[0..n], r, compare);
            ans = b[k - 1];
            if ((k - 1) % 2 == 1) ans += r - a[k - 1];
        }
        {
            const k = std.sort.upperBound(u32, a[0..n], l, compare);
            var t = b[k - 1];
            if ((k - 1) % 2 == 1) t += l - a[k - 1];
            ans -= t;
        }
        print("{d}\n", .{ans});
    }
