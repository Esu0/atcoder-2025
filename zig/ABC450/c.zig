...
pub fn solve() !void {
    ...
    var stack: std.ArrayList(struct {u32, u32}) = try .initCapacity(allocator, h * w);
    var vis: [1000][1000]bool = @splat(@splat(false));
    for (0..h) |i| {
        for (0..w) |j| {
            if ((i == 0 or i == h - 1 or j == 0 or j == w - 1) and s[i][j] == '.') {
                stack.appendAssumeCapacity(.{ @intCast(i), @intCast(j) });
                vis[i][j] = true;
            }
        }
    }
    ...
    while (stack.pop()) |u| {
        ...
        if (i > 0) {
            const ni = i - 1;
            const nj = j;
            if (!vis[ni][nj] and s[ni][nj] == '.') {
                vis[ni][nj] = true;
                stack.appendAssumeCapacity(.{ni, nj});
            }
        }
        if (i + 1 < h) {
            const ni = i + 1;
    ...
    for (0..h) |a| {
        for (0..w) |b| {
            if (vis[a][b] or s[a][b] != '.') continue;
            ans += 1;
            stack.appendAssumeCapacity(.{ @intCast(a), @intCast(b) });
            while (stack.pop()) |u| {
                const i, const j = u;
                if (i > 0) {
                    const ni = i - 1;
