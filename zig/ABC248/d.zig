...
pub fn solve() !void {
    const n = readInt(u32);
    var idcs: [2<<17]std.ArrayList(u32) = undefined;
    for (1..n+1) |i| idcs[i] = .initBuffer(&.{});
    {var i: u32 = 0; while (i < n) : (i += 1) {
        const a = readInt(u32);
        try idcs[a].append(allocator, i);
    }}
    const q = readInt(u32);
    for (0..q) |_| {
        ...
        const r = readInt(u32);
        const x = readInt(u32);
        print("{d}\n", .{std.sort.lowerBound(u32, idcs[x].items, r, compare) - std.sort.lowerBound(u32, idcs[x].items, l, compare)});
    }
}
