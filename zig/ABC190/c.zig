...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var ab: [100]struct {u8, u8} = undefined;
    for (0..m) |i| ab[i] = .{ readInt(u8) - 1, readInt(u8) - 1 };
    const k = readInt(u5);
    var cd: [16]struct {u8, u8} = undefined;
    for (0..k) |i| cd[i] = .{ readInt(u8) - 1, readInt(u8) - 1 };
    var ans: u32 = 0;
    for (0..@as(u32, 1) << k) |s| {
        var set: [100]bool = undefined;
        @memset(set[0..n], false);
        var i: u5 = 0;
        while (i < k) : (i += 1) {
            const x = if ((s >> i) & 1 == 0) cd[i][0] else cd[i][1];
            set[x] = true;
        }
        var cnt: u32 = 0;
        for (0..m) |j| {
            if (set[ab[j][0]] and set[ab[j][1]]) cnt += 1;
        }
        ans = @max(ans, cnt);
    }
    print("{d}\n", .{ans});
