...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var s: [2<<17]i64 = undefined;
    for (0..n) |i| s[i] = readInt(u32);
    const fw = FenwickTree(i64, struct {fn add(lhs: i64, rhs: i64) i64 {
        return lhs + rhs;
    }}.add).init(s[0..n]);
    for (0..q) |_| {
        const t = readInt(u8);
        if (t == 1) {
            ...
            const r = readInt(u32);
            const ans = fw.prefixSum(r).? - (fw.prefixSum(l) orelse 0);
            print("{d}\n", .{ans});
        } else {
            assert(t == 2);
            const x = readInt(u32);
            const v = readInt(u32);
            const cur = fw.prefixSum(x).? - (fw.prefixSum(x - 1) orelse 0);
            fw.add(x - 1, @as(i64, v) - cur);
        }
    }
