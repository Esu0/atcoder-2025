...
pub fn solve() !void {
    var isp: [2_000_001]bool = @splat(true);
    isp[0] = false;
    isp[1] = false;
    for (2..isp.len) |p| {
        if (!isp[p]) continue;
        var j = p * 2;
        while (j < isp.len) : (j += p) isp[j] = false;
    }
    var primes: std.ArrayList(u32) = .initBuffer(&.{});
    {
        var i: u32 = 2;
        while (i < isp.len) : (i += 1) {
            if (isp[i]) try primes.append(allocator, i);
        }
    }
    ...
    var ans: u32 = 0;
    for (primes.items) |p| {
        if (math.pow(u64, p, 8) <= n) ans += 1 else break;
    }
    for (0..primes.items.len) |i| {
        const p = primes.items[i];
        const k = std.sort.partitionPoint(u32, primes.items[i + 1..], .{p, n}, struct { fn predicate(pp: struct {u32, u64}, x: u32) bool {
            const t = @as(u64, pp[0]) * x;
            return t *| t <= pp[1];
        }}.predicate);
        ans += @intCast(k);
    }
