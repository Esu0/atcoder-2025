...
pub fn solve() !void {
    const n = readInt(u32);
    var x: [500]u32 = undefined;
    for (0..n*5) |i| x[i] = readInt(u32);
    mem.sortUnstable(u32, x[0..n*5], {}, std.sort.asc(u32));
    var sum: u32 = 0;
    for (n..n*4) |i| {
        sum += x[i];
    }
    const ans = @as(f64, @floatFromInt(sum)) / @as(f64, @floatFromInt(n*3));
    print("{d:.15}\n", .{ans});
}
