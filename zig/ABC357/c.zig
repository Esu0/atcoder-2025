...
var ans: [729][729]u8 = @splat(@splat('.'));

fn dfs(i: u32, j: u32, lv: u32, width: u32) void {
    if (lv == 0) {
        ans[i][j] = '#';
        return;
    }
    ...
    dfs(i, j, lv - 1, nw);
    dfs(i, j + nw, lv - 1, nw);
    dfs(i, j + nw + nw, lv - 1, nw);
    dfs(i + nw, j, lv - 1, nw);
    dfs(i + nw, j + nw + nw, lv - 1, nw);
    dfs(i + nw + nw, j, lv - 1, nw);
    dfs(i + nw + nw, j + nw, lv - 1, nw);
    dfs(i + nw + nw, j + nw + nw, lv - 1, nw);
}
pub fn solve() !void {
    const n = readInt(u32);
    const w = try math.powi(u32, 3, n);
    dfs(0, 0, n, w);
    for (0..w) |i| print("{s}\n", .{ans[i][0..w]});
}
