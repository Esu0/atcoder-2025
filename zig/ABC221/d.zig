...
pub fn solve() !void {
    const n = readInt(u32);
    var ev: [4<<17]struct {u1, u32} = undefined;
    for (0..n) |i| {
        const a = readInt(u32);
        const b = readInt(u32);
        ev[i*2] = .{ 0, a };
    ...
    mem.sortUnstable(struct {u1, u32}, ev[0..2*n], {}, struct {fn lessThan(_: void, lhs: struct {u1, u32}, rhs: struct {u1, u32}) bool {
        return lhs[1] < rhs[1];
    }}.lessThan);
    ev[2*n] = .{ 0, 2_000_000_001 };
    var ans: [2<<17]u32 = @splat(0);
    var k: u32 = 0;
    var l: u32 = 0;
    for (0..n+n+1) |i| {
        const t, const v = ev[i];
        ans[k] += v - l;
        l = v;
        if (t == 0) k += 1 else k -= 1;
    }
    for (1..n+1) |i| print("{d} ", .{ans[i]});
    print("\n", .{});
