...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var x: [5000]u32 = undefined;
    for (0..n) |i| x[i]  = readInt(u32);
    var bonus: [5001]u32 = @splat(0);
    for (0..m) |_| {
        const c = readInt(u32);
        const y = readInt(u32);
        bonus[c] = y;
    }
    var dp: [5001]i64 = @splat(math.minInt(i64));
    dp[0] = 0;
    for (0.., x[0..n]) |i, xi| {
        var ndp: [5001]i64 = undefined;
        ndp[0] = mem.max(i64, dp[0..i + 1]);
        for (1..i + 2) |j| {
            ndp[j] = dp[j - 1] + bonus[j] + xi;
        }
        @memcpy(dp[0..i + 2], ndp[0..i + 2]);
    }
    print("{d}\n", .{mem.max(i64, dp[0..n + 1])});
}
