...
pub fn solve() !void {
    ...
    var ta: [2<<17]struct {u8, u32} = undefined;
    for (0..n) |i| ta[i] = .{ readInt(u8), readInt(u32) };
    var s: u5 = 0;
    const tbl: [4][2][2]u1 = .{
        @splat(@splat(0)),
        .{.{0, 0}, .{0, 1}},
    ...
    while (s < 30) : (s += 1) {
        var dp: [2]u1 = .{0, 1};
        var cur: u1 = @intCast((c >> s) & 1);
        for (0.., ta[0..n]) |i, tai| {
            const ti, const ai = tai;
            const bi = (ai >> s) & 1;
            const ndp: [2]u1 = .{tbl[ti][dp[0]][bi], tbl[ti][dp[1]][bi]};
            dp = ndp;
            cur = dp[cur];
            ans[i] |= @as(u32, cur) << s;
        }
    }
    for (ans[0..n]) |ai| print("{d}\n", .{ai});
}
