...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var loc: [1<<20]u32 = undefined;
    var loc_rev: [1<<20]u32 = undefined;
    var pos: [1<<20]u32 = undefined;
    {
        var i: u32 = 1;
        while (i <= n) : (i += 1) {
            loc[i] = i;
    ...
    for (0..q) |_| {
        ...
        if (t == 1) {
            ...
            const b = readInt(u32);
            mem.swap(u32, &loc[loc_rev[a]], &loc[loc_rev[b]]);
            mem.swap(u32, &loc_rev[a], &loc_rev[b]);
        } else {
            assert(t==3);
            print("{d}\n", .{loc[pos[a]]});
        }
