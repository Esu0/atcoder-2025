...
pub fn solve() !void {
    const n = readInt(u32);
    var sum: [1<<17]u32 = @splat(0);
    var pq: std.AutoHashMap(struct {u32, u32}, u32) = .init(allocator);
    try pq.ensureTotalCapacity(n);
    var ans: i64 = 0;
    for (0..n) |_| {
        const p = readInt(u32);
        const q = readInt(u32);
        const res = pq.getOrPutAssumeCapacity(.{p, q});
    ...
    for (1..100_001) |i| ans += @divExact((@as(i64, sum[i]) - 1) * sum[i], 2);
    print("{d}\n", .{@as(u64, @intCast(ans))});
}
