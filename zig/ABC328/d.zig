...
pub fn solve() !void {
    const s = readString();
    var stack = try std.ArrayList(u8).initCapacity(allocator, s.len);
    for (s) |si| {
        stack.appendAssumeCapacity(si);
        if (stack.items.len >= 3 and std.mem.eql(u8, stack.items[stack.items.len-3..], "ABC")) stack.shrinkRetainingCapacity(stack.items.len - 3);
    }
    print("{s}\n", .{stack.items});
}
