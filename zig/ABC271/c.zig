...
pub fn solve() !void {
    ...
    a[n] = math.maxInt(u32);
    var r: u32 = 0;
    var b: std.ArrayList(u32) = try .initCapacity(allocator, n);
    for (0..n) |i| {
        if (a[i] != a[i + 1]) b.appendAssumeCapacity(a[i]) else r += 1;
    }
    var c: u32 = 0;
    var i: u32 = 0;
    var j: u32 = @intCast(b.items.len);
    while (j - i > 0 or r > 0) {
        if (j - i > 0 and b.items[i] == c + 1) {
            i += 1;
        } else {
            if (r == 0) {
                j -= 1;
