...
pub fn solve() !void {
    const n = readInt(u32);
    const s = readInt(u32);
    var ab: [101]struct {u32, u32} = undefined;
    var dp = try allocator.alloc([]?bool, n + 1);
    var i: u32 = 0;
    dp[i] = try allocator.alloc(?bool, s + 1);
    @memset(dp[i], null);
    dp[i][0] = false;
    i += 1;
    while (i <= n) : (i += 1) {
        ...
        var j: u32 = 0;
        while (j <= s) : (j += 1) {
            if (j >= ai and dp[i - 1][j - ai] != null) dp[i][j] = false;
            if (j >= bi and dp[i - 1][j - bi] != null) dp[i][j] = true;
        }
    }
    if (dp[n][s] == null) {
    ...
    var ans: std.ArrayList(u8) = try .initCapacity(allocator, n);
    var k = s;
    i = n;
    while (i > 0) : (i -= 1) {
        ans.appendAssumeCapacity(if (dp[i][k].?) 'T' else 'H');
        if (dp[i][k].?) {
            k -= ab[i][1];
        } else {
            k -= ab[i][0];
    ...
    assert(k == 0);
    while (ans.pop()) |c| try stdout.writeByte(c);
