...
pub fn solve() !void {
    ...
    for (0..n) |i| a[i] = readInt(u32) - 1;
    var ok: u32 = 0;
    var ng = n + 1;
    var set = try allocator.alloc(bool, n);
    while (ng - ok > 1) {
        const mid = (ng + ok) / 2;
        @memset(set[0..mid], false);
        var c: u32 = 0;
        for (a[0..n]) |ai| {
            if (ai >= mid or set[ai]) c += 1;
            if (ai < mid) set[ai] = true;
        }
        var r: u32 = 0;
        for (set[0..mid]) |f| r += @intFromBool(!f);
        if (c >= r * 2) ok = mid else ng = mid;
    }
    print("{d}\n", .{ok});
}
