...
pub fn solve() !void {
    ...
    var dp: [2]bool = @splat(true);
    var pb = readInt(u32);
    for (1..n) |i| {
        const pab: [2]u32 = .{a[i - 1], pb};
        const bi = readInt(u32);
        const ab: [2]u32 = .{a[i], bi};
        var ndp: [2]bool = @splat(false);
        inline for (0..2) |ii| {
            inline for (0..2) |jj| {
                if (dp[jj] and ab[ii] <= pab[jj] + k and pab[jj] <= ab[ii] + k) ndp[ii] = true;
            }
        }
        pb = bi;
        dp = ndp;
    }
    try stdout.writeAll(if (dp[0] or dp[1]) "Yes\n" else "No\n");
}
