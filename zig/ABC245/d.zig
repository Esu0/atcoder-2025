...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var a: [101]i32 = undefined;
    for (0..n+1) |i| a[i] = readInt(i32);
    var c: [201]i32 = undefined;
    for (0..n+m+1) |i| c[i] = readInt(i32);
    var b: [101]i32 = undefined;
    var i = m + 1;
    while (i > 0) {
        i -= 1;
        b[i] = @divExact(c[i+n], a[n]);
        var j: u32 = 0;
        while (j <= n) : (j += 1) {
            c[j + i] -= a[j] * b[i];
        }
    }
    for (b[0..m+1]) |bi| print("{d} ", .{bi});
