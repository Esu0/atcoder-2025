...
var col: [1<<17]u32 = @splat(0);
var k: u32 = undefined;

fn dfs(u: u32, s: u32) void {
    var c = s;
    for (g[u].items) |e| {
        const i, const to = e;
        if (col[i] != 0) continue;
...
pub fn solve() !void {
    ...
    while (i < n - 1) : (i += 1) {
        ...
        try g[a].append(allocator, .{ i, b });
        try g[b].append(allocator, .{ i, a });
    }
    k = 0;
    for (0..n) |j| k = @max(@as(u32, @intCast(g[j].items.len)), k);
    dfs(0, 1);
    print("{d}\n", .{k});
    for (0..n - 1) |j| {
        assert(col[j] != 0);
    ...
    //     for (g[i].items) |e| {
    //         assert(try set.fetchPut(col[e[0]], {}) == null);
    //     }
    // }
}
