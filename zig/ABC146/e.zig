...
pub fn solve() !void {
    ...
    try map.ensureTotalCapacity(n + 1);
    map.putAssumeCapacityNoClobber(0, 1);
    var ans: u64 = 0;
    var i: u32 = 1;
    // std.debug.print("{any}\n", .{b[0..n+1]});
    while (i <= n) : (i += 1) {
        if (i >= k) {
            const res = map.getEntry(b[i - k]).?;
            res.value_ptr.* -= 1;
        }
        ans += map.get(b[i]) orelse 0;
        const res = map.getOrPutAssumeCapacity(b[i]);
