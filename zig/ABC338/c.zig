...
pub fn solve() !void {
    const n = readInt(u32);
    var q: [10]u32 = undefined;
    var a: [10]u32 = undefined;
    var b: [10]u32 = undefined;
    for (0..n) |i| q[i] = readInt(u32);
    ...
    outer: while (true) {
        ...
        for (q[0..n], a[0..n], b[0..n]) |qi, ai, bi| {
            if (ai * k > qi) break :outer;
            if (bi != 0) min = @min(min, (qi - ai * k) / bi);
        }
        ans = @max(ans, k + min);
        k += 1;
    }
