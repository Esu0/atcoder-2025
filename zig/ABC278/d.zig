...
pub fn solve() !void {
    const n = readInt(u32);
    var base: u32 = 0;
    var diff: [2<<17]u64 = undefined;
    for (1..n+1) |i| diff[i] = readInt(u32);
    const q = readInt(u32);
    var stack: std.ArrayList(u32) = try .initCapacity(allocator, n+q);
    {var i: u32 = 1; while (i <= n) : (i += 1) stack.appendAssumeCapacity(i);}
    for (0..q) |_| {
        const t = readInt(u4);
        if (t == 1) {
            const x = readInt(u32);
            base = x;
            while (stack.pop()) |i| diff[i] = 0;
        } else if (t == 2) {
            const i = readInt(u32);
            const x = readInt(u32);
            if (diff[i] == 0) stack.appendAssumeCapacity(i);
            diff[i] += x;
        } else {
            assert(t == 3);
            const i = readInt(u32);
