...
const Item = struct {
    ...
    const id: Item = .{ .time = 0, .char = 'A' };
    fn op(self: Item, other: Item) Item {
        return if (self.time > other.time) self else other;
    }
};
...
pub fn solve() !void {
    ...
    const w = readInt(u32);
    const q = readInt(u32);
    var imos: [1<<20][]Item = undefined;
    for (0..h) |i| {
        imos[i] = try allocator.alloc(Item, w);
        @memset(imos[i], .id);
    }
    for (1..q + 1) |t| {
        ...
        const x = readChar();
        imos[r][c] = .{ .time = @intCast(t), .char = x };
    }
    for (0..h) |i| {
        var j: u32 = w - 1;
        while (j > 0) : (j -= 1) {
            imos[i][j - 1] = imos[i][j - 1].op(imos[i][j]);
        }
    }
    {var i: u32 = h - 1;
    while (i > 0) : (i -= 1) {
        for (0..w) |j| {
            imos[i - 1][j] = imos[i - 1][j].op(imos[i][j]);
        }
    }}
    for (0..h) |i| {
        for (0..w) |j| try stdout.writeByte(imos[i][j].char);
