...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var col: [3<<17]u32 = undefined;
    var count: [3<<17]u32 = @splat(0);
    var ans: u32 = 0;
    var q: [3<<17]std.ArrayList(struct {u32, u32}) = undefined;
    for (0..m) |i| q[i] = .initBuffer(&.{});
    for (0..n) |i| {
        const a = readInt(u32) - 1;
        col[i] = a;
        if (count[a] == 0) ans += 1;
        count[a] += 1;
        const d = readInt(u32) - 1;
        const b = readInt(u32) - 1;
        try q[d].append(allocator, .{@intCast(i), b});
    }

    for (0..m) |i| {
        for (q[i].items) |item| {
            const j, const c = item;
            count[col[j]] -= 1;
            if (count[col[j]] == 0) ans -= 1;
            if (count[c] == 0) ans += 1;
            count[c] += 1;
            col[j] = c;
        }
