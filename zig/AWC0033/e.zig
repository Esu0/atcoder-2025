...
pub fn solve() !void {
    const n = readInt(u32);
    const k = readInt(u32);
    var queue: FixedDeque(struct {u64, u32}, 0, 1<<20) = .{};
    // var dp: [1<<20]u64 = undefined;
    // dp[0] = 0;
    queue.pushBack(.{readInt(u32), 1});
    for (2..n) |i| {
        const a = readInt(u32);
        const dpi = queue.front().?[0] + a;
        while (queue.back()) |last| {
            if (last[0] < dpi) break;
            _ = queue.popBack();
        }
        queue.pushBack(.{ dpi, @intCast(i) });
        if (i >= k) {
            if (queue.front()) |f| {
                if (f[1] == i - k) _ = queue.popFront();
            }
        }
    }
    const a = readInt(u32);
