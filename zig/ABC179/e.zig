...
pub fn solve() !void {
    ...
    const x = readInt(u32);
    const m = readInt(u32);
    const idx = try allocator.alloc(u32, m);
    @memset(idx, math.maxInt(u32));
    var arr: std.ArrayList(u32) = try .initCapacity(allocator, m);
    var cur = x;
    while (idx[cur] == math.maxInt(u32)) {
        idx[cur] = @intCast(arr.items.len);
        arr.appendAssumeCapacity(cur);
        cur = @intCast(@as(u64, cur) * cur % m);
    }
    if (idx[cur] >= n) {
        var ans: u64 = 0;
        for (arr.items[0..n]) |v| ans += v;
        print("{d}\n", .{ans});
        return;
    }
    const l = idx[cur];
    var ans: u64 = 0;
    for (arr.items[0..l]) |v| ans += v;
    const k = n - l;
    const t = arr.items.len - l;
    var cumsum = try allocator.alloc(u64, t + 1);
    cumsum[0] = 0;
    for (l..arr.items.len) |i| {
        cumsum[i-l + 1] = cumsum[i-l] + arr.items[i];
    }
    ans += cumsum[cumsum.len-1] * (k / t) + cumsum[k % t];
    print("{d}\n", .{ans});
}
