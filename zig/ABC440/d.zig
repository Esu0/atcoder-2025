...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var a: [3<<17]u32 = undefined;
    for (0..n) |i| a[i] = readInt(u32);
    mem.sort(u32, a[0..n], {}, std.sort.asc(u32));
    for (0..q) |_| {
        const x = readInt(u32);
        const y = readInt(u32);
        const k = std.sort.lowerBound(u32, a[0..n], x, struct {fn compare(ctx: u32, val: u32) math.Order {
            return math.order(ctx, val);
        }}.compare);
        // std.debug.print("k: {d}\n", .{k});
        var l: u32 = x;
        var r: u32 = math.maxInt(u32);
        while (r - l > 1) {
            const m = l + (r - l) / 2;
            const cnt: u32 = @intCast(std.sort.lowerBound(u32, a[k..n], m, struct {fn compare(ctx: u32, val: u32) math.Order {
                return math.order(ctx, val);
            }}.compare));
            if (m - x - cnt > y - 1) {
                r = m;
