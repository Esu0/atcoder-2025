...
pub fn solve() !void {
    ...
    const k = readInt(u32);
    var x = readInt(u32);
    var a: [50]i32 = undefined;
    for (0..n) |i| a[i] = readInt(i32);
    var queue: std.PriorityQueue(Item, void, struct {fn compare(_: void, lhs: Item, rhs: Item) math.Order {
        return math.order(rhs.sum, lhs.sum);
    }}.compare) = .init(allocator, {});
    mem.sort(i32, a[0..n], {}, std.sort.desc(i32));

    var buffer: [1<<17][50]u32 = undefined;
    var last: u32 = 1;
    buffer[0] = @splat(0);
    buffer[0][0] = k;

    const sum = @as(i64, a[0]) * k;
    print("{d}\n", .{sum});
    x -= 1;

    const root_count = &buffer[0];
    for (0..n - 1) |i| {
        if (root_count[i] == 0) continue;
        try queue.add(.{ .sum = sum - a[i] + a[i + 1], .dir = @intCast(i), .arr = root_count });
    }
    while (x > 0) : (x -= 1) {
        const item = queue.remove();
        print("{d}\n", .{item.sum});
        const arr = &buffer[last];
        last += 1;
        arr.* = item.arr.*;
        arr[item.dir] -= 1;
        arr[item.dir + 1] += 1;
        for (item.dir..n - 1) |i| {
            if (arr[i] == 0) continue;
            try queue.add(.{ .sum = item.sum - a[i] + a[i + 1], .dir = @intCast(i), .arr = arr });
        }
    }
}
