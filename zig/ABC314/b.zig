...
pub fn solve() !void {
    const n = readInt(u32);
    var s: [100][37]bool = @splat(@splat(false));
    ...
    for (0..n) |i| {
        if (s[i][x]) min = @min(min, c[i]);
    }
    var ans = try std.ArrayList(usize).initCapacity(allocator, n);
    for (0..n) |i| {
        if (s[i][x] and min == c[i]) ans.appendAssumeCapacity(i + 1);
    }
    print("{d}\n", .{ans.items.len});
    for (ans.items) |t| print("{d} ", .{t});
