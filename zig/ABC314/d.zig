...
pub fn solve() !void {
    ...
    const q = readInt(u32);
    assert(s.len == n);
    var txc: [5<<17]struct {u8, u32, u8} = undefined;
    var last: ?usize = null;
    for (0..q) |i| {
        txc[i] = .{ readInt(u8), readInt(u32), readChar() };
        if (txc[i][0] != 1) {
            last = i;
        }
    }
    ...
    for (0..q) |i| {
        const ti, const xi, const ci = txc[i];
        if (ti == 1) {
            s[xi - 1] = ci;
        } else {
            if (i != last) continue;
            if (ti == 2) {
                for (0..s.len) |j| s[j] = std.ascii.toLower(s[j]);
            } else if (ti == 3) {
                for (0..s.len) |j| s[j] = std.ascii.toUpper(s[j]);
            }
        }
    }
    print("{s}\n", .{s});
