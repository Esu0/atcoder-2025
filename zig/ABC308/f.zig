...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var p: [2<<17]u32 = undefined;
    for (0..n) |i| p[i] = readInt(u32);
    var ld: [2<<17]struct {u32, u32} = undefined;
    for (0..m) |i| ld[i][0] = readInt(u32);
    for (0..m) |i| ld[i][1] = readInt(u32);
    mem.sortUnstable(u32, p[0..n], {}, std.sort.asc(u32));
    mem.sortUnstable(struct {u32, u32}, ld[0..m], {}, struct {fn lessThan(_: void, lhs: struct {u32, u32}, rhs: struct {u32, u32}) bool {
        return lhs[0] < rhs[0];
    }}.lessThan);
    ...
    var ans: u64 = 0;
    for (p[0..n]) |pi| {
        while (j < m and ld[j][0] <= pi) {
            try queue.add(ld[j][1]);
            j += 1;
        }
        ans += pi - (queue.removeOrNull() orelse 0);
    }
    print("{d}\n", .{ans});
}
