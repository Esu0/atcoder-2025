...
pub fn solve() !void {
    ...
    var is: [2<<17]u32 = undefined;
    {var i: u32 = 1; while (i <= n) : (i += 1) {
        is[i-1] = i;
        const ai = readInt(u32);
        const bi = ai + readInt(u32);
        ab[i] = .{ ai, bi };
    }}

    mem.sort(u32, is[0..n], ab[0..n+1], struct {fn lessThan(ctx: []struct {u32, u32}, lhs: u32, rhs: u32) bool {
        // ai / bi > aj / bj
        return @as(u64, ctx[lhs][0]) * ctx[rhs][1] > @as(u64, ctx[lhs][1]) * ctx[rhs][0];
    }}.lessThan);
    for (0..n) |i| {
        print("{d} ", .{is[i]});
    }
