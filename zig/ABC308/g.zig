...
pub fn solve() !void {
    const q = readInt(u32);
    var trie: std.ArrayList([2]u32) = try .initCapacity(allocator, q * 30+1);
    trie.appendAssumeCapacity(@splat(0));
    var dp: std.ArrayList(struct {u32, u32}) = try .initCapacity(allocator, q * 30 + 1);
    dp.appendAssumeCapacity(.{ math.maxInt(u32), math.maxInt(u32) });
    var set: std.AutoHashMap(u32, u32) = .init(allocator);
    var zero: u32 = 0;

    var stack: std.ArrayList(u32) = try .initCapacity(allocator, 30);
    for (0..q) |_| {
        const t = readInt(u4);
        if (t == 1) {
            const x = readInt(u32);
            const result = try set.getOrPut(u32);
            if (result.found_existing) {
                ...
                while (i > 0) {
                    ...
                    const b = (x >> i) & 1;
                    if (trie[cur][b] == 0) {
                        trie[cur][b] = @intCast(trie.items.len);
                        trie.appendAssumeCapacity(.{0, 0});
                        dp.appendAssumeCapacity(.{ math.maxInt(u32), math.maxInt(u32) });
                    }
                    stack.appendAssumeCapacity(cur);
                    cur = trie[cur][b];
                }
                dp[cur][1] = x;
                while (stack.pop()) |u| {
                    ...
                    if (trie[u][0] != 0) {
                        v = @min(v, dp[trie[u][0]][0]);
                        w = dp[trie[u][0]][1];
                    }
                    if (trie[u][1] != 0) {
                        v = @min(v, dp[trie[u][1]][0]);
                        w = dp[trie[u][1]][1];
                    }
                    if (trie[u][0] != 0 and trie[u][1] != 0) {
                        assert(dp[trie[u][0]][1] != math.maxInt(u32));
                        assert(dp[trie[u][1]][1] != math.maxInt(u32));
                        v = @min(v, dp[trie[u][0]] ^ dp[trie[u][1]]);
                    }
                    dp[u] = .{ v, w };
                }
            }
            ...
            const x = readInt(u32);
            const entry = set.getEntry(x).?;
            entry.value_ptr.* -= 1;
            if (entry.value_ptr.* == 0) {
                set.removeByPtr(entry.key_ptr);
