...
pub fn solve() !void {
    const h = readInt(u32);
    const w = readInt(u32);
    var s: [500][]u8 = undefined;
    for (0..h) |i| s[i] = readString();
    var next: [128]u8 = @splat(0);
    const tar = "snukes";
    for (0..5) |i| {
        next[tar[i]] = tar[i+1];
    }
    var vis: [500][500]bool = @splat(@splat(false));
    var stack: std.ArrayList(struct {u16, u16}) = try .initCapacity(allocator, h * w);
    if (s[0][0] == 's') {
        vis[0][0] = true;
    ...
    while (stack.pop()) |u| {
        ...
        if (i > 0) {
            const ni = i - 1;
            const nj = j;
            if (!vis[ni][nj] and s[ni][nj] == next[sij]) {
    ...
    try stdout.writeAll(if (vis[h-1][w-1]) "Yes\n" else "No\n");
}
