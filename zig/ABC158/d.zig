...
pub fn solve() !void {
    const s = readString();
    const q = readInt(u32);
    var head: std.ArrayList(u8) = try .initCapacity(allocator, q);
    var tail: std.ArrayList(u8) = try .initCapacity(allocator, q);
    var rev: bool = false;
    for (0..q) |_| {
        const t = readInt(u8);
        if (t == 1) {
            ...
            const f = readInt(u8);
            const c = readChar();
            (if ((f == 1) ^ rev) &head else &tail).appendAssumeCapacity(c);
        }
    }
    if (rev) {
        mem.reverse(u8, s);
        mem.swap(@TypeOf(head), &head, &tail);
    }
    mem.reverse(u8, head.items);
    print("{s}{s}{s}\n", .{head.items, s, tail.items});
}
