...
pub fn solve() !void {
    const s = readString();
    var set0: [26]u8 = @splat(0);
    for (s) |si| set0[si - 'a'] += 1;
    var set: std.ArrayList(struct {u8, u8}) = try .initCapacity(allocator, 8);
    {var i: u8 = 0; while (i < 26) : (i += 1) {
        if (set0[i] != 0) set.appendAssumeCapacity(.{i + 'a', set0[i]});
    }}
    // std.debug.print("{any}\n", .{set.items});
    K = readInt(u32);
    S = set.items;
