...
pub fn solve() !void {
    const n = readInt(u32);
    var inva: std.AutoHashMap(u32, u32) = .init(allocator);
    var invb: std.AutoHashMap(u32, u32) = .init(allocator);
    var i: u32 = 0;
    while (i < n) : (i += 1) {
        const a = readInt(u32);
        try inva.putNoClobber(a, i);
    }
    i = 0;
    while (i < n) : (i += 1) {
        const b = readInt(u32);
        try invb.putNoClobber(b, i);
    }
    var a1: u32 = 0;
    var a2: u32 = 0;
    var iter = inva.iterator();
    while (iter.next()) |entry| {
        const a = entry.key_ptr.*;
        if (invb.get(a)) |p| {
            if (p == entry.value_ptr.*) a1 += 1 else a2 += 1;
        }
    }
    print("{d}\n{d}\n", .{a1, a2});
}
