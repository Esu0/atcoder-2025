...
pub fn solve() !void {
    ...
    for (0..t) |_| {
        ...
        var c = b;
        while (r >= 2) : (r -= 2) {
            @memset(ans[head..head+w-1], 'R');
            head += w - 1;
            ans[head] = 'D';
            head += 1;
            @memset(ans[head..head+w-1], 'L');
            head += w - 1;
            ans[head] = 'D';
            head += 1;
            h -= 2;
        ...
        while (c >= 2) : (c -= 2) {
            @memset(ans[head..head+h-1], 'D');
            head += h - 1;
            ans[head] = 'R';
            head += 1;
            @memset(ans[head..head+h-1], 'U');
            head += h - 1;
            ans[head] = 'R';
            head += 1;
            w -= 2;
        ...
        while (h - r - 1 >= 2) : (h -= 2) {
            @memset(ans[tail-(w-1)..tail], 'R');
            tail -= w;
            ans[tail] = 'D';
            @memset(ans[tail-(w-1)..tail], 'L');
            tail -= w;
            ans[tail] = 'D';
        }
        while (w - c - 1 >= 2) : (w -= 2) {
            @memset(ans[tail-(h-1)..tail], 'D');
            tail -= h;
            ans[tail] = 'R';
            @memset(ans[tail-(h-1)..tail], 'U');
            tail -= h;
            ans[tail] = 'R';
        }
        assert(r == 1 and c == 0 or r == 0 and c == 1);
        assert(h == 2 and w == 2);
        if (r == 1 and c == 0) {
            ans[head] = 'R';
            ans[head + 1] = 'D';
