...
fn del(s: []u8) !std.ArrayList(u8) {
    var t: std.ArrayList(u8) = try .initCapacity(allocator, s.len);
    for (s) |si| {
        t.appendAssumeCapacity(si);
        if (t.items.len >= 4 and mem.eql(u8, t.items[t.items.len - 4..], "(xx)")) {
            t.shrinkRetainingCapacity(t.items.len - 4);
            t.appendSliceAssumeCapacity("xx");
        }
    }
    return t;
}
pub fn solve() !void {
    ...
    for (0..t) |_| {
        ...
        const a1 = try del(a);
        const b1 = try del(b);
        try stdout.writeAll(if (mem.eql(u8, a1.items, b1.items)) "Yes\n" else "No\n");
    }
}
