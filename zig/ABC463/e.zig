...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const y = readInt(u32);
    var g: [2<<17]std.ArrayList(struct {u32, u32}) = undefined;
    for (0..n+2) |i| g[i] = .initBuffer(&.{});
    for (0..m) |_| {
        const u = readInt(u32) - 1;
        const v = readInt(u32) - 1;
        const t = readInt(u32);
        try g[u].append(allocator, .{v ,t});
        try g[v].append(allocator, .{u, t});
    }
    var i: u32 = 0; while (i < n) : (i += 1) {
        const xi = readInt(u32);
        try g[i].append(allocator, .{n, xi});
        try g[n + 1].append(allocator, .{i, xi});
    }
    try g[n].append(allocator, .{n + 1, y});

    var dist: [2<<17]u64 = @splat(math.maxInt(u64));
    var queue: std.PriorityQueue(struct {u32, u64}, void, struct {fn compare(_: void, lhs: struct{u32, u64}, rhs: struct {u32, u64}) math.Order {
        return math.order(lhs[1], rhs[1]);
    }}.compare) = .init(allocator, {});
    try queue.ensureTotalCapacity(n*3 + m + 1);
    try queue.add(.{ 0, 0 });
    while (queue.removeOrNull()) |elem| {
        const u, const d = elem;
        if (dist[u] != math.maxInt(u64)) continue;
        dist[u] = d;
        for (g[u].items) |edge| {
            const to, const c = edge;
            if (dist[to] == math.maxInt(u64)) {
                try queue.add(.{ to, d + c });
            }
        }
    }
    var k: u32 = 1;
    while (k < n) : (k += 1) print("{d} ", .{dist[k]});
    print("\n", .{});
}
