...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u5);
    const s = readInt(u64) - 1;
    const t = readInt(u64) - 1;
    const sr: u32 = @intCast(s / n);
    const sc: u32 = @intCast(s % n);
    const tr: u32 = @intCast(t / n);
    const tc: u32 = @intCast(t % n);
    var p: [15]struct {u32, u32} = undefined;
    var dp: [1<<15][15]u64 = @splat(@splat(math.maxInt(u64)));

    if (m == 0) {
        print("{d}\n", .{absDiff(sr, tr) + absDiff(sc, tc)});
        return;
    }
    {var i: u5 = 0; while (i < m) : (i += 1) {
        const pi = readInt(u64) - 1;
        const pr: u32 = @intCast(pi / n);
        const pc: u32 = @intCast(pi % n);
        p[i] = .{pr, pc};
        dp[@as(u32, 1) << i][i] = absDiff(sr, pr) + absDiff(sc, pc);
    }}
    for (1..(@as(u32, 1) << m)) |set| {
        {var i: u5 = 0; while (i < m) : (i += 1) {
            if ((set >> i) & 1 == 0) continue;
            {var j: u5 = 0; while (j < m) : (j += 1) {
                if ((set >> j) & 1 != 0) continue;
                const idx = set | (@as(u32, 1) << j);
                dp[idx][j] = @min(dp[idx][j], dp[set][i] + absDiff(p[i][0], p[j][0]) + absDiff(p[i][1], p[j][1]));
            }}
        }}
    }
    var ans: u64 = math.maxInt(u64);
    for (0..m) |i| {
        ans = @min(ans, dp[(@as(u32, 1) << m) - 1][i] + absDiff(p[i][0], tr) + absDiff(p[i][1], tc));
    }
    print("{d}\n", .{ans});
}
