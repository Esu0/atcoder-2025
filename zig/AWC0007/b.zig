...
pub fn solve() !void {
    const n = readInt(u32);
    const k = readInt(u32);
    var sets: [500]std.StringHashMap(void) = undefined;
    var ans: u32 = 0;
    for (0..n) |i| {
        sets[i] = .init(allocator);
        const m = readInt(u32);
        var w: [50][]u8 = undefined;
        for (0..m) |j| {
            w[j] = readString();
            try sets[i].putNoClobber(w[j], {});
        }
        for (0..i) |j| {
            var cnt: u32 = 0;
            for (w[0..m]) |wi| {
                if (sets[j].contains(wi)) cnt += 1;
            }
            ans += @intFromBool(cnt >= k);
        }
    }
    print("{d}\n", .{ans});
}
