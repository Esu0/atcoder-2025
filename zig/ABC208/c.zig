...
pub fn solve() !void {
    ...
    var idxs: [2<<17]u32 = undefined;
    for (0..n) |i| idxs[i] = @intCast(i);
    mem.sortUnstable(u32, idxs[0..n], a[0..n], struct {fn lessThan(ctx: []u32, lhs: u32, rhs: u32) bool {
        return ctx[lhs] < ctx[rhs];
    }}.lessThan);
    var ans: [2<<17]u64 = undefined;
    for (0..n) |i| {
        if (i < k % n) ans[idxs[i]] = k / n + 1 else ans[idxs[i]] = k / n;
    }
    for (0..n) |i| print("{d}\n", .{ans[i]});
}
