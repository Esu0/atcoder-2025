...
pub fn solve() !void {
    ...
    const h = readInt(u32);
    const m = readInt(u32);
    const t0 = math.pi * tof(h * 60 + m) / 360;
    const x0 = tof(a) * @cos(t0);
    const y0 = tof(a) * @sin(t0);
    const t1 = math.pi * tof(m) / 30;
    const x1 = tof(b) * @cos(t1);
    const y1 = tof(b) * @sin(t1);
