...
pub fn solve() !void {
    ...
    for (0..testcases) |_| {
        ...
        var cnta: u32 = 0;
        for (s[l + 1..r - 1]) |si| cnta += @intFromBool(si == 'A');
        {
            var tmp: u32 = 0;
            for (t[l + 1..r - 1]) |ti| tmp += @intFromBool(ti == 'A');
            if (tmp != cnta) {
                print("No\n", .{});
                continue;
            }
        }
        var opf: std.ArrayList(struct {u32, u32}) = try .initCapacity(allocator, n / 2);
        var opb: std.ArrayList(struct {u32, u32}) = try .initCapacity(allocator, n / 2);
        mem.reverse(u8, t[l + 1..r - 1]);
        if (r - l - 2 - cnta > cnta) {
            var i = r - 1;
            var j = r - 1;
            // Aが少ない
            for (0..cnta) |_| {
                ...
                j -= 1;
                if (j < i) {
                    opb.appendAssumeCapacity(.{j, i});
                } else if (i < j) {
                    opf.appendAssumeCapacity(.{i, j});
                }
            }
        } else {
            var i = l + 1;
            ...
            for (0..r - l - 2 - cnta) |_| {
                ...
                while (t[j] != 'B') j += 1;
                if (j < i) {
                    opf.appendAssumeCapacity(.{j, i});
                } else if (i < j) {
                    opb.appendAssumeCapacity(.{i, j});
                }
                i += 1;
                j += 1;
            }
        ...
        // mem.reverse(u8, t[l + 1..r - 1]);
        print("Yes\n{d}\n", .{opf.items.len + opb.items.len + 1});
        // assert(opf.items.len + opb.items.len + 1 <= n / 2);
        for (opf.items) |op| {
            // assert(s[op[0]] == 'A');
            // assert(s[op[1]] == 'B');
            // mem.reverse(u8, s[op[0]..op[1]+1]);
            print("{d} {d}\n", .{op[0] + 1, op[1] + 1});
        }
        // assert(s[l] == 'A');
        // assert(s[r - 1] == 'B');
        // mem.reverse(u8, s[l..r]);
        print("{d} {d}\n", .{l + 1, r});
        mem.reverse(struct {u32, u32}, opb.items);
        for (opb.items) |op| {
            // assert(s[r - op[1] + l - 1] == 'A');
            // assert(s[r - op[0] + l - 1] == 'B');
            // mem.reverse(u8, s[r - op[1] + l - 1..r - op[0] + l]);
            print("{d} {d}\n", .{r - op[1] + l, r - op[0] + l});
        }
        // assert(mem.eql(u8, s, t));
    }
}
