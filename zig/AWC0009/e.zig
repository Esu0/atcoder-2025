...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var segmin: [1<<18]i32 = undefined;
    var segmax: [1<<18]i32 = undefined;
    const OFF = 1<<17;
    for (0..n) |i| {
        const a = readInt(i32);
        segmin[OFF+i] = a;
    ...
    @memset(segmin[OFF+n..], math.maxInt(i32));
    @memset(segmax[OFF+n..], math.minInt(i32));
    ...
    for (0..q) |_| {
        ...
        var max: i32 = math.minInt(i32);
        while (r - l >= 1) {
            if (l & 1 != 0) {
                min = @min(segmin[l], min);
                max = @max(segmax[l], max);
                l += 1;
            }
            if (r & 1 != 0) {
                r -= 1;
                min = @min(segmin[r], min);
                max = @max(segmax[r], max);
            }
            l >>= 1;
            r >>= 1;
        }
        // std.debug.print("(l, r): {d}, {d}\n", .{l, r});
        // std.debug.print("{d}, {d}\n", .{min, max});
        print("{d}\n", .{max - min});
    }
}
