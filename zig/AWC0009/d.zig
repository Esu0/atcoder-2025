...
pub fn solve() !void {
    var n = readInt(u32);
    const m = readInt(u32);
    var lr: [1<<17]struct {u64, u64} = undefined;
    for (0..m) |i| lr[i] = .{ readInt(u64), readInt(u64) };
    mem.sortUnstable(struct {u64, u64}, lr[0..m], {}, struct { fn lessThan(_: void, lhs: struct {u64, u64}, rhs: struct {u64, u64}) bool {
        return lhs[0] < rhs[0];
    }}.lessThan);
    var maxr: u64 = 0;
    lr[m] = .{ math.maxInt(u64), math.maxInt(u64) };
    for (lr[0..m+1]) |lri| {
        const li, const ri = lri;
        if (li <= maxr) {
            maxr = @max(ri, maxr);
            continue;
        }
