...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var g = try allocator.alloc(std.AutoHashMap(u32, void), n);
    for (0..n) |i| g[i] = .init(allocator);
    var ans: u32 = n;
    for (0..q) |_| {
        const t = readInt(u4);
        if (t == 1) {
            ...
            if (g[u].count() == 0) ans -= 1;
            if (g[v].count() == 0) ans -= 1;
            try g[u].putNoClobber(v, {});
            try g[v].putNoClobber(u, {});
        } else {
            assert(t == 2);
            const v = readInt(u32) - 1;
            var iter = g[v].keyIterator();
            while (iter.next()) |u| {
                g[u.*].removeByPtr(g[u.*].getKeyPtr(v).?);
                if (g[u.*].count() == 0) ans += 1;
            }
            if (g[v].count() > 0) ans += 1;
            g[v].clearRetainingCapacity();
        }
        print("{d}\n", .{ans});
