...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var g: [4<<17]std.ArrayList(struct{u1, u32}) = undefined;
    for (0..n+m) |i| {
        g[i] = .initBuffer(&.{});
    }
    { var i: u32 = 0; while (i < n) : (i += 1) {
        const a = readInt(u32);
        for (0..a) |_| {
            const s = readInt(u32) - 1;
            try g[i+m].append(allocator, .{ 1, s });
            try g[s].append(allocator, .{0, i+m});
        }
    }}

    var queue: FixedDeque(struct { u32, u32 }, 1<<19, 1<<19) = .{};
    var dist: [4<<17]u32 = @splat(math.maxInt(u32));
    queue.pushBack(.{0, 0});
    while (queue.popFront()) |du| {
        const d, const u = du;
        if (dist[u] != math.maxInt(u32)) continue;
        dist[u] = d;
        for (g[u].items) |wv| {
            const w, const v = wv;
            if (w == 0) queue.pushFront(.{d, v}) else queue.pushBack(.{d + 1, v});
        }
    }
    if (dist[m - 1] == math.maxInt(u32)) {
