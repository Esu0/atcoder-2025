...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const d = readInt(u63);
    var a: [2<<17]u63 = undefined;
    var b: [2<<17]u63 = undefined;
    for (0..n) |i| a[i] = readInt(u63);
    for (0..m) |i| b[i] = readInt(u63);

    mem.sortUnstable(u63, b[0..m], {}, std.sort.asc(u63));
    var ans: i64 = -1;
    for (a[0..n]) |ai| {
        const k = std.sort.upperBound(u63, b[0..m], ai + d, struct { fn compare(x: u63, y: u63) math.Order {
            return math.order(x, y);
        }}.compare);
        if (k > 0 and b[k - 1] >= @as(i64, ai) - d) {
            ans = @max(ans, ai + b[k - 1]);
        }
    }
    print("{d}\n", .{ans});
}
