...
pub fn solve() !void {
    ...
    for (0..testcases) |_| {
        const n = readInt(u32);
        const d: i64 = readInt(u63);
        var tx: [5000]struct {i64, i64} = undefined;
        for (0..n) |i| tx[i] = .{ readInt(u63), readInt(u63) };
        mem.sortUnstable(struct {i64, i64}, tx[0..n], {}, struct {fn lessThan(_: void, lhs: struct {i64, i64}, rhs: struct {i64, i64}) bool {
            return lhs[0] + lhs[1] < rhs[0] + rhs[1];
        }}.lessThan);
        var dp: [5001]i64 = undefined;
        @memset(dp[1..n + 1], math.maxInt(i64));
        dp[0] = 0;
        var i: u32 = 0;
        while (i < n) : (i += 1) {
            const dpi = dp[i];
            if (dpi == math.maxInt(i64)) continue;
            var sum: i64 = 0;
            var border: i64 = math.maxInt(i64);
            var j: u32 = i;
            while (j < n) {
                const tj, const xj = tx[j];
                j += 1;
                var t = @max(dpi, tj - xj);
                t += 2 * xj;
                if (t <= d + tj + xj and t <= border) {
                    dp[j] = @min(dp[j], t + sum);
                }
                sum += 2 * xj;
                border = @min(border, d + tj + xj - sum);
            }
        }
        try stdout.writeAll(if (dp[n] == math.maxInt(i64)) "No\n" else "Yes\n");
    }
}
