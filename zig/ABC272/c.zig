...
pub fn solve() !void {
    ...
    for (0..n) |_| {
        const a = readInt(u32);
        if (a % 2 == 0) {
            if (even1 == null or even1.? < a) {
                even2 = even1;
                even1 = a;
            } else if (even2 == null or even2.? < a) {
                even2 = a;
            }
        } else {
            if (odd1 == null or odd1.? < a) {
                odd2 = odd1;
                odd1 = a;
            } else if (odd2 == null or odd2.? < a) {
                odd2 = a;
            }
        }
    }
    // std.debug.print("even: {any}, {any}\nodd: {any}, {any}\n", .{even1, even2, odd1, odd2});
    var ans: ?u32 = null;
    if (even1 != null and even2 != null) {
        ans = even1.? + even2.?;
    }
    if (odd1 != null and odd2 != null) {
        if (ans == null) ans = odd1.? + odd2.? else ans = @max(ans.?, odd1.? + odd2.?);
    }
    if (ans) |a| print("{d}\n", .{a}) else try stdout.writeAll("-1\n");
}
