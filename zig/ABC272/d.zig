...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var dxy: std.ArrayList(struct {u16, u16}) = .initBuffer(&.{});
    for (0..n) |di| {
        for (0..n) |dj| {
            if (di * di + dj * dj == m) {
                try dxy.append(allocator, .{ @intCast(di), @intCast(dj) });
            }
        }
    }
    var stack: std.ArrayList(struct {u32, u16, u16}) = try .initCapacity(allocator, n * n);
    var dist: [400][400]u32 = @splat(@splat(math.maxInt(u32)));
    stack.appendAssumeCapacity(.{ 0, 0, 0 });
    dist[0][0] = 0;
    {var i: u32 = 0;
    while (i < stack.items.len) : (i += 1) {
        const d, const x, const y = stack.items[i];
        for (dxy.items) |dxyi| {
            const dx, const dy = dxyi;
            inline for (0..2) |s1| {
                inline for (0..2) |s2| {
                    var nx = x;
                    var ny = y;
                    if (s1 == 0) nx += dx else nx -%= dx;
                    if (s2 == 0) ny += dy else ny -%= dy;
                    if (nx < n and ny < n and dist[nx][ny] == math.maxInt(u32)) {
                        try stack.append(allocator, .{ d + 1, nx, ny });
                        dist[nx][ny] = d + 1;
                    }
                }
            }
    ...
    for (0..n) |i| {
        for (0..n) |j| {
            print("{d}{c}", .{@as(i32, @bitCast(dist[i][j])), "\n "[@intFromBool(j != n - 1)]});
        }
    }
}
