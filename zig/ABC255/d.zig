...
pub fn solve() !void {
    ...
    var cumsum: [2<<17]u64 = undefined;
    cumsum[0] = 0;
    for (0..n) |i| cumsum[i + 1] = cumsum[i] + a[i];

    for (0..q) |_| {
        const x = readInt(u32);
        const k = std.sort.upperBound(u32, a[0..n], x, struct { fn compare(lhs: u32, rhs: u32) math.Order {
            return math.order(lhs, rhs);
        }}.compare);
        print("{d}\n", .{@as(u64, x) * k - cumsum[k] + (cumsum[n] - cumsum[k]) - @as(u64, x) * (n - k)});
    }
}
