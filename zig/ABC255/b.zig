...
fn dist(p1: struct {i32, i32}, p2: struct {i32, i32}) u64 {
    const dx = p1[0] - p2[0];
    const dy = p1[1] - p2[1];
    return @intCast(@as(i64, dx) * dx + @as(i64, dy) * dy);
}
...
pub fn solve() !void {
    ...
    for (0..k) |i| a[i] = readInt(u32) - 1;
    a[k] = 1000000;
    var ans: u64 = 0;
    var xy: [1000]struct {i32, i32} = undefined;
    for (0..n) |i| xy[i] = .{ readInt(i32), readInt(i32) };
    var j: u32 = 0;
    for (0..n) |i| {
        if (a[j] == i) {
            j += 1;
        ...
        var dmin: u64 = math.maxInt(u64);
        for (a[0..k]) |ai| {
            dmin = @min(dmin, dist(xy[i], xy[ai]));
        }
        ans = @max(ans, dmin);
    }
    print("{d:.15}\n", .{math.sqrt(@as(f64, @floatFromInt(ans)))});
}
