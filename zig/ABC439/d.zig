...
pub fn solve() !void {
    ...
    var ans: u64 = 0;
    var set0 = std.AutoHashMap(u32, u32).init(allocator);
    for (a[0..n]) |ai| {
        if (ai % 5 == 0) {
            const b = @divExact(ai, 5);
            ans += @as(u64, set0.get(b*3) orelse 0) * (set0.get(b*7) orelse 0);
        }
        {
            const result = try set0.getOrPut(ai);
