...
fn dfs(u: u32, k: u8) bool {
    if (k == 0) return g.s[u] == 'A';
    if (dp[k][u] != 0) {
        return dp[k][u] == 1;
    }
    var res = false;
    defer dp[k][u] = if (res) 1 else 2;
    for (g.g[u].items) |v| {
        if (!dfs(v, k - 1)) {
            res = true;
            return res;
...
pub fn solve() !void {
    ...
    for (0..test_cases) |_| {
        ...
        g.g = try allocator.alloc(std.ArrayList(u32), g.n);
        defer allocator.free(g.g);
        for (0..g.n) |i| g.g[i] = .initBuffer(&.{});
        for (0..g.m) |_| {
            const u = readInt(u32) - 1;
            const v = readInt(u32) - 1;
            try g.g[u].append(allocator, v);
        }
        try stdout.writeAll(if (dfs(0, g.k * 2)) "Alice\n" else "Bob\n");
    }
}
