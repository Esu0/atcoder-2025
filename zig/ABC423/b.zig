...
pub fn solve() !void {
    const n = readInt(u32);
    var l: [100]u1 = undefined;
    for (0..n) |i| l[i] = readInt(u1);
    if (mem.indexOfScalar(u1, l[0..n], 1)) |idx| {
        mem.reverse(u1, l[0..n]);
        const j = mem.indexOfScalar(u1, l[0..n], 1).?;
        print("{d}\n", .{n - 1 - @as(u32, @intCast(idx + j))});
    } else {
