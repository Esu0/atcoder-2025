...
var abcd: []struct {u8, u8, u8, u32} = undefined;

fn dfs(i: u8, prev: u8) void {
    if (i == n) {
        var sc: u32 = 0;
        for (abcd) |abcdi| {
            const a, const b, const c, const d = abcdi;
            if (buf[b] - buf[a] == c) sc += d;
        }
        ans = @max(ans, sc);
        return;
    }
...
pub fn solve() !void {
    ...
    m = readInt(u8);
    const q = readInt(u32);
    abcd = try allocator.alloc(struct{u8, u8, u8, u32}, q);
    for (0..q) |i| {
        abcd[i] = .{ readInt(u8) - 1, readInt(u8) - 1, readInt(u8), readInt(u32) };
    }
    dfs(0, 1);
    print("{d}\n", .{ans});
}
