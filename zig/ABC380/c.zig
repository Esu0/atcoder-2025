...
pub fn solve() !void {
    ...
    const k = readInt(u32);
    const s = readString();
    var rle: std.ArrayList(struct {u8, u32}) = try .initCapacity(allocator, n);
    var cnt: u32 = 0;
    for (0..n) |i| {
        cnt += 1;
        if (i == n - 1 or s[i] != s[i + 1]) {
            rle.appendAssumeCapacity(.{ s[i], cnt });
            cnt = 0;
        }
    }
    ...
    assert(i >= 1);
    mem.swap(@TypeOf(rle.items[0]), &rle.items[i], &rle.items[i - 1]);
    for (rle.items) |b| {
        for (0..b[1]) |_| {
            try stdout.writeByte(b[0]);
        }
    }
