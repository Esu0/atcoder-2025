...
pub fn solve() !void {
    const n = readInt(u5);
    var xy: [15][]struct {u5, u1} = undefined;
    for (0..n) |i| {
        const a = readInt(u32);
        xy[i] = try allocator.alloc(struct {u5, u1}, a);
        for (0..a) |j| {
            xy[i][j] = .{ readInt(u5) - 1, readInt(u1) };
        }
    }
    ...
    next: while (s < @as(u32, 1) << n) : (s += 1) {
        var i: u5 = 0;
        while (i < n) : (i += 1) {
            if (s & (@as(u32, 1) << i) == 0) continue;
            for (xy[i]) |xyi| {
                const x, const y = xyi;
                if ((s >> x) & 1 != y) continue :next;
            }
        }
