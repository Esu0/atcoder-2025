...
fn janken(a: u8, b: u8) Result {
    return if (a == b) .draw else if (a == 'G' and b == 'C' or a == 'C' and b == 'P' or a == 'P' and b == 'G') .win else .lose;
}
...
pub fn solve() !void {
    ...
    for (0..n+n) |i| a[i] = readString();
    var round: u32 = 0;
    var p: [100]struct {u32, u32} = undefined;
    {var i: u32 = 0; while (i < n+n) : (i += 1) {
        p[i] = .{ 0, i };
    }}
    while (round < m) : (round += 1) {
        for (0..n) |i| {
            const p1 = p[i * 2][1];
            const p2 = p[i * 2 + 1][1];
            switch (janken(a[p1][round], a[p2][round])) {
                .win => p[i * 2][0] += 1,
                .lose => p[i * 2 + 1][0] += 1,
                .draw => {},
            }
        }
        mem.sortUnstable(struct {u32, u32}, p[0..n+n], {}, struct {fn lessThan(_: void, lhs: struct {u32, u32}, rhs: struct {u32, u32}) bool {
            return if (lhs[0] == rhs[0]) lhs[1] < rhs[1] else lhs[0] > rhs[0];
        }}.lessThan);
    }
    for (0..n+n) |i| {
        print("{d}\n", .{p[i][1]+1});
