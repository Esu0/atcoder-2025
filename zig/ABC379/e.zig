...
pub fn solve() !void {
    const n = readInt(u32);
    const s = readString();
    var ansbuf: [2<<17]u64 = undefined;
    var ans: std.ArrayList(u64) = .initBuffer(&ansbuf);

    var sum: u64 = 0;
    var i: u32 = 0;
    while (i < n) : (i += 1) {
        sum += (i + 1) * (s[i] - '0');
        ans.appendAssumeCapacity(sum);
    }
    mem.reverse(u64, ans.items);
    var carry: u64 = 0;
    i = 0;
    while (i < ans.items.len) : (i += 1) {
        const a = ans.items[i] + carry;
        carry = a / 10;
        ans.items[i] = a % 10;
        if (ans.items.len - 1 == i and carry != 0) ans.appendAssumeCapacity(0);
    }
    var iter = mem.reverseIterator(ans.items);
    while (iter.next()) |d| {
        try stdout.writeByte(@intCast('0' + d));
    }
