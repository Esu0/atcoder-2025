...
pub fn solve() !void {
    const t = toF64(readInt(u32));
    const l = toF64(readInt(u32));
    const x = toF64(readInt(u32));
    const y = toF64(readInt(u32));
    const q = readInt(u32);
    for (0..q) |_| {
        const e = toF64(readInt(u32));
        const py = - l / 2 * math.sin(e / t * math.tau);
        const pz = l / 2 * (1.0 - math.cos(e / t * math.tau));
        const d0 = math.hypot(x, y - py);
        const d = math.sqrt(x * x + (py - y) * (py - y) + pz * pz);
        const ans = math.radiansToDegrees(math.acos(d0 / d));
        print("{d:.15}\n", .{ans});
    }
}
