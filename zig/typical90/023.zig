...
pub fn solve() !void {
    const h = readInt(u5);
    const w = readInt(u5);
    var idx: [1<<25]u32 = undefined;
    var buf1 = std.ArrayList(u32).initBuffer(&.{});
    try buf1.append(allocator, 0);
    try buf1.append(allocator, 1);
    {
        var i: u5 = 1;
        while (i <= w) : (i += 1) {
            const s = buf1.items.len;
            for (0..s) |j| {
                const k = buf1.items[j];
                if ((k >> (i - 1)) & 1 == 0)try buf1.append(allocator, k | (@as(u32, 1) << i));
            }
        }
    }
    for (0.., buf1.items) |i, k| {
        idx[k] = @intCast(i);
    }
    const key = buf1.items;
    // std.debug.print("{any}\n", .{key});

    var dp = try allocator.alloc(MInt, key.len);
    var ndp = try allocator.alloc(MInt, key.len);
    @memset(dp, .zero);
    dp[idx[0]] = .one;
    for (0..h) |_| {
        const s = readString();
        assert(s.len == w);
        {
            ...
            // var iter = dp.iterator();
            // while (iter.next()) |entry| {
            //     const k = entry.key_ptr.*;
            //     const v = entry.value_ptr.*;
            //     const e1 = try ndp.getOrPutValue(k << 1, .zero);
            //     e1.value_ptr.* = e1.value_ptr.*.add(v);
            //     if (c == '.' and k & 3 == 0) {
            //         const e2 = try ndp.getOrPutValue(2*k + 1, .zero);
            //         e2.value_ptr.* = e2.value_ptr.*.add(v);
            //     }
            // }
            mem.swap(@TypeOf(dp), &dp, &ndp);
        }
        // std.debug.print("{any}\n", .{dp});
        var i: u5 = 1;
        while (i < w) : (i += 1) {
            ...
            for (0.., dp) |j, v| {
                const k = key[j];
                {
                    const ni = idx[k & ~(@as(u32, 1) << i)];
                    ndp[ni] = ndp[ni].add(v);
                }
                if (s[i] == '.' and 15 & (k >> (i - 1)) == 0) {
                    const ni = idx[k | (@as(u32, 1) << i)];
                    ndp[ni] = ndp[ni].add(v);
                }
            }
            ...
            //     const v = entry.value_ptr.*;
            //     {
            //         const e = try ndp.getOrPutValue(k & ~(@as(u32, 1) << i), .zero);
            //         e.value_ptr.* = e.value_ptr.*.add(v);
            //     }
            //     if (s[i] == '.' and 15 & (k >> (i - 1)) == 0) {
            //         const e = try ndp.getOrPutValue(k | (@as(u32, 1) << i), .zero);
            //         e.value_ptr.* = e.value_ptr.*.add(v);
        ...
        }
        {
            ...
            //     const k = entry.key_ptr.*;
            //     const v = entry.value_ptr.*;
            //     const e = try ndp.getOrPutValue(k & ((@as(u32, 1) << w) - 1), .zero);
            //     e.value_ptr.* = e.value_ptr.*.add(v);
            // }
            mem.swap(@TypeOf(dp), &dp, &ndp);
        }
    }
    // var iter = dp.valueIterator();
    var ans = MInt.zero;
    for (dp) |v| ans = ans.add(v);
    print("{d}\n", .{ans.value});
}
