...
pub fn solve() !void {
    ...
    const w = readInt(u32);
    const q = readInt(u32);
    var col: [2002][2002]bool = @splat(@splat(false));
    var uf = try Unionfind.init(allocator, h * w);
    for (0..q) |_| {
        const t = readInt(u2);
        if (t == 1) {
            const r = readInt(u32);
            ...
            const k = (r-1)*w+c-1;
            col[r][c] = true;
            if (col[r-1][c]) _ = uf.unite((r-2)*w+c-1, k);
            if (col[r+1][c]) _ = uf.unite(r*w+c-1, k);
            if (col[r][c-1]) _ = uf.unite((r-1)*w+c-2, k);
            if (col[r][c+1]) _ = uf.unite((r-1)*w+c, k);
        } else {
            assert(t == 2);
            const ra = readInt(u32);
            const ca = readInt(u32);
            const rb = readInt(u32);
            const cb = readInt(u32);
            try stdout.writeAll(if (col[ra][ca] and col[rb][cb] and uf.find((ra-1)*w+ca-1) == uf.find((rb-1)*w+cb-1))"Yes\n" else "No\n");
        }
    }
}
