...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var buf: [3<<17]i32 = @splat(0);
    var fw: FenwickTree(i32, struct {fn binop(lhs: i32, rhs: i32) i32 {
        return lhs + rhs;
    }}.binop) = .init(&buf);
    var ls: [3<<17]std.ArrayList(u32) = undefined;
    for (0..n+1) |i| ls[i] = .initBuffer(&.{});
    for (0..m) |_| {
        const l = readInt(u32);
        const r = readInt(u32);
        try ls[r].append(allocator, l);
    }
    ...
    while (i <= n) : (i += 1) {
        for (ls[i].items) |l| {
            ans += fw.prefixSum(i).? - fw.prefixSum(l).?;
        }
        for (ls[i].items) |l| {
            fw.add(l, -1);
        }
        fw.add(i-1, @intCast(ls[i].items.len));
    }
    print("{d}\n", .{ans});
}
