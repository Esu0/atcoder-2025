...
pub fn solve() !void {
    const w = readInt(u32);
    const n = readInt(u32);
    var dp: [10001]i64 = @splat(math.minInt(i64));
    var ndp: [10001]i64 = undefined;
    var queue: FixedDeque(u32, 0, 10001) = .{};

    dp[0] = 0;
    for (0..n) |_| {
        @memcpy(ndp[0..w + 1], dp[0..w + 1]);
        const l = readInt(u32);
        const r = readInt(u32);
        const v = readInt(u32);
        var i = l;
        ...
        while (i <= r) : (i += 1) {
            const x = dp[i - l];
            while (queue.back()) |val| {
                if (dp[val] <= x) _ = queue.popBack() else break;
            }
            queue.pushBack(i - l);
            const idx = queue.front().?;
            ndp[i] = @max(ndp[i], dp[idx] + v);
        }
        while (i <= w) : (i += 1) {
            const x = dp[i - l];
            while (queue.back()) |val| {
            ...
            queue.pushBack(i - l);
            if (queue.front().? == i - r - 1) _ = queue.popFront();
            const idx = queue.front().?;
            ndp[i] = @max(ndp[i], dp[idx] + v);
        }
        @memcpy(dp[0..w + 1], ndp[0..w + 1]);
    }
    if (dp[w] < 0) {
