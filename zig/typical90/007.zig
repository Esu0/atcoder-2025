...
pub fn solve() !void {
    const n = readInt(u32);
    var a: [3<<17]i32 = undefined;
    for (0..n) |i| a[i] = readInt(u31);
    mem.sort(i32, a[0..n], {}, std.sort.asc(i32));
    const q = readInt(u32);
    for (0..q) |_| {
        const b: i32 = readInt(u31);
        const k = std.sort.upperBound(i32, a[0..n], b, struct { fn compare(x: i32, y: i32) std.math.Order {
            return math.order(x, y);
        }}.compare);
        print("{d}\n", .{if (k == 0) @abs(a[0]-b) else if (k == n) @abs(a[n-1]-b) else @min(@abs(a[k]-b),@abs(a[k-1]-b))});
    }
}
