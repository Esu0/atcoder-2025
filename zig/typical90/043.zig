...
pub fn solve() !void {
    const h = readInt(u32);
    const w = readInt(u32);
    const rs = readInt(u32);
    const cs = readInt(u32);
    const rt = readInt(u32);
    const ct = readInt(u32);

    var s: [1002][1002]u8 = undefined;
    @memset(&s[0], '#');
    for (1..h+1) |i| {
        const si = readString();
        assert(si.len == w);
    ...
    const drc: [5]u32 = .{ 0, 1, 0, math.maxInt(u32), 0 };

    var dist: [1002][1002][4]u32 = @splat(@splat(@splat(math.maxInt(u32))));
    ...
    {var k: u8 = 0; while (k < 4) : (k += 1) {
        const new_item = try allocator.create(Item);
        new_item.* = .{ .d = 0, .r = rs, .c = cs, .dir = k };
        queue.append(&new_item.node);
    }}
    while (queue.popFirst()) |node| {
        ...
        if (dist[r][c][dir] != math.maxInt(u32)) continue;
        dist[r][c][dir] = d;
        comptime var k: u8 = 0;
    ...
    const ans = mem.min(u32, &dist[rt][ct]);
    print("{d}\n", .{ans});
}
