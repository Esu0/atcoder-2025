...
pub fn solve() !void {
    const n = readInt(u32);
    var a: [400]u32 = undefined;
    for (0..n*2) |i| a[i] = readInt(u32);
    var dp: [201][401]u32 = @splat(@splat(math.maxInt(u32)));
    for (0..2*n + 1) |l| {
        dp[0][l] = 0;
    }
    var d: u32 = 1;
    while (d <= n) : (d += 1) {
        ...
        while (l + 2*d <= 2 * n) : (l += 1) {
            var i: u32 = 1;
            while (i < 2*d) : (i += 2) {
                dp[d][l] = @min(dp[d][l], dp[(i - 1) / 2][l + 1] +| dp[d - (i - 1) / 2 - 1][l + i + 1] +| absDiff(a[l], a[l+i]));
            }
        }
    }
    print("{d}\n", .{dp[n][0]});
