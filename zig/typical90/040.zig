...
const global = struct {
    var g: []const std.ArrayList(u32) = undefined;
    var cap: *[102][102]u32 = undefined;
    var dist: []u32 = undefined;
    var t: u32 = undefined;
    var ctx: [102]u32 = undefined;

    fn dfs(u: u32, flow: u32) u32 {
        if (u == t) return flow;
        var k = ctx[u];
        defer ctx[u] = k;
        while (k < g[u].items.len) {
            const v = g[u].items[k];
            k += 1;
            if (cap[u][v] > 0 and dist[u] < dist[v]) {
                const f = dfs(v, @min(flow, cap[u][v]));
                if (f > 0) {
                    cap[u][v] -= f;
                    cap[v][u] += f;
                    return f;
...
pub fn solve() !void {
    ...
    const w = readInt(u32);
    var cap: [102][102]u32 = undefined;
    var g: [102]std.ArrayList(u32) = undefined;

    for (0..n+2) |i| g[i] = .initBuffer(&.{});
    const s = n;
    const t = n + 1;
    var sum: u32 = 0;
    {
        ...
        while (i < n) : (i += 1) {
            const a = readInt(u32);
            sum += a;
            try g[s].append(allocator, i);
            try g[i].append(allocator, s);
            cap[s][i] = a;
            cap[i][s] = 0;
            try g[i].append(allocator, t);
            try g[t].append(allocator, i);
            cap[i][t] = w;
            cap[t][i] = 0;
        }
    }
    {
        ...
        while (i < n) : (i += 1) {
            ...
            for (0..k) |_| {
                ...
                try g[i].append(allocator, c);
                try g[c].append(allocator, i);
                cap[i][c] = 0;
                cap[c][i] = math.maxInt(u32);
            }
        }
    }

    var queue: FixedQueue(u32, 11000) = .{};
    var dist: [102]u32 = @splat(math.maxInt(u32));
    global.g = g[0..n+2];
    global.cap = &cap;
    global.dist = dist[0..n+2];
    global.t = t;
    var flow: u32 = 0;
    while (true) {
        queue.clear();
        queue.push(s);
        @memset(dist[0..n+2], math.maxInt(u32));
        dist[s] = 0;
        while (queue.pop()) |u| {
            const d = dist[u] + 1;
            for (g[u].items) |v| {
                if (cap[u][v] == 0) continue;
                if (dist[v] != math.maxInt(u32)) continue;
                dist[v] = d;
                queue.push(v);
            }
        }
        if (dist[t] == math.maxInt(u32)) break;
        // std.debug.print("{any}\n", .{dist[0..n+2]});
        @memset(global.ctx[0..n+2], 0);
        // dfs
        while (true) {
            const f = global.dfs(s, math.maxInt(u32));
            if (f == 0) break;
            flow += f;
        }
        // for (0..n + 2) |i| {
    ...
    // }
    // for (g[t].items) |j| {
    //     std.debug.print("{d} -> t: {d}\n", .{j, cap[j][t]});
    // }
    // for (g[t].items) |j| {
    //     std.debug.print("t -> {d}: {d}\n", .{j, cap[t][j]});
    // }
    // for (0..n + 2) |i| {
    //     for (g[i].items) |j| {
    //         std.debug.print("{d} -> {d}: {d}\n", .{i, j, cap[i][j]});
