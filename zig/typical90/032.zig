...
var mat: [10][10]bool = @splat(@splat(false));
var P: [10]u32 = undefined;
var ans: u32 = math.maxInt(u32);

fn dfs(i: u32, sum: u32) void {
    if (i == N) {
        // std.debug.print("{any}\n", .{P[0..N]});
        ans = @min(sum, ans);
        return;
    ...
    var j: u32 = i;
    while (j < N) : (j += 1) {
        mem.swap(u32, &P[i], &P[j]);
        if (i == 0 or mat[P[i-1]][P[i]] == false) dfs(i + 1, sum + A[P[i]][i]);
        mem.swap(u32, &P[i], &P[j]);
    }
}
