...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var xy: [1<<17]struct {i32, i32} = undefined;
    var xmax: i32 = math.minInt(i32);
    var ymax = xmax;
    var xmin: i32 = math.maxInt(i32);
    var ymin = xmin;
    for (0..n) |i| {
        const x = readInt(i32);
        const y = readInt(i32);
    ...
    for (0..q) |_| {
        const i = readInt(u32) - 1;
        const x: i64, const y: i64 = xy[i];
        print("{d}\n", .{@max(@max(xmax - x, x - xmin), @max(ymax - y, y - ymin))});
    }
}
