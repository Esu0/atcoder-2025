...
const Point = struct {
    ...
    pub fn arg(p: Point) f80 {
        return math.radiansToDegrees(math.atan2(@as(f64, @floatFromInt(p.x)), @as(f64, @floatFromInt(p.y))));
    }
};

pub fn solve() !void {
    const n = readInt(u32);
    var xy = try allocator.alloc(Point, n);
    for (0..n) |i| xy[i] = .{ .x = readInt(u31), .y = readInt(u31) };
    var buf = std.ArrayList(f80).initBuffer(&.{});
    var ans: f80 = 0;
    for (0.., xy) |i, p| {
        buf.clearRetainingCapacity();
        for (0..n) |j| {
            if (i != j) try buf.append(allocator, xy[j].diff(p).arg());
        }
        mem.sort(f80, buf.items, {}, std.sort.asc(f80));
        // std.debug.print("{d}, {any}\n", .{i, buf.items});
        assert(buf.items.len == n - 1);
        for (0..n-1) |j| try buf.append(allocator, buf.items[j] + 360);

        var j: u32 = 0;
        for (buf.items) |a| {
            while (j < buf.items.len and buf.items[j] - a <= 180) j += 1;
            ans = @max(ans, buf.items[j-1] - a);
        }
    }
    print("{d:.20}\n", .{ans});
}
