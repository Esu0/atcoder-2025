...
pub fn solve() !void {
    const n = readInt(u32);
    var imos: [1001][1001]i32 = @splat(@splat(0));
    for (0..n) |_| {
        const lx = readInt(u32);
        const ly = readInt(u32);
    ...
    for (0..1000) |i| {
        for (0..1001) |j| imos[i + 1][j] += imos[i][j];
    }
    for (0..1001) |i| {
        for (0..1000) |j| imos[i][j+1] += imos[i][j];
    }
    var ans: [1<<17]u32 = @splat(0);
    for (0..1000) |i| {
        for (0..1000) |j| {
            ans[@intCast(imos[i][j])] += 1;
        }
    }
    for (1..n+1) |i| {
        print("{d}\n", .{ans[i]});
