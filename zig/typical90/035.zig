...
const SEGSIZE = 1<<18;
var seg: [SEGSIZE*2]u32 = @splat(math.maxInt(u32));
var time: u32 = 0;
var F: [1<<17]u32 = @splat(math.maxInt(u32));
var L: [1<<17]u32 = undefined;
var dep: [1<<17]u32 = undefined;

fn dfs(u: u32, depth: u32) void {
    F[u] = time;
    dep[u] = depth;
    seg[SEGSIZE+time] = depth;
    time += 1;
    for (G[u].items) |v| {
        if (F[v] != math.maxInt(u32)) continue;
        dfs(v, depth + 1);
        seg[SEGSIZE+time] = depth;
        time += 1;
    }
...
pub fn solve() !void {
    ...
    {var i: u32 = SEGSIZE-1; while (i > 0) : (i -= 1) {
    ...
    for (0..q) |_| {
        const k = readInt(u32);
        var vs: [1<<17]u32 = undefined;
        for (0..k) |i| vs[i] = readInt(u32) - 1;
        mem.sortUnstable(u32, vs[0..k], {}, struct { fn lessThan(_: void, lhs: u32, rhs: u32) bool {
            return F[lhs] < F[rhs];
        }}.lessThan);
        // std.debug.print("{any}\n", .{vs[0..k]});
        var ans: u32 = 0;
        ...
        while (i < k - 1) : (i += 1) {
            const v1 = vs[i];
            const v2 = vs[i+1];
            const lca_d = segGetMin(F[v1], @max(L[v1], L[v2]));
            const dist = dep[v1] - lca_d + (dep[v2] - lca_d);
            // std.debug.print("lca depth: {d}\n", .{lca_d});
