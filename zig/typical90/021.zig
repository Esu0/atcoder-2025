...
const G = struct {
    var n: u32 = undefined;
    var g: [1<<17]std.ArrayList(u32) = undefined;
    var ord: [1<<17]u32 = @splat(math.maxInt(u32));
    var low: [1<<17]u32 = @splat(math.maxInt(u32));
    var buf1: [1<<17]u32 = undefined;
    var buf2: [1<<17]u32 = undefined;
    var stack: std.ArrayList(u32) = undefined;
    var scc_cnt: std.ArrayList(u32) = undefined;
    var cur_id: u32 = 0;
    ...
    fn init() !void {
        ...
        for (0..m) |_| {
            ...
            try g[a].append(allocator, b);
        }
        stack = .initBuffer(&buf1);
        scc_cnt = .initBuffer(&buf2);
    }

    fn dfs(u: u32) void {
        stack.appendAssumeCapacity(u);
        ord[u] = cur_id;
        low[u] = cur_id;
        cur_id += 1;
        for (g[u].items) |v| {
            if (ord[v] == math.maxInt(u32)) {
                dfs(v);
        ...
        if (ord[u] == low[u]) {
            ...
            while (true) {
                const k = stack.pop().?;
                ord[k] = math.maxInt(u32) - 1;
                cnt += 1;
                if (u == k) break;
            }
            scc_cnt.appendAssumeCapacity(cnt);
        }
    }
...
pub fn solve() !void {
    try G.init();
    for (0..G.n) |i| {
        if (G.ord[i] == math.maxInt(u32)) G.dfs(@intCast(i));
    }
    print("{d}\n", .{G.getAns()});
}
...
pub fn ModIntEx(modulo: comptime_int, comptime modulo_is_prime: bool) type {
    ...
    return struct {
        ...
        pub const Combination = struct {
            ...
            pub fn init(max_n: usize, gpa: mem.Allocator) mem.Allocator.Error!Combination {
                const fact = try gpa.alloc(Self, max_n + 1);
                errdefer gpa.free(fact);
                const fact_i = try gpa.alloc(Self, max_n + 1);
                fact[0] = .one;
                for (1..max_n + 1) |i| {
                    fact[i] = Self.init(i).mul(fact[i - 1]);
                }
                fact_i[max_n] = fact[max_n].inv();
                var i = max_n;
                while (i > 0) : (i -= 1) {
            ...
            pub fn combi(self: Combination, num: usize, k: usize) Self {
                return if (k > num) .zero else self.factorial[num].mul(self.factorial_inv[k]).mul(self.factorial_inv[num - k]);
            }
        };
    };
}