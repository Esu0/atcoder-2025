...
pub fn solve() !void {
    ...
    var queue: std.PriorityQueue(struct {u32, u32}, void, struct {fn compare(_: void, lhs: struct {u32, u32}, rhs: struct {u32, u32}) math.Order {
        return math.order(lhs[0], rhs[0]);
    }}.compare) = .init(allocator, {});
    var dist: [1<<17]u32 = @splat(math.maxInt(u32));
    try queue.add(.{0, 0});
    while (queue.removeOrNull()) |du| {
        const d, const u = du;
        if (dist[u] != math.maxInt(u32)) continue;
        dist[u] = d;
        for (g[u].items) |vw| {
            const v, const w = vw;
            try queue.add(.{d + w, v});
    ...
    var dist2: [1<<17]u32 = @splat(math.maxInt(u32));
    try queue.add(.{0, n - 1});
    while (queue.removeOrNull()) |du| {
        const d, const u = du;
        if (dist2[u] != math.maxInt(u32)) continue;
        dist2[u] = d;
        for (g[u].items) |vw| {
            const v, const w = vw;
            try queue.add(.{d + w, v});
