...
pub fn solve() !void {
    ...
    const n = readInt(u32);
    const OFF = 1<<19;
    var seg: [OFF+OFF]u32 = @splat(0);
    var lazy: [OFF+OFF]u32 = @splat(math.maxInt(u32));
    const log = 19;
    for (0..n) |_| {
        const l = readInt(u32) - 1 + OFF;
        const r = readInt(u32) + OFF;
        {var i: u5 = log; while (((l >> i) << i) != l) {
            const idx = l >> i;
            if (lazy[idx] != math.maxInt(u32)) {
                lazy[idx*2] = lazy[idx];
                lazy[idx*2+1] = lazy[idx];
                seg[idx*2] = lazy[idx];
                seg[idx*2+1] = lazy[idx];
                lazy[idx] = math.maxInt(u32);
            }
            i -= 1;
        }}
        {var i: u5 = log; while (((r >> i) << i) != r) {
            const idx = (r - 1) >> i;
            if (lazy[idx] != math.maxInt(u32)) {
                lazy[idx*2] = lazy[idx];
                lazy[idx*2+1] = lazy[idx];
