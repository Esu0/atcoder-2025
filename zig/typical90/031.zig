...
pub fn solve() !void {
    const n = readInt(u32);
    const MAX_BLUE = 50*51 / 2 + 50;
    // white, blue
    var grundy: [51][MAX_BLUE+1]u32 = undefined;
    grundy[0][0] = 0;
    grundy[0][1] = 0;
    // white = 0
    var set: [MAX_BLUE/2+5]bool = undefined;
    for (2..MAX_BLUE+1) |b| {
        @memset(&set, false);
        for (1..b / 2 + 1) |j| {
            set[grundy[0][b - j]] = true;
        }
        var mex: u32 = 0;
        while (set[mex]) mex += 1;
        grundy[0][b] = mex;
    ...
    for (1..51) |w| {
        for (0..MAX_BLUE+1) |b| {
            @memset(&set, false);
            for (1..b/2+1) |j| {
                set[grundy[w][b-j]] = true;
            }
            if (b + w <= MAX_BLUE) {
                set[grundy[w - 1][b + w]] = true;
            }
            var mex: u32 = 0;
            while (set[mex]) mex += 1;
            grundy[w][b] = mex;
    ...
    var w: [1<<17]u8 = undefined;
    var b: [1<<17]u8 = undefined;
    for (0..n) |i| w[i] = readInt(u8);
    for (0..n) |i| b[i] = readInt(u8);
    var g: u32 = 0;
    for (0..n) |i| g ^= grundy[w[i]][b[i]];
    try stdout.writeAll(if (g == 0) "Second\n" else "First\n");
}
