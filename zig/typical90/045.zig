...
pub fn solve() !void {
    const n = readInt(u5);
    const k = readInt(u5);
    var xy: [15]struct {u32, u32} = undefined;
    for (0..n) |i| xy[i] = .{ readInt(u32), readInt(u32) };
    var dist: [15][15]u64 = undefined;
    for (0..n) |i| {
        for (0..n) |j| {
            const dx = absDiff(xy[i][0], xy[j][0]);
            const dy = absDiff(xy[i][1], xy[j][1]);
            dist[i][j] = @as(u64, dx) * dx + @as(u64, dy) * dy;
        }
    }

    var dp: [1<<15][16]u64 = undefined;
    dp[0][0] = 0;
    @memset(dp[0][1..], math.maxInt(u64));
    var ep: [1<<15]u64 = undefined;
    ep[0] = 0;
    {var i: u5 = 0; while (i < n) : (i += 1) {
        var s: u32 = 0;
        while (s < @as(u32, 1) << i) : (s += 1) {
            var j: u5 = 0;
            var max: u64 = ep[s];
            while (j < i) : (j += 1) {
                if ((s >> j) & 1 != 0) max = @max(dist[i][j], max);
            }
            ep[s | (@as(u32, 1) << i)] = max;
        }
    }}
    ...
    {
        var s: u32 = 1;
        while (s < @as(u32, 1) << n) : (s += 1) {
            var t = s;
            @memset(dp[s][0..k+1], math.maxInt(u64));
            while (t > 0) : (t = (t - 1) & s) {
                var j: u5 = 0; while (j < k) : (j += 1) {
                    dp[s][j + 1] = @min(dp[s][j + 1], @max(dp[s ^ t][j], ep[t]));
                }
            }
        }
    }
