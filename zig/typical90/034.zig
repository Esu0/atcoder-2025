...
pub fn solve() !void {
    ...
    var i: u32 = 0;
    while (i < n) : (i += 1) {
        while (j < n and set.count() <= k) {
            const entry = try set.getOrPutValue(a[j], 0);
            entry.value_ptr.* += 1;
            j += 1;
        }
        ans = @max(ans, if (set.count() <= k) j - i else j - 1 - i);
        const entry = set.getEntry(a[i]).?;
        entry.value_ptr.* -= 1;
        if (entry.value_ptr.* == 0) set.removeByPtr(entry.key_ptr);
    }
    // if (ans > n) @panic("");
    print("{d}\n", .{ans});
}
