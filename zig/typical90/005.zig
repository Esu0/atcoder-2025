...
pub fn solve() !void {
    ...
    const b = readInt(u32);
    const k = readInt(u8);
    var c: [9]u8 = undefined;
    for (0..k) |i| c[i] = readInt(u8);

    var ans: [1000]MInt = @splat(.zero);
    ans[0] = .one;
    var base: [1000]MInt = @splat(.zero);
    var buf: [1000]MInt = undefined;
    for (0..k) |i| base[c[i] % b] = base[c[i] % b].add(.one);
    var cur = n;
    var p10 = 10 % b;
    while (cur > 0) : (cur >>= 1) {
        if (cur % 2 != 0) {
            ...
            for (0..b) |i| {
                for (0..b) |j| {
                    buf[(i * p10 + j) % b] = buf[(i * p10 + j) % b].add(ans[i].mul(base[j]));
                }
            }
            @memcpy(ans[0..b], buf[0..b]);
        }
        @memset(buf[0..b], .zero);
        for (0..b) |i| {
            for (0..b) |j| {
                buf[(i*p10+j)%b] = buf[(i*p10+j)%b].add(base[i].mul(base[j]));
            }
        }
        @memcpy(base[0..b], buf[0..b]);
        p10 = p10 * p10 % b;
        // std.debug.print("{any}\n", .{base[0..b]});
    }
    print("{d}\n", .{ans[0].value});
