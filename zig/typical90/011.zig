...
pub fn solve() !void {
    const n = readInt(u32);
    var dcs: [5000]struct {u32, u32, u32} = undefined;
    for (0..n) |i| dcs[i] = .{ readInt(u32), readInt(u32), readInt(u32) };
    mem.sort(struct {u32, u32, u32}, dcs[0..n], {}, struct { fn lessThan(_: void, lhs: struct {u32, u32, u32}, rhs: struct {u32, u32, u32}) bool {
        return lhs[0] < rhs[0];
    }}.lessThan);
    var dp: [5001]u64 = @splat(0);
    for (dcs[0..n]) |dcsi| {
        const d, const c, const s = dcsi;
        var ndp = dp;
        var i: u32 = 0;
        while (i <= 5000) : (i += 1) {
            if (i + c <= d) ndp[i + c] = @max(ndp[i+c], dp[i] + s);
        }
        dp = ndp;
    }
    print("{d}\n", .{mem.max(u64, &dp)});
