...
var N: u32 = undefined;
var A: [100]std.ArrayList(u32) = undefined;
var X: u64 = undefined;
var ans: u32 = 0;
fn dfs(i: u32, prod: u64) void {
    if (i == N) {
        ans += @intFromBool(prod == X);
        return;
    }
...
pub fn solve() !void {
    ...
    for (0..N) |i| {
        const l = readInt(u32);
        A[i] = try .initCapacity(allocator, l);
        for (0..l) |_| A[i].appendAssumeCapacity(readInt(u32));
    }
    dfs(0, 1);
    print("{d}\n", .{ans});
}
