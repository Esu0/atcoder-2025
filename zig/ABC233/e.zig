...
pub fn solve() !void {
    const x = readString();
    var ans: std.ArrayList(u32) = try .initCapacity(allocator, x.len + 100);
    var sum: u32 = 0;
    for (x) |xi| {
        sum += xi - '0';
        ans.appendAssumeCapacity(sum);
    }
    mem.reverse(u32, ans.items);
    var i: u32 = 0;
    var carry: u32 = 0;
    while (i < ans.items.len) : (i += 1) {
        const s = ans.items[i] + carry;
        carry = s / 10;
        ans.items[i] = s % 10;
    }
    while (carry > 0) : (carry /= 10) {
        ans.appendAssumeCapacity(carry % 10);
    }
    while (ans.pop()) |d| try stdout.writeByte(@intCast(d + '0'));
