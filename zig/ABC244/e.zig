...
pub fn solve() !void {
    ...
    const t = readInt(u32);
    const x = readInt(u32);
    var g: [2001]std.ArrayList(u32) = undefined;
    ...
    var dp: [2001][2]MInt = @splat(@splat(.zero));
    var ndp: [2001][2]MInt = undefined;
    dp[s][@intFromBool(s == x)] = .one;
    for (0..k) |_| {
        var u: u32 = 1;
        while (u <= n) : (u += 1) {
            var v0: MInt = .zero;
