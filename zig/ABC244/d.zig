...
pub fn solve() !void {
    const s: [3]u8 = .{ readChar(), readChar(), readChar() };
    var t: [3]u8 = .{ readChar(), readChar(), readChar() };
    var xoshiro = std.Random.DefaultPrng.init(@bitCast(std.time.timestamp()));
    const rng = xoshiro.random();
    const pairs: [3]struct {u8, u8} = .{ .{ 0, 1 }, .{ 0, 2 }, .{ 1, 2 } };
    var count: u32 = 0;
    while (!mem.eql(u8, &s, &t)) {
        const k = rng.uintLessThan(u8, 3);
        const i, const j = pairs[k];
        mem.swap(u8, &t[i], &t[j]);
        count += 1;
    }
    try stdout.writeAll(if (count % 2 == 0) "Yes\n" else "No\n");
}
