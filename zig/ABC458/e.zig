...
pub fn solve() !void {
    const x1 = readInt(u32);
    const x2 = readInt(u32);
    const x3 = readInt(u32);
    const C = try MInt.Combination.init(x1 + x2 + x3, allocator);
    var ans: MInt = .zero;
    var k: u32 = 2;
    while (k <= x2 + 1) : (k += 1) {
        var t: MInt = undefined;
        if (k % 2 == 0) {
            t = C.combi(x1 - 1, k / 2 - 1).mul(C.combi(x3 - 1, k / 2 - 1));
            t = t.add(t);
        } else {
            const i = k / 2;
            t = C.combi(x1 - 1, i - 1).mul(C.combi(x3 - 1, i));
            t = t.add(C.combi(x1 - 1, i).mul(C.combi(x3 - 1, i - 1)));
        }
        ans = ans.add(t.mul(C.combi(x1 + x2 + x3 - (k - 1), x1 + x3)));
    }
    print("{d}\n", .{ans.value});
}
