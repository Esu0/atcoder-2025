...
pub fn solve() !void {
    var x = readInt(u32);
    const q = readInt(u32);
    var lq: std.PriorityQueue(u32, void, struct {fn compare(_: void, lhs: u32, rhs: u32) math.Order {
        return math.order(lhs, rhs).invert();
    }}.compare) = .init(allocator, {});
    var gq: std.PriorityQueue(u32, void, struct {fn compare(_: void, lhs: u32, rhs: u32) math.Order {
        return math.order(lhs, rhs);
    }}.compare) = .init(allocator, {});
    for (0..q) |_| {
        const a = readInt(u32);
        const b = readInt(u32);
        try (if (a < x) lq.add(a) else gq.add(a));
        try (if (b < x) lq.add(b) else gq.add(b));
        if (lq.count() > gq.count()) {
            try gq.add(x);
            x = lq.remove();
        } else if (lq.count() < gq.count()) {
            try lq.add(x);
            x = gq.remove();
        }
        assert(lq.count() == gq.count());
        assert(lq.peek().? <= x and gq.peek().? >= x);
        print("{d}\n", .{x});
    }
}
