...
pub fn solve() !void {
    const n = readInt(u32);
    var a: [2<<17]u1 = undefined;
    for (1..n+1) |i| a[i] = @intCast(readChar() - '0');
    var ans: [2<<17]u1 = undefined;
    var i: u32 = n;
    while (i > 0) : (i -= 1) {
        var j: u32 = i + i;
        var xor: u1 = a[i];
    ...
    var s: std.ArrayList(u32) = try .initCapacity(allocator, n);
    i = 1;
    while (i <= n) : (i += 1) {
        if (ans[i] == 1) s.appendAssumeCapacity(i);
    }
    print("{d}\n", .{s.items.len});
    for (s.items) |b| print("{d} ", .{b});
    print("\n", .{});
}
