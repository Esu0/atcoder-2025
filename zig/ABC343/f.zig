...
const Item = struct {
    ...
    fn merge(lhs: Item, rhs: Item) Item {
        var res: Item = undefined;
        var second: struct {struct {u32, u32}, struct {u32, u32}} = undefined;

        switch (math.order(lhs.fval, rhs.fval)) {
            .lt => {
                res.fval = rhs.fval;
                res.fcnt = rhs.fcnt;
                second = .{.{lhs.fval, lhs.fcnt}, .{rhs.sval, rhs.scnt}};
            },
            .eq => {
                res.fval = rhs.fval;
                res.fcnt = lhs.fcnt + rhs.fcnt;
                second = .{.{lhs.sval, lhs.scnt}, .{rhs.sval, rhs.scnt}};
            },
            .gt => {
                res.fval = lhs.fval;
                res.fcnt = lhs.fcnt;
                second = .{.{lhs.sval, lhs.scnt}, .{rhs.fval, rhs.fcnt}};
            },
        }
        switch (math.order(second[0][0], second[1][0])) {
            .lt =>{
                res.sval, res.scnt = second[1];
            },
            .eq => {
                res.sval = second[0][0];
                res.scnt = second[0][1] + second[1][1];
            },
            .gt => {
                res.sval, res.scnt = second[0];
            },
        }
        return res;
    }
...
pub fn solve() !void {
    ...
    var seg: [1<<19]Item = undefined;
    const OFF = 1<<18;
    for (0..n) |i| {
        const a = readInt(u32);
        seg[i+OFF] = .{ .fval = a, .fcnt = 1 };
    }
    @memset(seg[n+OFF..], .{ .fval = 0, .fcnt = 0 });
    {var i: u32 = OFF - 1; while (i > 0) : (i -= 1) {
        seg[i] = seg[i*2].merge(seg[i*2+1]);
    }}

    for (0..q) |_| {
        const t = readInt(u8);
        if (t == 1) {
            var p = readInt(u32) - 1 + OFF;
            const x = readInt(u32);
            seg[p].fval = x;
            while (p > 1) {
                p /= 2;
                seg[p] = seg[p*2].merge(seg[p*2+1]);
            }
        } else {
            assert(t == 2);
            var l = readInt(u32) - 1 + OFF;
            var r = readInt(u32) + OFF;
            var ans: Item = .{ .fval = 0, .fcnt = 0 };
            while (r - l > 0) {
                if (l & 1 != 0) {
                    ans = ans.merge(seg[l]);
                    l += 1;
