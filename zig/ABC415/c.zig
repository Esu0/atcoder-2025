...
pub fn solve() !void {
    ...
    for (0..t) |_| {
        const n = readInt(u5);
        const s = readString();
        assert(s.len == (@as(u32, 1) << n) - 1);
        var vis: [1<<18]bool = undefined;
        @memset(vis[0..@as(u32, 1) << n], false);
        var stack: std.ArrayList(u32) = try .initCapacity(allocator, @as(u32,1)<<n);
        ...
        while (stack.pop()) |u| {
            ...
            while (i < n) : (i += 1) {
                const m = @as(u32, 1) << i;
                const v = u | m;
                if (!vis[v] and s[v-1] == '0') {
                    stack.appendAssumeCapacity(v);
                    vis[v] = true;
                }
            }
        }
        try stdout.writeAll(if (vis[(@as(u32, 1) << n) - 1]) "Yes\n" else "No\n");
    }
}
