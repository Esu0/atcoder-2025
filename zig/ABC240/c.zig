...
pub fn solve() !void {
    const n = readInt(u32);
    const x = readInt(u32);
    var dp = try allocator.alloc(bool, x + 1);
    var ndp = try allocator.alloc(bool, x + 1);
    @memset(dp, false);
    dp[0] = true;
    for (0..n) |_| {
        const a = readInt(u32);
        ...
        var i: u32 = 0;
        while (i <= x) : (i += 1) {
            if (i >= a and dp[i - a]) ndp[i] = true;
            if (i >= b and dp[i - b]) ndp[i] = true;
        }
        @memcpy(dp, ndp);
    }
    try stdout.writeAll(if (dp[x]) "Yes\n" else "No\n");
}
