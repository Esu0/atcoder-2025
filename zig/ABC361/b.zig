...
fn intersect(r1: struct {u32, u32}, r2: struct {u32, u32}) bool {
    return !(r1[1] <= r2[0] or r2[1] <= r1[0]);
}

pub fn solve() !void {
    var a: [3]struct {u32, u32} = undefined;
    var b: [3]struct {u32, u32} = undefined;
    for (0..3) |i| a[i][0] = readInt(u32);
    for (0..3) |i| a[i][1] = readInt(u32);
    for (0..3) |i| b[i][0] = readInt(u32);
    for (0..3) |i| b[i][1] = readInt(u32);
    try stdout.writeAll(if (intersect(a[0], b[0]) and intersect(a[1], b[1]) and intersect(a[2], b[2])) "Yes\n" else "No\n");
}
