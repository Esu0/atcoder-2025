...
pub fn solve() !void {
    var pcg = std.Random.Pcg.init(@bitCast(std.time.timestamp()));
    const rng = pcg.random();
    const n = readInt(u32);
    const q = readInt(u32);
    var hash: [2<<17]u64 = undefined;
    for (0..2<<17) |i| hash[i] = rng.int(u64);
    // std.debug.print("{any}\n", .{hash[0..20]});
    ...
    for (0..n) |i| b[i + 1] +%= b[i];
    for (0..q) |_| {
        const l1 = readInt(u32) - 1;
        const r1 = readInt(u32);
        const l2 = readInt(u32) - 1;
        const r2 = readInt(u32);
        try stdout.writeAll(if (r1 - l1 == r2 - l2 and a[r1] -% a[l1] == b[r2] -% b[l2]) "Yes\n" else "No\n");
    }
}
