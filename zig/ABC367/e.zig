...
pub fn solve() !void {
    const n = readInt(u32);
    var k = readInt(u64);
    var x: [2<<17]u32 = undefined;
    var a: [2<<17]u32 = undefined;
    for (1..n+1) |i| x[i] = readInt(u32);
    for (1..n+1) |i| a[i] = readInt(u32);
    var buf: [2<<17]u32 = undefined;
    while (k > 0) : (k >>= 1) {
        if (k & 1 != 0) {
            for (1..n+1) |i| buf[i] = a[x[i]];
            @memcpy(a[1..n+1], buf[1..n+1]);
        }
        for (1..n+1) |i| buf[i] = x[x[i]];
        @memcpy(x[1..n+1], buf[1..n+1]);
    }
    for (1..n+1) |i| print("{d} ", .{a[i]});
