...
pub fn solve() !void {
    ...
    a[0] = 0;
    for (0..n) |i| a[i + 1] = readInt(u32) + a[i];
    var b: [2<<17]u64 = undefined;
    b[0] = 0;
    for (0..m) |i| b[i + 1] = readInt(u32) + b[i];
    var ans: u32 = 0;
    var i: u32 = 0;
    while (i <= n) : (i += 1) {
        const t = a[i];
        const j: u32 = @intCast(std.sort.partitionPoint(u64, b[0..m+1], .{t, k}, struct {fn pred(ctx: struct {u64, u32}, x: u64) bool {
