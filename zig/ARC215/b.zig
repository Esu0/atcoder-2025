...
pub fn solve() !void {
    const t = readInt(u32);
    var ans: std.ArrayList(u32) = try .initCapacity(allocator, 2<<17);
    for (0..t) |_| {
        const n = readInt(u32);
        var set1: [2<<17]bool = undefined;
        var set2: [2<<17]bool = undefined;
        @memset(set1[0..n], false);
        @memset(set2[0..n], false);
        var i: u32 = 0;
        var f = false;
        ans.clearRetainingCapacity();
        while (i < n+n) : (i += 1) {
            const a = readInt(u32) - 1;
            var set = if (f) &set1 else &set2;
            if (set[a]) {
                f ^= true;
                set = if (f) &set1 else &set2;
                assert(set[a] == false);
                set[a] = true;
        ...
        if (ans.items.len > n) @panic("");
        print("{d}\n", .{ans.items.len});
        for (ans.items) |x| print("{d} ", .{x});
