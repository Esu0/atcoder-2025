...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    const C = try MInt.Combination.init(n + m, allocator);
    const l = (n + 1) / 2;
    const ans = C.combi(m + 1 + l - 2, l - 1).mul(C.combi(m + 1 + n + 1 - l - 1, n + 1 - l));
    var i: u32 = 1;
    var s: MInt = .zero;
    while (i <= m) : (i += 1) {
        s = s.add(C.combi(m + 1 - i + l - 1 - 1, l - 1));
    }
    print("{d}\n", .{ans.add(s.mul(C.combi(m + 1 + n + 1 - l - 1 - 1, n + 1 - l - 1))).value});
}
