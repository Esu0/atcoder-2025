...
var c: [3][3]u4 = undefined;
var count: u32 = 0;
var open_count: u4 = 0;
var opened: [3][3]bool = @splat(@splat(false));
...
fn dfs() void {
    ...
    while (i < 3) : (i += 1) {
        ...
        while (j < 3) : (j += 1) {
            ...
            const j1 = (j + 1) % 3;
            const j2 = (j + 2) % 3;
            if (opened[i_1][j] and opened[i_2][j] and c[i_1][j] == c[i_2][j])return;
            if (opened[i][j1] and opened[i][j2] and c[i][j1] == c[i][j2]) return;
            if (i==j and opened[i_1][j1] and opened[i_2][j2] and c[i_1][j1] == c[i_2][j2]) return;
            if (i+j==2 and opened[i_1][j2] and opened[i_2][j1] and c[i_1][j2] == c[i_2][j1]) return;
            opened[i][j] = true;
            open_count += 1;
            dfs();
            opened[i][j] = false;
...
pub fn solve() !void {
    ...
    const all_cnt: comptime_float = 9*8*7*6*5*4*3*2;
    dfs();
    const ans = @as(f64, @floatFromInt(count)) / all_cnt;
    print("{d:.15}\n", .{ans});
}
