...
pub fn solve() !void {
    const n = readInt(u32);
    var st: [100]struct {[]u8, []u8} = undefined;
    var set: std.StringHashMap(u32) = .init(allocator);
    for (0..n) |i| {
        st[i] = .{ readString(), readString() };
        (try set.getOrPutValue(st[i][0], 0)).value_ptr.* += 1;
        (try set.getOrPutValue(st[i][1], 0)).value_ptr.* += 1;
    }
    for (0..n) |i| {
        const si, const ti = st[i];
        if (mem.eql(u8, si, ti)) {
            if (set.get(si).? > 2) {
            ...
            if (set.get(si).? > 1 and set.get(ti).? > 1) {
                try stdout.writeAll("No\n");
                return;
            }
        }
