...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var stack = try std.ArrayList(struct {i32, i32}).initCapacity(allocator, q+1);
    ...
    for (0..q) |_| {
        ...
        if (t == 1) {
            ...
            stack.appendAssumeCapacity(last);
        } else {
            assert(t == 2);
            const p = readInt(u32);
            assert(1 <= p and p <= n);
            if (p <= stack.items.len) {
                print("{d} {d}\n", stack.items[stack.items.len - p]);
            } else {
                print("{d} {d}\n", .{p - (stack.items.len - 1), 0});
            }
        }
    }
}
