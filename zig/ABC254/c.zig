...
pub fn solve() !void {
    const n = readInt(u32);
    const k = readInt(u32);
    var arrs: [2<<17]std.ArrayList(u32) = undefined;
    for (0..k) |i| arrs[i] = try .initCapacity(allocator, try math.divCeil(u32, n, k));
    for (0..n) |i| {
        arrs[i % k].appendAssumeCapacity(readInt(u32));
    }
    var a: [2<<17]u32 = undefined;
    for (0..k) |i| {
        mem.sortUnstable(u32, arrs[i].items, {}, std.sort.asc(u32));
        var j = i;
        for (arrs[i].items) |elem| {
            a[j] = elem;
            j += k;
        }
    }
    // std.debug.print("{any}\n", .{a[0..n]});
    try stdout.writeAll(if (std.sort.isSorted(u32, a[0..n], {}, std.sort.asc(u32))) "Yes\n" else "No\n");
}
