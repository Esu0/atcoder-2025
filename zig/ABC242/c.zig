...
pub fn solve() !void {
    ...
    var n = readInt(u32) - 1;
    var mat: [9][9]MInt = @splat(@splat(.zero));
    for (0..9) |i| {
        for (i-|1..@min(i + 2, 9)) |j| mat[i][j] = .one;
    }
    var buf: [9][9]MInt = undefined;
    var res: [9][9]MInt = @splat(@splat(.zero));
    for (0..9) |i| res[i][i] = .one;
    while (n > 0) : (n >>= 1) {
        if (n % 2 != 0) {
