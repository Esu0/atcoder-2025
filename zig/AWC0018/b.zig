...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var c: [1<<17]u32 = undefined;
    for (0..m) |i| c[i] = readInt(u32);
    var cnt: [1<<17]u32 = @splat(0);
    var stack: std.ArrayList(u32) = try .initCapacity(allocator, m);
    var ans: u32 = 0;
    for (0..n) |_| {
        const k = readInt(u32);
        for (0..k) |_| {
            const p = readInt(u32) - 1;
            if (cnt[p] == 0) stack.appendAssumeCapacity(p);
            cnt[p] += 1;
        }
        while (stack.pop()) |p| {
            if (cnt[p] <= c[p]) ans += cnt[p];
            cnt[p] = 0;
        }
    }
    print("{d}\n", .{ans});
