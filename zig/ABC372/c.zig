...
pub fn solve() !void {
    ...
    const q = readInt(u32);
    const s0 = readString();
    const s = try allocator.alloc(u8, n + 4);
    s[0] = 0;
    s[1] = 0;
    s[n + 2] = 0;
    s[n + 3] = 0;
    ...
    var ans: u32 = 0;
    {var i: u32 = 0; while (i < s.len - 2) : (i += 1) {
        if (s[i] == 'A' and s[i + 1] == 'B' and s[i + 2] == 'C') ans += 1;
    }}
    for (0..q) |_| {
        const x = readInt(u32) - 1 + 2;
        const c = readChar();
        if (s[x] != c) {
            if (switch (s[x]) {
                'A' => s[x + 1] == 'B' and s[x + 2] == 'C',
                'B' => s[x - 1] == 'A' and s[x + 1] == 'C',
                'C' => s[x - 2] == 'A' and s[x - 1] == 'B',
                else => false,
            }) ans -= 1;
            if (switch (c) {
                'A' => s[x + 1] == 'B' and s[x + 2] == 'C',
