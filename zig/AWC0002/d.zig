...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var c: [2<<17]u32 = undefined;
    var r: [2<<17]u32 = undefined;
    for (0..n) |i| c[i] = readInt(u32);
    for (0..m) |i| r[i] = readInt(u32);
    mem.sortUnstable(u32, c[0..n], {}, std.sort.asc(u32));
    mem.sortUnstable(u32, r[0..m], {}, std.sort.asc(u32));
    var ok: u32 = 0;
    var ng: u32 = @min(n, m) + 1;
    next: while (ng - ok > 1) {
        const mid = (ok + ng) / 2;
        const off = m - mid;
        for (0..mid) |i| {
