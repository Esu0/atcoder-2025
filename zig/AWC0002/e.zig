...
pub fn solve() !void {
    const n = readInt(u6);
    const s = readInt(u64);
    var a: [40]u64 = undefined;
    for (0..n) |i| a[i] = readInt(u64);
    var x: std.AutoHashMap(u64, u32) = .init(allocator);
    var y: std.AutoHashMap(u64, u32) = .init(allocator);
    for (0..@as(u64, 1) << (n / 2)) |set| {
        var sum: u64 = 0;
        var i: u5 = 0;
        while (i < n/2) : (i += 1) {
            if ((set >> i) & 1 != 0) sum += a[i];
        }
        const result = try x.getOrPutValue(sum, 0);
        result.value_ptr.* += 1;
    }
    for (0..@as(u64, 1) << (n - n / 2)) |set| {
        var sum: u64 = 0;
        var i: u5 = 0;
        while (i < n - n / 2) : (i += 1) {
            if ((set >> i) & 1 != 0) sum += a[@as(usize, i) + n / 2];
        }
        (try y.getOrPutValue(sum, 0)).value_ptr.* += 1;
    }
    var iter = x.iterator();
    var ans: u64 = 0;
    while (iter.next()) |entry| {
        ...
        if (k > s) continue;
        ans += @as(u64, v) * (y.get(s - k) orelse 0);
    }
    print("{d}\n", .{ans});
}
