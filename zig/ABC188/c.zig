...
pub fn solve() !void {
    const n = readInt(u5);
    var a: [1<<16]u32 = undefined;
    var num: [1<<16]u32 = undefined;
    for (0..@as(u32, 1) << n) |i| a[i] = readInt(u32);
    { var i: u32 = 0; while (i < @as(u32, 1) << n) : (i += 1) {
        num[i] = i;
    }}
    ...
    while (i > 1) {
        i -= 1;
        var j: u32 = 0;
        while (j < @as(u32, 1) << i) : (j += 1) {
            const num1 = num[j * 2];
            const num2 = num[j * 2 + 1];
            num[j] = if (a[num1] > a[num2]) num1 else num2;
        }
    }
    const num1 = num[0];
    const num2 = num[1];
    const ans = if (a[num1] > a[num2]) num2 else num1;
    print("{d}\n", .{ans + 1});
}
