...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var g = try allocator.alloc(std.ArrayList(struct {u32, i32}), n);
    for (0..n) |i| g[i] = .initBuffer(&.{});
    for (0..m) |_| {
        const u = readInt(u32) - 1;
        const v = readInt(u32) - 1;
        const w = readInt(i32);
        try g[u].append(allocator, .{ v, w });
        try g[v].append(allocator, .{ u, -w });
    }
    var ans = try allocator.alloc(i64, n);
    var buf: [4<<17]u32 = undefined;
    var stack = std.ArrayList(u32).initBuffer(&buf);
    @memset(ans, math.maxInt(i64));
    var u: u32 = 0;
    while (u < n) : (u += 1) {
        if (ans[u] != math.maxInt(i64)) continue;
        ...
        while (stack.pop()) |v| {
            ...
            for (g[v].items) |zw| {
                const z, const w = zw;
                if (ans[z] != math.maxInt(i64)) {
                    assert(p + w == ans[z]);
                } else {
                    ans[z] = p + w;
                    stack.appendAssumeCapacity(z);
                }
            }
        }
    }
