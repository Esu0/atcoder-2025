...
pub fn solve() !void {
    ...
    _ = readInt(u32);
    const q = readInt(u32);
    var all: [2<<17]bool = undefined;
    @memset(all[1..n+1], false);
    var pairs: std.AutoHashMap(struct {u32, u32}, void) = .init(allocator);
    try pairs.ensureTotalCapacity(q);
    for (0..q) |_| {
        const t = readInt(u4);
        const x = readInt(u32);
        if (t == 1) {
            const y = readInt(u32);
            pairs.putAssumeCapacity(.{x, y}, {});
        } else if (t == 2) {
            all[x] = true;
        } else {
            assert(t == 3);
            const y = readInt(u32);
            try stdout.writeAll(if (all[x] or pairs.get(.{x, y}) != null) "Yes\n" else "No\n");
        }
    }
}
