...
pub fn solve() !void {
    ...
    const s = readString();
    const t = readString();
    var ta: [26]bool = @splat(false);
    var ao: [26]bool = @splat(false);
    for (s) |si| ta[si-'a'] = true;
    for (t) |ti| ao[ti-'a'] = true;
    const q = readInt(u8);
    for (0..q) |_| {
        var ist = true;
        var isa = true;
        const w = readString();
        for (w) |wi| {
            if (!ta[wi-'a']) ist = false;
            if (!ao[wi-'a']) isa = false;
        }
        try stdout.writeAll(if (ist and isa) "Unknown\n" else if (ist) "Takahashi\n" else "Aoki\n");
    }
}
