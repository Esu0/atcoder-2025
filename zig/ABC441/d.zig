...
var ans: [2<<17]bool = @splat(false);
var st: struct {u32, u32} = undefined;

pub fn dfs(u: u32, rem: u8, cost: u32) void {
    if (rem == 0) {
        if (st[0] <= cost and cost <= st[1]) ans[u] = true;
        return;
    }
    for (g[u].items) |vc| {
        const v, const c = vc;
