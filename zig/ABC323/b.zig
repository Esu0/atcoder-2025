...
pub fn solve() !void {
    const n = readInt(u32);
    var cnt: [101]u32 = undefined;
    for (1..n+1) |i| {
        var c: u32 = 0;
        const s = readString();
        for (s) |si| c += @intFromBool(si == 'o');
        cnt[i] = c;
    }
    var ans: [100]u32 = undefined;
    {var i: u32 = 0; while (i < n) : (i += 1) ans[i] = i + 1;}
    mem.sort(u32, ans[0..n], &cnt, struct { fn lessThan(c: *[101]u32, lhs: u32, rhs: u32) bool {
        return c[lhs] > c[rhs];
    }}.lessThan);
    for (0..n) |i| {
        print("{d} ", .{ans[i]});
