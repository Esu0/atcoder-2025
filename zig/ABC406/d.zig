...
pub fn solve() !void {
    ...
    const w = readInt(u32);
    const n = readInt(u32);
    var row: [2<<17]std.AutoHashMap(u32, void) = undefined;
    var col: [2<<17]std.AutoHashMap(u32, void) = undefined;
    for (1..h + 1) |i| row[i] = .init(allocator);
    for (1..w + 1) |i| col[i] = .init(allocator);
    ...
    for (0..q) |_| {
        ...
        if (t == 1) {
            const x = readInt(u32);
            ans = row[x].count();
            var iter = row[x].keyIterator();
            while (iter.next()) |k| {
                assert(col[k.*].remove(x));
            }
            row[x].clearAndFree();
        } else {
            assert(t == 2);
            const y = readInt(u32);
            ans = col[y].count();
            var iter = col[y].keyIterator();
            while (iter.next()) |k| {
                assert(row[k.*].remove(y));
            }
            col[y].clearAndFree();
        }
        print("{d}\n", .{ans});
