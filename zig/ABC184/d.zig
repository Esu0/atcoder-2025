...
var memo: [101][101][101]?f64 = @splat(@splat(@splat(null)));

fn tof(x: anytype) f64 {
    return @floatFromInt(x);
}
fn dfs(a: u8, b: u8, c: u8) f64 {
    if (a == 100 or b == 100 or c == 100) return 0;
    if (memo[a][b][c]) |v| return v;
    var p: f64 = 0;
    if (a != 0) p += tof(a) * dfs(a + 1, b, c);
    if (b != 0) p += tof(b) * dfs(a, b + 1, c);
    if (c != 0) p += tof(c) * dfs(a, b, c + 1);
    p /= tof(@as(u32, a) + b + c);
    p += 1;
    memo[a][b][c] = p;
    return p;
