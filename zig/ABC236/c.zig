...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    const ans = try allocator.alloc(bool, n);
    ...
    for (0..m) |_| {
        const t = readString();
        while (!mem.eql(u8, s[i], t)) : (i += 1) ans[i] = false;
        ans[i] = true;
        i += 1;
    }
    for (0..i) |j| try stdout.writeAll(if (ans[j]) "Yes\n" else "No\n");
    while (i < n) : (i += 1) try stdout.writeAll("No\n");
}
