...
pub fn solve() !void {
    const n = readInt(u32);
    var k: u64 = readInt(u32);
    var ab: [2<<17]struct {u64, u32} = undefined;
    for (0..n) |i| ab[i] = .{ readInt(u64), readInt(u32) };
    mem.sortUnstable(struct {u64, u32}, ab[0..n], {}, struct { fn lessThan(_: void, lhs: struct {u64, u32}, rhs: struct {u64, u32}) bool {
        return lhs[0] < rhs[0];
    }}.lessThan);
    ab[n] = .{ math.maxInt(u64), 0 };
    var cur: u64 = 0;
    for (ab[0..n+1]) |abi| {
