...
pub fn solve() !void {
    ...
    for (1..n+1) |i| g[i] = .initBuffer(&.{});
    for (0..m) |_| {
        const u = readInt(u32);
        const v = readInt(u32);
        const b = readInt(u32);
        try g[u].append(allocator, .{v, b});
        try g[v].append(allocator, .{u, b});
    }

    var queue: std.PriorityQueue(struct {u64, u32}, void, struct {fn compare(_: void, lhs: struct {u64, u32}, rhs: struct {u64, u32}) math.Order {
        return math.order(lhs[0], rhs[0]);
    }}.compare) = .init(allocator, {});
    try queue.ensureTotalCapacity(m+m);
    var dist: [2<<17]u64 = @splat(math.maxInt(u64));
    try queue.add(.{ a[1], 1 });
    while (queue.removeOrNull()) |elem| {
        const d, const u = elem;
        if (dist[u] != math.maxInt(u64)) continue;
        dist[u] = d;
        for (g[u].items) |edge| {
            const v, const w = edge;
            try queue.add(.{ d + w + a[v], v});
        }
    }
    var i: u32 = 2;
    while (i <= n) : (i += 1) print("{d} ", .{dist[i]});
