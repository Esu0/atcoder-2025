...
pub fn solve() !void {
    const x1 = readInt(i32);
    const y1 = readInt(i32);
    const x2 = readInt(i32);
    const y2 = readInt(i32);
    const x3 = readInt(i32);
    const y3 = readInt(i32);
    const d1 = dist2(x1, y1, x2, y2);
    const d2 = dist2(x2, y2, x3, y3);
    const d3 = dist2(x3, y3, x1, y1);
    try stdout.writeAll(if (d1 + d2 == d3 or d1 + d3 == d2 or d2 + d3 == d1) "Yes\n" else "No\n");
}
