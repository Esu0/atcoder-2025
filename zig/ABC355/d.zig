...
pub fn solve() !void {
    const n = readInt(u32);
    var lr: [5<<17]struct {u32, u32} = undefined;
    for (0..n) |i| lr[i] = .{ readInt(u32), readInt(u32) };
    mem.sortUnstable(struct {u32, u32}, lr[0..n], {}, struct { fn lessThan(_: void, lhs: struct {u32, u32}, rhs: struct {u32, u32}) bool {
        return lhs[1] < rhs[1];
    }}.lessThan);
    ...
    for (0..n) |i| {
        const li, _ = lr[i];
        const k = std.sort.lowerBound(struct {u32, u32}, lr[0..i], li, struct {fn compare(x: u32, y: struct {u32, u32}) math.Order {
            return math.order(x, y[1]);
        }}.compare);
        ans += i - k;
    }
    print("{d}\n", .{ans});
