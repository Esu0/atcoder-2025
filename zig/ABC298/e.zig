...
var P: u32 = undefined;
var Q: u32 = undefined;
var dp: [101][101][2]?MInt = @splat(@splat(@splat(null)));
var invP: MInt = undefined;
var invQ: MInt = undefined;

fn dfs(a: u32, b: u32, takahashi: bool) MInt {
    assert(!(a >= N and b >= N));
    if (a >= N) {
        assert(!takahashi);
        return .zero;
    ...
    if (dp[a][b][@intFromBool(takahashi)]) |ans| return ans;
    var ans: MInt = .zero;
    defer dp[a][b][@intFromBool(takahashi)] = ans;
    if (takahashi) {
        var i: u32 = 1;
        while (i <= P) : (i += 1) {
            ans = ans.add(.one).sub(dfs(a + i, b, false));
        }
        ans = ans.mul(invP);
    } else {
        var i: u32 = 1;
        while (i <= Q) : (i += 1) {
            ans = ans.add(.one).sub(dfs(a, b + i, true));
        }
        ans = ans.mul(invQ);
    }
    return ans;
...
pub fn solve() !void {
    ...
    P = readInt(u32);
    Q = readInt(u32);
    invP = MInt.init(P).inv();
    invQ = MInt.init(Q).inv();
    const ans = dfs(a, b, true);
    print("{d}\n", .{ans.value});
}
