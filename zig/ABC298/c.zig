...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var box: [2<<17]std.ArrayList(u32) = undefined;
    for (1..n+1) |i| box[i] = .initBuffer(&.{});
    var sets: [2<<17]std.AutoHashMap(u32, void) = undefined;
    for (1..2<<17) |i| sets[i] = .init(allocator);
    var buf: std.ArrayList(u32) = try .initCapacity(allocator, n);
    for (0..q) |_| {
        const t = readInt(u4);
        if (t == 1) {
            const i = readInt(u32);
            const j = readInt(u32);
            try box[j].append(allocator, i);
            try sets[i].put(j, {});
        } else if (t == 2) {
            const i = readInt(u32);
            mem.sortUnstable(u32, box[i].items, {}, std.sort.asc(u32));
            for (box[i].items) |e| {
                print("{d} ", .{e});
            }
            ...
            var iter = sets[i].keyIterator();
            while (iter.next()) |e| buf.appendAssumeCapacity(e.*);
            mem.sortUnstable(u32, buf.items, {}, std.sort.asc(u32));
            for (buf.items) |e| print("{d} ", .{e});
            try stdout.writeByte('\n');
        }
    }
}
