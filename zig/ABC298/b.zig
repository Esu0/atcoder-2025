...
fn rot(a: [][101]u1) void {
    const n = a.len - 1;
    var buf: [101][101]u1 = undefined;
    for (1..n+1) |i| {
        for (1..n+1) |j| {
            buf[i][j] = a[n + 1 - j][i];
        }
    ...
    for (1..n+1) |i| {
        @memcpy(a[i][1..n+1], buf[i][1..n+1]);
    }
}

pub fn solve() !void {
    const n = readInt(u32);
    var a: [101][101]u1 = undefined;
    var b: [101][101]u1 = undefined;
    for (1..n+1) |i| {
        for (1..n+1) |j| a[i][j] = readInt(u1);
    }
    for (1..n+1) |i| {
        for (1..n+1) |j| b[i][j] = readInt(u1);
    }
