...
var N: u32 = undefined;
var S: [26]u32 = @splat(0);
var K: u32 = undefined;
var ans: u32 = 0;
var buf: [10]u8 = undefined;

fn isPalindrome(string: []u8) bool {
    for (0..string.len / 2) |i| {
        if (string[i] != string[string.len - i - 1]) return false;
    }
    return true;
}

fn dfs(i: u32) void {
    if (i >= K and isPalindrome(buf[i - K..i])) {
        return;
    }
    if (i == N) {
        ans += 1;
...
const Unionfind = struct {
    ...
    pub fn init(alloc: mem.Allocator, num: u32) !Self {
        const buffer = try alloc.alloc(u32, num);
        const buffer2 = try alloc.alloc(u32, num);
        @memset(buffer, 1);
        @memset(buffer2, std.math.maxInt(u32));
        return .{
            .size = buffer,
            .parent = buffer2,
        };
