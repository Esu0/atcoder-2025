    ...
    for (0..h) |i| {
        for (0..w) |j| {
            ...
            if (i == 0 or i == h - 1 or j == 0 or j == w - 1) {
                try queue.add(.{a[idx], @intCast(idx)});
            }
        }
    }

    const dij: [5]u32 = .{0, 1, 0, math.maxInt(u32), 0};

    var vis: [1<<20]bool = @splat(false);
    var ans = h * w;
    {var i: u32 = 1; while (i <= y) : (i += 1) {
        while (queue.peek()) |au| {
            const val, const u = au;
            if (val > i) break;
            _ = queue.remove();
            if (vis[u]) continue;
            vis[u] = true;
