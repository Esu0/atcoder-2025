...
pub fn solve() !void {
    const n = readInt(u5);
    const m = readInt(u32);
    var p: [12]u32 = undefined;
    for (0..n) |i| p[i] = readInt(u32);
    const s = readInt(u5) - 1;
    const t = readInt(u5) - 1;
    var mat: [12][12]u32 = @splat(@splat(math.maxInt(u32)));
    for (0..m) |_| {
    ...
    var dp: [12][1<<12]i64 = @splat(@splat(math.minInt(i32)));
}
