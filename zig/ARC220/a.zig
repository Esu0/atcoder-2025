...
pub fn solve() !void {
    ...
    try queue.ensureTotalCapacity(1<<17);
    for (0..t) |_| {
        var n = readInt(u32);
        ...
        const c: u8 = if (n % 3 == 0) 1 else if (n % 3 == 2) 2 else 0;
        try queue.add(1);
        n -= 1;
        for (0..c) |_| {
            n -= 5;
            const k = queue.remove();
            try queue.addSlice(&.{k * 2, k * 2, k * 2, k * 3, k * 3, k * 6});
        }
        ...
        while (n != 0) {
            n -= 3;
            const k = queue.remove();
            try queue.addSlice(&.{k * 2, k * 2, k * 2, k * 2});
        }
        var iter = queue.iterator();
        while (iter.next()) |v| print("{d} ", .{v});
        print("\n", .{});
    }
}
