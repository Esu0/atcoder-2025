...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    var buf1: [1<<17]i64 = undefined;
    var buf2: [1<<17]i32 = undefined;
    var prev: i32 = 0;
    var i: u32 = 0; while (i < n) : (i += 1) {
        const ci: i32 = readInt(u16);
        buf2[i] = ci - prev;
        buf1[i] = @as(i64, prev - ci) * i;
        prev = ci;
    }
    buf1[n] = @as(i64, prev) * n;
    buf2[n] = -prev;

    var fw1: FenwickTree(i64, struct { fn add(lhs: i64, rhs: i64) i64 {
        return lhs + rhs;
    }}.add) = .init(buf1[0..n]);
    var fw2: FenwickTree(i32, struct { fn add(lhs: i32, rhs: i32) i32 {
        return lhs + rhs;
    }}.add) = .init(buf2[0..n]);

    // for (1..n+1) |j| {
    //     std.debug.print("{d} ", .{fw1.prefixSum(j).?});
    // }
    // std.debug.print("\n", .{});
    // for (1..n+1) |j| {
    //     std.debug.print("{d} ", .{fw2.prefixSum(j).?});
    // }
    // std.debug.print("\n", .{});
    ...
    for (0..q) |_| {
        ...
        const r = readInt(u32);
        if (t == 1) {
            const v: i32 = readInt(u16);
            fw1.add(l - 1, -@as(i64, l - 1) * v);
            fw1.add(r, @as(i64, r) * v);
            fw2.add(l - 1, v);
            fw2.add(r, -v);
            // for (1..n+1) |j| {
            //     std.debug.print("{d} ", .{fw1.prefixSum(j).?});
            ...
            assert(t == 2);
            const lv: i64 = if (l == 1) 0 else (fw1.prefixSum(l - 1).? + @as(i64, l - 1) * fw2.prefixSum(l - 1).?);
            const rv = fw1.prefixSum(r).? + @as(i64, r) * fw2.prefixSum(r).?;
            print("{d}\n", .{@as(u64, @intCast(rv - lv))});
        }
    }
}
