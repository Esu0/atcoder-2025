...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const t = readInt(u32);
    var g: [1<<17]std.ArrayList(struct {u32, u32}) = undefined;
    for (1..n+1) |i| g[i] = .initBuffer(&.{});
    for (0..m) |_| {
        const a = readInt(u32);
        const b = readInt(u32);
        const c = readInt(u32);
        try g[a].append(allocator, .{b, c});
        try g[b].append(allocator, .{a, c});
    }

    var dist: [1<<17]u64 = @splat(math.maxInt(u64));
    ...
    while (queue.removeOrNull()) |elem| {
        ...
        for (g[u].items) |e| {
            const to, const c = e;
            if (dist[to] == math.maxInt(u64)) try queue.add(.{ d + c, to });
        }
    }
    print("-1\n", .{});
}
