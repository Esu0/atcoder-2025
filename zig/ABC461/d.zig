...
pub fn solve() !void {
    ...
    const w = readInt(u32);
    const k = readInt(u32);
    var bucket: [500*500+1]u32 = @splat(0);
    var stack: std.ArrayList(u32) = try .initCapacity(allocator, w);
    var ans: u64 = 0;
    var s: [500][]u8 = undefined;
    var cnt: [500]u32 = undefined;
    for (0..h) |i| {
        s[i] = readString();
        assert(s[i].len == w);
        for (0..w) |j| s[i][j] -= '0';
        ...
        var j = i;
        while (true) {
            for (0..w) |l| cnt[l] += s[j][l];
            // std.debug.print("{any}\n", .{cnt[0..w]});
            var sum: u32 = 0;
            bucket[0] = 1;
            for (0..w) |l| {
                sum += cnt[l];
                if (sum >= k) ans += bucket[sum - k];
                bucket[sum] += 1;
                stack.appendAssumeCapacity(sum);
            }
            while (stack.pop()) |x| bucket[x] = 0;
            if (j == 0) break;
            j -= 1;
        }
    }
