...
pub fn solve() !void {
    const n = readInt(u64);
    var divs: std.ArrayList(u64) = try .initCapacity(allocator, 3000);
    var map: std.AutoHashMap(u64, u32) = .init(allocator);
    try map.ensureTotalCapacity(3000);
    {
    var i: u64 = 1;
    while (i * i < n) : (i += 1) {
        if (n % i == 0) {
            const s: [2]u64 = .{i, @divExact(n, i) };
            divs.appendSliceAssumeCapacity(&s);
        }
    }
    if (i * i == n) {
        divs.appendAssumeCapacity(i);
    }
    }
    mem.sortUnstable(u64, divs.items, {}, std.sort.asc(u64));
    for (0.., divs.items) |i, di| {
        map.putAssumeCapacityNoClobber(di, @intCast(i));
    }
    var dp: [15][3000]MInt = @splat(@splat(.zero));
    var dp2: [15][3000]MInt = @splat(@splat(.zero));
    dp2[0][0] = .one;
    var ans: MInt = .zero;
    for (0.., divs.items) |i, di| {
        var l: u32 = 14;
        while (l > 0) : (l -= 1) {
            for (i.., divs.items[i..]) |j, dj| {
                if (dj % di == 0) {
                    const dk = dj / di;
                    const k = map.get(dk).?;
                    dp2[l][j] = dp2[l][j].add(dp2[l - 1][k]);
                    dp[l][j] = dp[l][j].add(dp[l - 1][k].add(dp2[l - 1][k].mul(.init(di))));
                }
            }
        }
    }
    assert(n == divs.items[divs.items.len - 1]);
    // std.debug.print("{any}\n", .{dp2[1][0..divs.items.len]});
    // std.debug.print("{any}\n", .{dp2[2][0..divs.items.len]});
    var fact: MInt = .one;
    var l: u32 = 1;
    while (l < 15) : (l += 1) {
        fact = fact.mul(.init(l));
        ans = ans.add(fact.mul(dp[l][divs.items.len - 1]));
    }
    print("{d}\n", .{ans.value});
}
