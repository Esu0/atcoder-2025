...
pub fn solve() !void {
    const n = readInt(u32);
    const q = readInt(u32);
    // var stack: std.ArrayList(u32) = .initCapacity(allocator, n);
    var lastr: [3<<17]?u32 = @splat(null);
    var lastc: [3<<17]?u32 = @splat(null);
    var ans: u64 = 0;
    var fw1_buf: [3<<17]i32 = @splat(0);
    var fw1: FenwickTree(i32, struct {fn add(lhs: i32, rhs: i32) i32 {
        return lhs + rhs;
    }}.add) = .init(&fw1_buf);

    var fw2_buf: [3<<17]i32 = @splat(0);
    ...
    // naive
    // var grid: [200][200]bool = @splat(@splat(false));
    // var cnt: u32 = 0;
    ...
    for (0..q) |_| {
        ...
        if (ty == 1) {
            ...
            if (lastr[r]) |lr| {
                fw2.add(lr, -1);
                ans += @intCast(fw1.prefixSum(3 << 17).? - fw1.prefixSum(lr + 1).?);
            } else {
                ans += n;
            }
            fw2.add(t, 1);
            ...
            if (lastc[c]) |lc| {
                fw1.add(lc, -1);
                ans -= @intCast(fw2.prefixSum(3 << 17).? - fw2.prefixSum(lc + 1).?);
            } else {
                ans -= @intCast(fw2.prefixSum(3 << 17).?);
            }
            fw1.add(t, 1);
            lastc[c] = t;
