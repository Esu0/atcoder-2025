...
pub fn solve() !void {
    ...
    const y = readInt(u32);
    const z = readInt(u32);
    var a: [1001]u32 = undefined;
    var b: [1001]u32 = undefined;
    for (1..n+1) |i| a[i] = readInt(u32);
    for (1..n+1) |i| b[i] = readInt(u32);
    var ans: std.ArrayList(u32) = try .initCapacity(allocator, x + y + z);
    const rem = try allocator.alloc(u32, n);
    {var i: u32 = 0;
    while (i < n) : (i += 1) rem[i] = i + 1;}
    mem.sort(u32, rem, a[0..n+1], struct { fn lessThan(arr: []u32, lhs: u32, rhs: u32) bool {
        return arr[lhs] > arr[rhs];
    }}.lessThan);
    for (0..x) |i| ans.appendAssumeCapacity(rem[i]);
    mem.sortUnstable(u32, rem[x..], b[0..n+1], struct { fn lessThan(arr: []u32, lhs: u32, rhs: u32) bool {
        return if (arr[lhs] == arr[rhs]) lhs < rhs else arr[lhs] > arr[rhs];
    }}.lessThan);
    for (x..x+y) |i| ans.appendAssumeCapacity(rem[i]);
    mem.sortUnstable(u32, rem[x+y..], .{a[0..n+1], b[0..n+1]}, struct {fn lessThan(arr: struct {[]u32, []u32}, lhs: u32, rhs: u32) bool {
        return if (arr[0][lhs] + arr[1][lhs] == arr[0][rhs] + arr[1][rhs]) lhs < rhs else arr[0][lhs] + arr[1][lhs] > arr[0][rhs] + arr[1][rhs];
    }}.lessThan);
    for (x+y..x+y+z) |i| ans.appendAssumeCapacity(rem[i]);
    mem.sortUnstable(u32, ans.items, {}, std.sort.asc(u32));
    for (ans.items) |i| print("{d}\n", .{i});
}
