...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const q = readInt(u32);
    var b: [2<<17]u32 = @splat(math.maxInt(u32));
    for (0..m) |_| {
        const ai = readInt(u32) - 1;
        const bi = readInt(u32);
        b[ai] = @min(b[ai], bi);
    }

    var double: [18][2<<17]u32 = undefined;
    var min: u32 = math.maxInt(u32);
    double[0][n] = math.maxInt(u32);
    {var i = n; while (i > 0) {
        i -= 1;
        min = @min(min, b[i]);
        // std.debug.print("i: {d}, min: {d}\n", .{i, min});
        double[0][i] = min;
    }}

    for (1..18) |i| {
        for (0..n) |j| {
            var nxt = double[i - 1][j];
            // std.debug.print("i: {d}, j: {d}, nxt: {d}\n", .{i, j, nxt});
            if (nxt != math.maxInt(u32)) nxt = double[i - 1][nxt];
            double[i][j] = nxt;
        }
        double[i][n] = math.maxInt(u32);
    }
