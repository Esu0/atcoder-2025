...
var p2: [30]struct {u32, u32} = undefined;
var memo: std.AutoHashMap(u32, void) = undefined;

fn dfs(n: u32) void {
    const result = memo.getOrPut(n) catch unreachable;
    if (result.found_existing) return;
    for (p2) |elem| {
        const a, const b = elem;
        const new = @as(u64, n) * b + a;
...
pub fn solve() !void {
    comptime var power_of_two: [30]struct {u32, u32} = undefined;
    inline for (0..30) |i| {
        comptime var m = 1 << i;
        comptime var c = 1;
        inline while (m > 0) : (m /= 10) c *= 10;
        power_of_two[i] = .{ 1 << i, c };
    }
    p2 = power_of_two;
    memo = .init(allocator);
    dfs(0);
    var arr: std.ArrayList(u32) = try .initCapacity(allocator, memo.count());
    var iter = memo.keyIterator();
    while (iter.next()) |k| arr.appendAssumeCapacity(k.*);
