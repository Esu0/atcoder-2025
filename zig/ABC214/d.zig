...
pub fn solve() !void {
    const n = readInt(u32);
    var uvw: [1<<17]struct {u32, u32, u32} = undefined;
    for (0..n-1) |i| uvw[i] = .{ readInt(u32) - 1, readInt(u32) - 1, readInt(u32) };
    mem.sortUnstable(struct {u32, u32, u32}, uvw[0..n-1], {}, struct {fn lessThan(_: void, lhs: struct {u32, u32, u32}, rhs: struct {u32, u32, u32}) bool {
        return lhs[2] < rhs[2];
    }}.lessThan);
    var uf: Unionfind = try .init(allocator, n);
    var ans: u64 = 0;
    for (uvw[0..n - 1]) |uvwi| {
        const ui, const vi, const wi = uvwi;
        const ru = uf.find(ui);
        const rv = uf.find(vi);
        assert(ru != rv);
        ans += @as(u64, uf.size[ru]) * uf.size[rv] * wi;
        _ = uf.unite(ru, rv);
    }
    print("{d}\n", .{ans});
}
