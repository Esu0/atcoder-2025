...
pub fn solve() !void {
    const n = readInt(u32);
    var ans: [101]std.ArrayList(u32) = undefined;
    for (1..n+1) |i| ans[i] = .initBuffer(&.{});
    var i: u32 = 1;
    while (i <= n) : (i += 1) {
        const k = readInt(u32);
        for (0..k) |_| {
            const a = readInt(u32);
            ans[a].append(allocator, i) catch unreachable;
        }
    }
    i = 1;
    while (i <= n) : (i += 1) {
        print("{d}", .{ans[i].items.len});
        for (ans[i].items) |v| print(" {d}", .{v});
        print("\n", .{});
    }
}
