...
pub fn solve() !void {
    ...
    var b: [2<<17]u64 = undefined;
    var pcg = std.Random.DefaultPrng.init(@bitCast(std.time.timestamp()));
    const rng = pcg.random();
    var set: std.AutoHashMap(u32, u64) = .init(allocator);
    try set.ensureTotalCapacity(n);
    var h: u64 = 0;
    for (0..n) |i| {
        const ai = readInt(u32);
        const res = set.getOrPutAssumeCapacity(ai);
    ...
    h = 0;
    var set2: std.AutoHashMap(u32, void) = .init(allocator);
    try set2.ensureTotalCapacity(n);
    for (0..n) |i| {
        const bi = readInt(u32);
        const res = set2.getOrPutAssumeCapacity(bi);
        if (!res.found_existing) {
            const r = set.get(bi) orelse rng.int(u64);
            h ^= r;
        }
        b[i] = h;
    }
