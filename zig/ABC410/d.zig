...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var vis: [1000][1<<10]bool = @splat(@splat(false));
    var g: [1000]std.ArrayList(struct { u32, u32 }) = undefined;
    for (0..n) |i| g[i] = .initBuffer(&.{});
    for (0..m) |_| {
        const a = readInt(u32) - 1;
        const b = readInt(u32) - 1;
        const w = readInt(u32);
        try g[a].append(allocator, .{ b, w });
    }

    var buf: [1<<20]u32 = undefined;
    ...
    while (stack.pop()) |ux| {
        ...
        for (g[u].items) |vw| {
            ...
            const y = x ^ vw[1];
            if (vis[v][y]) continue;
            stack.appendAssumeCapacity((v << 10) | y);
            vis[v][y] = true;
        }
    }
    if (mem.indexOfScalar(bool, &vis[n - 1], true)) |i| {
        print("{d}\n", .{i});
    } else {
