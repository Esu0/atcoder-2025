...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var dp: [11][11][11]MInt = @splat(@splat(@splat(.zero)));
    dp[m][m][m] = .one;
    for (0..n) |_| {
        var ndp: [11][11][11]MInt = @splat(@splat(@splat(.zero)));
        var i: u8 = 0;
        while (i <= m) : (i += 1) {
            var j: u8 = i;
            while (j <= m) : (j += 1) {
                ...
                while (k <= m) : (k += 1) {
                    ...
                    while (a < m) : (a += 1) {
                        var arr = [_]u8{i, j, k};
                        var idx: u32 = 0;
                        while (idx < arr.len and arr[idx] < a) idx += 1;
                        if (idx == 3) continue;
                        arr[idx] = @intCast(a);
                        ndp[arr[0]][arr[1]][arr[2]] = ndp[arr[0]][arr[1]][arr[2]].add(dp[i][j][k]);
                    }
                }
            }
        }
    ...
    while (i < m) : (i += 1) {
        ...
        while (j < m) : (j += 1) {
            var k: u8 = j;
            while (k < m) : (k += 1) ans = ans.add(dp[i][j][k]);
        }
    }
    print("{d}\n", .{ans.value});
}
