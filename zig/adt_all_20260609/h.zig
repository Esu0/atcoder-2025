...
pub fn solve() !void {
    const n = readInt(u32);
    var trie: std.ArrayList([26]u32) = try .initCapacity(allocator, 500_001);
    var cnt: [500_001]u32 = @splat(0);
    var s: [500_000][]u8 = undefined;

    trie.appendAssumeCapacity(@splat(math.maxInt(u32)));
    for (0..n) |i| {
        s[i] = readString();
        var cur: u32 = 0;
        cnt[cur] += 1;
        for (s[i]) |c| {
            const di = c - 'a';
            if (trie.items[cur][di] == math.maxInt(u32)) {
                trie.items[cur][di] = @intCast(trie.items.len);
                trie.appendAssumeCapacity(@splat(math.maxInt(u32)));
            }
            cur = trie.items[cur][di];
            cnt[cur] += 1;
        }
    }
    for (0..n) |i| {
