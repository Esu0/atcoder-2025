...
pub fn solve() !void {
    const n = readInt(u32);
    const m = readInt(u32);
    var row: [3<<17]std.AutoHashMap(u32, void) = undefined;
    var col: [3<<17]std.AutoHashMap(u32, void) = undefined;
    for (0..n) |i| {
        row[i] = .init(allocator);
        col[i] = .init(allocator);
    }
    for (0..m) |_| {
        const r = readInt(u32) - 1;
        const c = readInt(u32) - 1;
        var iter = row[r].keyIterator();
        while(iter.next()) |c0| {
            _ = col[c0.*].remove(r);
        }
        row[r].clearRetainingCapacity();
        iter = col[c].keyIterator();
        while(iter.next()) |r0| {
            _ = row[r0.*].remove(c);
        }
        col[c].clearRetainingCapacity();
        try row[r].putNoClobber(c, {});
        try col[c].putNoClobber(r, {});
    }
    var ans: u32 = 0;
    for (0..n) |i| ans += row[i].count();
    print("{d}\n", .{ans});
}
