...
pub fn solve() !void {
    ...
    inline for (0..5) |i| a[i] = readInt(u32);
    mem.sortUnstable(u32, &a, {}, std.sort.asc(u32));
    if (mem.allEqual(u32, &a, a[0])) {
        try stdout.writeAll("No\n");
        return;
    }
    try stdout.writeAll(if (a[0] == a[1] and a[2] == a[3] and a[3] == a[4] or a[0] == a[1] and a[1] == a[2] and a[3] == a[4]) "Yes\n" else "No\n");
}
