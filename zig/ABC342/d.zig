...
pub fn solve() !void {
    const n = readInt(u32);
    var cnt: [2<<17]u32 = @splat(0);
    for (0..n) |_| {
        const a = readInt(u32);
        cnt[a] += 1;
    }
    var is_prime: [2<<17]bool = @splat(true);
    is_prime[0] = false;
    is_prime[1] = false;
    {var p: u32 = 2; while (p < is_prime.len) : (p += 1) {
        if (!is_prime[p]) continue;
        var j: u32 = 2;
        while (j * p < is_prime.len) : (j += 1) {
            is_prime[j * p] = false;
        }
        while (j > 1) {
            j -= 1;
    ...
    var ans: u64 = 0;
    for (cnt) |ci| {
        if (ci > 0) ans += @as(u64, ci) * (ci - 1) / 2;
    }
    ans += @as(u64, cnt[0]) * (n - cnt[0]);
    print("{d}\n", .{ans});
}
