...
pub fn solve() !void {
    const n = readInt(u32);
    var dp: [501][501]u8 = @splat(@splat(101));
    dp[0][0] = 0;
    var sum: u32 = 0;
    for (0..n) |_| {
        const a = readInt(u4);
        const b = readInt(u32);
        var ndp: [501][501]u8 = @splat(@splat(101));
        defer dp = ndp;
        const c3: u1 = if (a == 3) 0 else 1;
        for (0..501) |i| {
            for (0..501) |j| {
                ndp[i][j] = dp[i][j] + c3;
            }
        ...
        if (b > 500) continue;
        const c2: u1 = if (a == 2) 0 else 1;
        const c1: u1 = if (a == 1) 0 else 1;
        for (0..501) |i| {
            for (0..501 - b) |j| {
                ndp[i][j + b] = @min(ndp[i][j + b], dp[i][j] + c2);
            }
        }
        for (0..501 - b) |i| {
            for (0..501) |j| {
                ndp[i + b][j] = @min(ndp[i + b][j], dp[i][j] + c1);
            }
        }
        sum += b;
    }
    if (sum % 3 == 0 and dp[sum/3][sum/3] <= n) {
        print("{d}\n", .{dp[sum/3][sum/3]});
    } else {
