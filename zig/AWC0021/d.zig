...
pub fn solve() !void {
    ...
    const m = readInt(u32);
    const k = readInt(u32);
    var g: [50000]std.ArrayList(struct{u32, u32}) = undefined;
    ...
    var p: [10]u32 = undefined;
    for (0..k) |i| p[i] = readInt(u32) - 1;

    var dist: [11][50000]u64 = @splat(@splat(math.maxInt(u64)));
    var queue: std.PriorityQueue(struct {u64, u32, u32}, void, struct {fn compare(_: void, lhs: struct {u64, u32, u32}, rhs: struct {u64, u32, u32}) math.Order {
        return math.order(lhs[0], rhs[0]);
    }}.compare) = .init(allocator, {});
    try queue.ensureTotalCapacity((n + m)*(k+1));
    {
        var x: u32 = 0;
        while (x < k and p[x] == 0) x += 1;
        try queue.add(.{ 0, x, 0 });
    }

    while (queue.removeOrNull()) |elem| {
        const d, const t, const u = elem;
        if (dist[t][u] != math.maxInt(u64)) continue;
        dist[t][u] = d;
        for (g[u].items) |e| {
            const to, const c = e;
            var nt = t;
            while (nt < k and p[nt] == to) nt += 1;
            if (dist[nt][to] == math.maxInt(u64)) {
                try queue.add(.{ d + c, nt, to });
            }
        }
    }

    print("{d}\n", .{@as(i64, @bitCast(dist[k][n - 1]))});
}
