...
pub fn solve() !void {
    ...
    var idxs: [26]std.ArrayList(u32) = undefined;
    for (0..idxs.len) |i| idxs[i] = try .initCapacity(allocator, s.len*2);
    for (0.., s) |i, si| {
        idxs[si].appendAssumeCapacity(@intCast(i));
    }
    for (0..idxs.len) |i| {
        const l = idxs[i].items.len;
        for (0..l) |j| idxs[i].appendAssumeCapacity(@intCast(s.len + idxs[i].items[j]));
    }

    var ans: u64 = 0;
    var ofs: u32 = 0;
    for (t) |ti| {
        if (idxs[ti].items.len == 0) {
            print("-1\n", .{});
            return;
        }
        const k = std.sort.lowerBound(u32, idxs[ti].items, ofs, struct {fn compare(lhs: u32, rhs: u32) math.Order {
            return math.order(lhs, rhs);
        }}.compare);
        ans += idxs[ti].items[k] + 1 - ofs;
        ofs = (idxs[ti].items[k] + 1) % @as(u32, @intCast(s.len));
    }
    print("{d}\n", .{ans});
}
