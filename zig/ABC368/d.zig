...
pub fn solve() !void {
    ...
    var parent: [2<<17]u32 = @splat(math.maxInt(u32) - 1);
    parent[r] = math.maxInt(u32);
    while (stack.pop()) |u| {
        for (g[u].items) |v| {
            if (parent[v] != math.maxInt(u32) - 1) continue;
            parent[v] = u;
    ...
    // std.debug.print("{any}\n", .{parent[0..n]});
    var used: [2<<17]bool = @splat(false);
    used[r] = true;
    var ans: u32 = 1;
    for (1..k) |_| {
        var v = readInt(u32) - 1;
        while (!used[v]) : (v = parent[v]) {
            used[v] = true;
            ans += 1;
        }
    }
