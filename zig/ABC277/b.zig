...
pub fn solve() !void {
    const n = readInt(u32);
    var set: [128*128]bool = @splat(false);
    for (0..n) |_| {
        const s = readString();
        assert(s.len == 2);
        if (mem.containsAtLeastScalar(u8, "HDCS", 1, s[0]) and mem.containsAtLeastScalar(u8, "A23456789TJQK", 1, s[1])) {
            if (set[@as(usize, s[0]) * 128 + s[1]]) {
                try stdout.writeAll("No\n");
                return;
            } else {
                set[@as(usize, s[0]) * 128 + s[1]] = true;
            }
        } else {
            try stdout.writeAll("No\n");
            return;
