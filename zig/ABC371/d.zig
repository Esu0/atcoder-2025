...
pub fn solve() !void {
    ...
    var p = try allocator.alloc(u32, n);
    for (0..n) |i| x[i] = readInt(i32);
    for (0..n) |i| p[i] = readInt(u32);
    var id = try allocator.alloc(u32, n);
    {var i: u32 = 0; while (i < n) : (i += 1) id[i] = i;}
    mem.sortUnstable(u32, id, x, struct { fn lessThan(pos: []i32, lhs: u32, rhs: u32) bool {
        return pos[lhs] < pos[rhs];
    }}.lessThan);
    var cumsum = try allocator.alloc(u64, n + 1);
    cumsum[0] = 0;
    {var i: u32 = 0; while (i < n) : (i += 1) {cumsum[i + 1] = p[id[i]] + cumsum[i];}}

    const q = readInt(u32);
    for (0..q) |_| {
        const l = readInt(i32);
        const r = readInt(i32);
        const il = std.sort.lowerBound(u32, id, .{l, x}, struct {fn compare(ctx: struct {i32, []i32}, val: u32) math.Order {
            return math.order(ctx[0], ctx[1][val]);
        }}.compare);
        const ir = std.sort.upperBound(u32, id, .{r, x}, struct {fn compare(ctx: struct {i32, []i32}, val: u32) math.Order {
            return math.order(ctx[0], ctx[1][val]);
        }}.compare);
        print("{d}\n", .{cumsum[ir] - cumsum[il]});
    }
