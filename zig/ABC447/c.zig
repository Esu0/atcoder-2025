...
pub fn solve() !void {
    ...
    while (true) {
        var ca: u32 = 0;
        while (i < s.len and s[i] == 'A') : (i += 1) ca += 1;
        var cb: u32 = 0;
        while (j < t.len and t[j] == 'A') : (j += 1) cb += 1;
        const c1 = if (i < s.len) s[i] else 0;
        const c2 = if (j < t.len) t[j] else 0;
        if (c1 != c2) {
            try stdout.writeAll("-1\n");
            return;
        }
        ans += if (ca > cb) ca - cb else cb - ca;
        if (c1 == 0) break;
        i += 1;
        j += 1;
    }
