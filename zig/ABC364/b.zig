...
pub fn solve() !void {
    const h = readInt(u32);
    const w = readInt(u32);
    var px = readInt(u32) - 1;
    var py = readInt(u32) - 1;
    var grid: [50][]u8 = undefined;
    for (0..h) |i| {
        grid[i] = readString();
        assert(grid[i].len == w);
    }
    ...
    for (x) |xi| {
        switch (xi) {
            'U' => {if (px > 0 and grid[px - 1][py] == '.') px -= 1;},
            'D' => {if (px + 1 < h and grid[px + 1][py] == '.') px += 1;},
            'R' => {if (py + 1 < w and grid[px][py + 1] == '.') py += 1;},
            'L' => {if (py > 0 and grid[px][py - 1] == '.') py -= 1;},
            else => unreachable,
        }
    }
    print("{d} {d}\n", .{px + 1, py + 1});
