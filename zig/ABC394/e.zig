...
pub fn solve() !void {
    ...
    var vis: [100][100]bool = @splat(@splat(false));
    var dist: [100][100]u32 = @splat(@splat(math.maxInt(u32)));
    for (0..n) |i| {
        for (0..n) |j| {
            if (c[i][j] != '-') {
                dist[i][j] = 1;
    ...
    while (true) {
        ...
        for (0..n) |i| {
            for (0..n) |j| {
                if (!vis[i][j]) min = @min(min, dist[i][j]);
            }
        }
        if (min == math.maxInt(u32)) break;
        for (0..n) |i| {
            for (0..n) |j| {
                if (!vis[i][j] and dist[i][j] == min) {
                    vis[i][j] = true;
                    for (0..n) |k| {
                        if (c[k][i] == '-') continue;
                        for (0..n) |l| {
                            if (c[k][i] == c[j][l]) {
                                dist[k][l] = @min(dist[k][l], min + 2);
                            }
                        }
                    }
                }
    ...
    for (0..n) |i| {
        for (0..n) |j| {
            if (dist[i][j] == math.maxInt(u32)) try stdout.writeAll("-1 ") else print("{d} ", .{dist[i][j]});
        }
