...
pub fn solve() !void {
    ...
    @memset(s[h+1][0..w+2], 0);

    var queue = FixedQueue(u32, 1<<22){};
    var dist: [1<<10][1<<10]u32 = @splat(@splat(math.maxInt(u32)));
    dist[1][1] = 0;
    queue.push((1<<10)+1);
    ...
    while (queue.pop()) |u| {
        ...
        inline for (0..4) |k| {
            const ni = i +% dij[k];
            const nj = j +% dij[k+1];
            if (s[ni][nj] != 0 and dist[ni][nj] == math.maxInt(u32) and s[ni][nj] != s[i][j]) {
                dist[ni][nj] = d + 1;
                queue.push((ni << 10) | nj);
            }
        }
    ...
    print("{d}\n", .{if (dist[h][w]==math.maxInt(u32)) -1 else @as(i32, @intCast(dist[h][w]))});
}
