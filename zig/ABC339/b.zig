...
pub fn solve() !void {
    ...
    const w = readInt(u32);
    const n = readInt(u32);
    var ans: [100][100]bool = @splat(@splat(false));
    const dirs: [4]struct {u32, u32} = .{
        .{h - 1, 0},
        .{0, 1},
        .{1, 0},
        .{0, w - 1},
    ...
    for (0..n) |_| {
        ...
        ans[cur[0]][cur[1]] ^= true;
        cur[0] = (cur[0] + dirs[dir][0]) % h;
        cur[1] = (cur[1] + dirs[dir][1]) % w;
    }
    for (0..h) |i| {
        for (0..w) |j| try stdout.writeByte(if (ans[i][j]) '#' else '.');
