...
fn get(dp: *[60][60][60][60]u32, idx: struct {struct {u16, u16}, struct {u16, u16}}) *u32 {
    return &dp[idx[0][0]-1][idx[0][1]-1][idx[1][0]-1][idx[1][1]-1];
}

pub fn solve() !void {
    const n = readInt(u32);
    var s: [62][62]u8 = undefined;
    var dp: [60][60][60][60]u32 = @splat(@splat(@splat(@splat(math.maxInt(u32)))));
    var queue: FixedQueue(struct {struct {u16, u16} , struct {u16, u16}}, 4*60*60*60*60) = .{};
    var p1: struct {u16, u16} = .{0, 0};
    var p2: struct {u16, u16} = .{0, 0};
    @memset(s[0][0..n+2], '#');
    @memset(s[n+1][0..n+2], '#');
    for (1..n+1) |i| {
        const si = readString();
        ...
        for (1..n+1) |j| {
            if (s[i][j] == 'P') {
                if (p1[0] == 0) {
                    p1 = .{@intCast(i), @intCast(j)};
                } else {
                    p2 = .{@intCast(i), @intCast(j)};
                }
            }
        }
    }
    queue.push(.{p1, p2});
    dp[p1[0]-1][p1[1]-1][p2[0]-1][p2[1]-1] = 0;
    const dij: [5]u16 = .{0, 1, 0, math.maxInt(u16), 0};
    while (queue.pop()) |u| {
        const pos1, const pos2 = u;
        const d = get(&dp, u).*;
        for (0..4) |k| {
            const di = dij[k];
            const dj = dij[k+1];
            var np1 = .{ pos1[0] +% di, pos1[1] +% dj };
            var np2 = .{ pos2[0] +% di, pos2[1] +% dj };
            if (s[np1[0]][np1[1]] == '#') np1 = pos1;
            if (s[np2[0]][np2[1]] == '#') np2 = pos2;
            const ptr = get(&dp, .{np1, np2});
            if (ptr.* == math.maxInt(u32)) {
                ptr.* = d + 1;
                queue.push(.{np1, np2});
            }
        }
