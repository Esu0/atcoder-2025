...
pub fn solve() !void {
    const n = readInt(u32);
    var c: [2<<17]u30 = undefined;
    for (0..n) |i| c[i] = readInt(u30);
    mem.sortUnstable(u30, c[0..n], {}, std.sort.desc(u30));
    const p2n = MInt.fromRaw(2).pow(n);
    var p2 = p2n;
    ...
    for (0..n) |i| {
        p2 = p2.mul(inv2);
        ans = s.mul(.fromRaw(c[i])).mul(p2).add(ans);
        s = s.add(s).add(t);
        t = t.add(t);
    }
    print("{d}\n", .{ans.mul(MInt.fromRaw(2).pow(n)).value});
}
