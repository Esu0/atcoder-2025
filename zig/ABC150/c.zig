...
fn ord(p: []u8) u32 {
    ...
    while (i < p.len) : (i += 1) {
        ...
        var j: u32 = 0;
        var c: u32 = 0;
        while (j < pi) : (j += 1) c += @intFromBool(e[j]);
        cnt += c * fact(@intCast(p.len - i - 1));
        e[pi] = false;
    }
    return cnt;
}
...
pub fn solve() !void {
    ...
    for (0..n) |i| p[i] = readInt(u8) - 1;
    for (0..n) |i| q[i] = readInt(u8) - 1;
    
    const a = ord(p[0..n]);
    const b = ord(q[0..n]);
    // std.debug.print("ord: {d} {d}\n", .{a, b});
    print("{d}\n", .{if (a > b) a - b else b - a});
}
